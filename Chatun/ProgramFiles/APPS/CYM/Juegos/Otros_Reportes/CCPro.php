...
WHERE b.ACC_FECHA = '".$_POST["FechaInicio"]."' AND b.ACC_TIPO = 3"));
...
<body class="menubar-hoverable header-fixed menubar-pin ">
	...
	<?php include("../../../../MenuTop.php")?>
		...
		<div id="content">
			...
			if ($id_user==23783 || $id_user==22045 || $id_user==53711 || $id_user==435849 || $id_user==141797 || $id_user==23118|| $id_user==31121) 
		{ 
				...
				$sql = "SELECT APERTURA_CIERRE_CAJA.*, CIERRE_DETALLE.* 
						...
						AND APERTURA_CIERRE_CAJA.ACC_TIPO = 3  $filtroOtro";
				...
				$Query = "SELECT COUNT(F_CODIGO) AS FACTURAS_EMITIDAS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser";
				...
				$Query1 = "SELECT COUNT(F_CODIGO) AS FACTURAS_ANULADAS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_ESTADO = 2";
				...
				$Query3 = "SELECT SUM(F_TOTAL) AS TOTAL_CREDITO FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_TIPO = 2";
				...
				$Query4 = "SELECT SUM(F_TOTAL) AS TOTAL_DOLARES FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_TIPO = 1 AND F_MONEDA = 2";
				...
				$Query4 = "SELECT SUM(F_TOTAL) AS TOTAL_LEMPIRAS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_TIPO = 1 AND F_MONEDA = 3";
				$Result4 = mysqli_query($db,$Query4);
				while($row4 = mysqli_fetch_array($Result4))
				{
				...
				$Query5 = "SELECT SUM(F_TOTAL) AS TOTAL_DEPOSITOS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_TIPO = 4";
				...
				$Query6 = "SELECT SUM(F_TOTAL) AS TOTAL_CREDITOS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_TIPO = 3";
				...
				$Query7 = "SELECT SUM(F_TOTAL) AS TOTAL_EFECTIVO FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_MONEDA = 1";
				...
				$Query9 = "SELECT SUM(F_CAMBIO) AS TOTAL_CAMBIO_LEMPIRA FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_MONEDA = 3";
				...
				$Query10 = "SELECT SUM(F_CAMBIO) AS TOTAL_CAMBIO_DOLARES FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser AND F_MONEDA = 2";
				...
				$Query11 = "SELECT SUM(F_TOTAL) AS TOTAL_FACTURADO FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' $FiltroUser";
								...
								<div id="accordion6-3" class="collapse" aria-expanded="false" style="height: 0px;">
									<div class="card-body">
										<table class="table" id="tbl_resultados">
											...
											<tbody>
												<?php
													$SqlFacturas = "SELECT FACTURA_HS.* FROM Bodega.FACTURA_HS
																	WHERE FACTURA_HS.F_FECHA_TRANS = '".$_POST["FechaInicio"]."' $FiltroUser
																	ORDER BY FACTURA_HS.F_SERIE, FACTURA_HS.F_NUMERO";
		...
		echo '<div class="col-lg-12 text-center">
			...
			<h2 class="text-light">Lo sentimos, no tienes permiso de ver este reporte.</h2>
		</div>';
}
?>
		</form>
