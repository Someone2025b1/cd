			...
			<div class="container">
				...
				<div class="col-lg-12">
					<div class="card">
						...
						<a href="IFProGuardar.php" style="margin: 12px;"><button type="button" class="btn btn-warning">
														    Nuevo Inventario
														  </button></a>
						</div>
						<div class="card-body">
						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								<thead>
                                    ...
                                    <th colspan="2"><strong  align="center"><h6 align="center"> Editar</h1></strong></th>
								</thead>
                                
								<tbody>
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Productos.INVENTARIO AS A WHERE A.I_ESTADO = 0 AND A.I_PUNTO = 6 ORDER BY A.I_FECHA ";
									...
									while($row = mysqli_fetch_array($Resultado))
									{
										...
										$EN = $row['I_PUNTO'];
                                                ...
                                                echo '<td style="font-size: 10px">'.$row["I_CODIGO"].'</td>';
										    	...
										    	echo '<td style="font-size: 10px">'.$row["I_OBSERVACIONES"].'</td>';
                                                echo '<td style="font-size: 10px">'.$row["I_FECHA"].' a las: '.$row["I_HORA"].' horas</td>';
										    	echo '<td><a href="IFProEditar.php?Codigo='.$Codigo.'"><button type="button" class="btn btn-warning">
														     Editar Conteo
														  </button></a>
														</td>';
									...
									$Consulta = "SELECT * FROM Productos.INVENTARIO_RECONTEO AS A WHERE A.I_ESTADO = 0 AND A.I_PUNTO = 6 ORDER BY A.I_FECHA ";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
										$Codigo = $row["IR_CODIGO"];
										$EN = $row['I_PUNTO'];
                                        										echo '<tr>';
										    	echo '<td style="font-size: 10px">'.$Contador.'</td>'; 
                                                echo '<td style="font-size: 10px">'.$row["IR_CODIGO"].'</td>';
                                                $ListEvento = mysqli_query($db, "SELECT a.PV_NOMBRE FROM CompraVenta.PUNTO_VENTA a WHERE a.PV_CODIGO =".$EN);
												while ($RowEvento = mysqli_fetch_array($ListEvento))
												{
													$Envia = $RowEvento["PV_NOMBRE"];
										    	...
										    	echo '<td style="font-size: 10px">'.$row["I_OBSERVACIONES"].'</td>';
                                                echo '<td style="font-size: 10px">'.$row["I_FECHA"].' a las: '.$row["I_HORA"].' horas</td>';
										    	echo '<td><a href="IFProEditarReconteo.php?Codigo='.$Codigo.'"><button type="button" class="btn btn-warning">
														     Editar Conteo
														  </button></a>
														</td>';
