				...
				while($row = mysqli_fetch_array($result))
				{
					...
					$NombreAre=$row["AR_NOMBRE"];
								...
								for($i=0; $i<$Contador; $i++)
								{
									...
									$queryAbonoProd = mysqli_query($db, "UPDATE RRHH.EVALUACION_DETALLE SET EDD_PUNTEO = '".$Pun."'
									WHERE ED_CODIGO = '".$CodEvaluacion."'
									AND AR_CODIGO=$CodAs
									AND ARE_CODIGO =$CodAre");
						...
						while($row = mysqli_fetch_array($result))
						{
							...
							$NombreAre=$row["AR_NOMBRE"];
										...
										$Observaciones  = $_POST["Observaciones".$CodAre];
											...
											$queryAbonoProd = mysqli_query($db, "UPDATE RRHH.EVALUACION_DES_RESUMEN SET EVDR_PROMEDIO = '".$Promedio."', EVDR_PUNTEO = '".$Punteo."', EVDR_PUNTEO_POS = '".$PuntosPosibles."', EVDR_OBSERVACIONES = '".$Observaciones."'
											WHERE ED_CODIGO = '".$CodEvaluacion."'
											AND AR_CODIGO = $CodAre");
						...
						$queryAbonoProd = mysqli_query($db, "UPDATE RRHH.EVALUACION_DESEMPENO SET ED_PROMEDIO = '".$PromedioFinal."', ED_FECHA_MOD = CURRENT_DATE()
						WHERE ED_CODIGO = '".$CodEvaluacion."'");
