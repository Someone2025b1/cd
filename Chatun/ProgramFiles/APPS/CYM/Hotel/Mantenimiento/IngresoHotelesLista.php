					...
					<div class="card">
						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								...
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Taquilla.INGRESO_HOTEL AS A WHERE A.IH_FECHA BETWEEN '$fecha_inicio' AND '$fecha_fin' ORDER BY A.IH_FECHA ";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
										$Codigo = $row["IH_ID"];
										$EID = $row['H_CODIGO'];
										$Col = $row["IH_COLABORADOR"];										echo '<tr>';
										    	echo '<td style="font-size: 10px">'.$Contador.'</td>'; 

												$ListEvento = mysqli_query($db, "SELECT a.H_NOMBRE FROM Taquilla.HOTEL a WHERE a.H_CODIGO =".$EID);
												while ($RowEvento = mysqli_fetch_array($ListEvento))
												{
													$IDE = $RowEvento["H_NOMBRE"];
												}



										    	echo '<td style="font-size: 10px">'.$IDE.'</td>';
										    	echo '<td style="font-size: 10px">'.$row["IH_VALE"].'</td>';
                                                ...
                                                echo '<td style="font-size: 10px">'.$NombreGenero.'</td>';
                                                echo '<td style="font-size: 10px">'.$row["IH_FECHA"].'</td>';
										    	echo '<td><a href="IngresoHotelesMod.php?Codigo='.$Codigo.'"><button type="button" class="btn btn-warning btn-xs">
