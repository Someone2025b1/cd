<?php
      ...
      $query_services = mysqli_query($db, "SELECT * FROM Bodega.PRODUCTO WHERE (P_NOMBRE LIKE '%" . $search . "%') AND (CP_CODIGO = 'EVE' OR CP_CODIGO = 'HS') ORDER BY P_CODIGO LIMIT 100");
      ...
      $query_recetas = mysqli_query($db, "SELECT * FROM Bodega.RECETA_SUBRECETA WHERE RS_NOMBRE LIKE '%" . $search . "%' AND (RS_BODEGA = 'TR' OR RS_BODEGA = 'HS') AND RS_TIPO = 1 ORDER BY RS_NOMBRE LIMIT 100");
