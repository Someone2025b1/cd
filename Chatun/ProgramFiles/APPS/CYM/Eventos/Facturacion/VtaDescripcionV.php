...
#purchaseOrder {
	...
	<script>
		function BuscarProducto(x)
        {
		        ...
		        var EligeSabor = document.getElementsByName('EligeSabor[]');
		        var service = x.value;
		            ...
		            success: function(data) {
		            	...
		            	else
		            	{
			                ...
			                $('.suggest-element').click(function(){
			                    ...
			                    EligeSabor[Indice].value = $(this).attr('dataSabor');
			                    ...
			                    if($(this).attr('dataSabor') > 0)
			                    {
			                    	...
			                    	ModalElegirSabores(Sabores, Indice);
			                    }

								if($(this).attr('dataYogurt') > 0)
			                    {
			                    	Sabores1 = $(this).attr('dataFruta');
			                    	ModalElegirYogurt(Sabores1, Indice);
			                    }
			...
			function ModalElegirSabores(x, y)
		{
			$('#ModalSugerencias').modal('hide');
		...
		function ModalElegirYogurt(x, y)
		{
			$('#ModalSugerencias').modal('hide');
			$('#ModalYogurt').modal('show');

			AddLine();

			$.ajax({
		            type: "POST",
		            url: "AgregarSaboresYogurt.php",
		            data: "Cantidad="+x+"&Fila="+y,
		            beforeSend: function()
		            {
		            	$('#suggestions').html('<img src="../../../../../img/Preloader.gif" />');
		            ...
		            success: function(data) {
		            	$('#SelectsSaboresYogurt').html(data);
		            }
		        });
		...
		function AgregarSaboresYogurt(x)
		{
			if( $('#FormularioSaboresYogurt')[0].checkValidity() )
			{
				var SaborHelado = document.getElementsByName('TipoYogurt[]');
				...
				for(i = 0; i< Contador; i++)
				{
					...
					$.ajax({
			            ...
			            data: "Codigo="+AtributoCodigo+"&Fila="+AtributoFila,
			            beforeSend: function()
			            {
			            	$('#suggestions').html('<img src="../../../../../img/Preloader.gif" />');
			            },
				...
				$('#ModalYogurt').modal('hide');
			}
			else
			{
				$('#FormularioSaboresYogurt').find(':submit').click();
			}
			//Filas de la tabla Sabores
			//var FilasTabla = document.getElementById('TablaSabores').rows.length;
		}
		...
		function AgregarNombreSabor(x)
		{

			var ProductoNombre = document.getElementsByName('ProductoNombre[]');
			var Fila = $(x).attr('data');
			var Elemento = ProductoNombre[Fila];
			var Valor = ProductoNombre[Fila].value;
			var Texto = x.selectedOptions[0].text;
			var TextoFull = ProductoNombre[Fila].value + ', '+Texto;


			ProductoNombre[Fila].value = TextoFull;
			
		}
		function AgregarNombreYogurt(x)
		{
		...
		function CRD()
		{
			...
			$('#TotalFacturaCobrarMixto').val(TotalDescuento.toFixed(2));
		}
		...
		function CompCredito(x){
			
			var NIT = x;
			
			$.ajax({
				url: 'BuscarNitCliente.php',
				type: 'POST',
				dataType: 'html',
				data: {NIT:NIT},
				success:function(data)
				{
					if (data==1) 
					{
						$('#DivCredito').modal('show');
					}else{
						$('#DivCredito').modal('hide');
					}
		...
		function CalcularTotal()
		{
			...
			var CantidadDes = document.getElementsByName('CantidadDes[]');
			var SubtotalDescr = document.getElementsByName('SubtotalDescr[]');
			var PrecioDes = document.getElementsByName('PrecioDes[]');
			
			var Total = 0;
			var TotalDes = 0;
			var PrecioDesCalculado = 0;
			...
			for(j = 1; j < SubtotalDescr.length; j++)
			{
				PrecioDesCalculado =  parseFloat(SubtotalDescr[j].value)/parseFloat(CantidadDes[j].value);
				PrecioDes[j].value = PrecioDesCalculado.toFixed(4);
				TotalDes = TotalDes + parseFloat(SubtotalDescr[j].value);
			...
			$('#TotalDescripcion').val(TotalDes.toFixed(2));
			
			$('#TotalFacturaCobrarTCredito').val(Total.toFixed(2));
			$('#TotalFacturaCobrarCredito').val(Total.toFixed(2));
			$('#TotalFacturaCobrarDeposito').val(Total.toFixed(2));
			$('#TotalFacturaCobrarMixto').val(Total.toFixed(2));
		...
		///Parte de Mixto buelto #base--

		function AgregarLineaA()
		{
			$("#tablaAnticipo tbody tr:eq(0)").clone().removeClass('fila-base-anticipo').appendTo("#tablaAnticipo tbody");
		}

		function AgregarLineaD()
		{
			$("#tableDescripcion tbody tr:eq(0)").clone().removeClass('fila-baseD').appendTo("#tableDescripcion tbody");
		}
		
		function EliminarLineaD(x)
		{
			var Indice = $(x).closest('tr').index();
			var parent = $(x).parents().get(1);
                $(parent).remove();
		...
		function BuscarAnticipo(x)
        {

				//Obtenemos el value del input
		        var NombreA = document.getElementsByName('NombreA[]');
		        var Monto = document.getElementsByName('MontoMA[]');
		        var CodigoA = document.getElementsByName('CodigoA[]');
		        var service = x.value;
		        var dataString = 'service='+service;
		        var Indice = $(x).closest('tr').index();
		        //Le pasamos el valor del input al ajax
		        $.ajax({
		            type: "POST",
		            url: "BuscarAnticipo.php",
		            dataNombre: dataString,
		            beforeSend: function()
		            {
		            	$('#suggestions').html('<img src="../../../../../img/Preloader.gif" />');
		            },
		            success: function(dataNombre) {
		            	...
		            	{
		            		$('#ModalSugerenciasAnticipo').modal('show');
			                //Escribimos las sugerencias que nos manda la consulta
			                $('#suggestionsAn').fadeIn(1000).html(dataNombre);
			                //Al hacer click en algua de las sugerencias
			                $('.suggest-element').click(function(){
			                    x.value = $(this).attr('dataNombre');
			                    Monto[Indice].value = $(this).attr('dataMonto');
			                    CodigoA[Indice].value = $(this).attr('idA');
			                    //Hacemos desaparecer el resto de sugerencias
			                    $('#suggestionsAn').fadeOut(500);
			                    $('#ModalSugerenciasAnticipo').modal('hide');
			                    SumaMixto();			                    
			                });
			            }
		            }
		...
		function TotalMixto(){
			var TotalM = $('#BoldTotal').val();
			var TotalFacturaM = $('#TotalFacturaCobrarMixto').val();

			TotalFacturaM.value = TotalM.toFixed(2);
		}

		function SumaMixto()
		{
			var MontoEfectivo = $('#MontoME').val();
			var MontoTarjeta     = $('#MontoMT').val();
			var MontoBanco     = $('#MontoMB').val();
			var TotalSumaM  = 0;
			var MontoAnticipo = 0;
			var TotalM       = document.getElementById('TotalEfectivoM');

			var MontoA   = document.getElementsByName('MontoMA[]');

			for(i = 1; i < MontoA.length; i++)
			{
				MontoAnticipo = MontoAnticipo + parseFloat(MontoA[i].value);
			}
			

			TotalSumaM =  parseFloat(MontoEfectivo) + parseFloat(MontoTarjeta) + parseFloat(MontoBanco) + parseFloat(MontoAnticipo);

			TotalM.value = TotalSumaM.toFixed(2);
		...
		function CalcularCambio2()
		{
			...
			var TotalFacturaM = $('#TotalFacturaCobrarMixto').val();
			var EfectivoM     = $('#TotalEfectivoM').val();
			var TotalCambioM  = 0;
			var CambioM       = document.getElementById('CambioM');
			

			TotalCambioM =  parseFloat(EfectivoM) - parseFloat(TotalFacturaM);

			CambioM.value = TotalCambioM.toFixed(2);
...
<body class="menubar-hoverable header-fixed menubar-pin " onload="CalcularTotal()">
	...
	$ConsultaApertura = "SELECT ACC_CODIGO FROM Bodega.APERTURA_CIERRE_CAJA WHERE ACC_FECHA = CURRENT_DATE AND ACC_TIPO = 8";
	...
	$ConsultaCierre = "SELECT ACC_CODIGO FROM Bodega.APERTURA_CIERRE_CAJA WHERE ACC_FECHA = CURRENT_DATE AND ACC_TIPO = 8 AND ACC_ESTADO = 2";
								...
								<div class="card">
									<div class="card-body ">
										<div id="rootwizard2" class="form-wizard form-wizard-horizontal">
											<form class="form form-validation" role="form" novalidate="novalidate" action="VtaDescripcionVPro.php" method="POST" role="form" id="FormularioPrincipal">
												...
												<div class="tab-content clearfix">
													<div class="tab-pane active" id="step1">
														<div class="row">
															...
															<div class="col-lg-2 text-center" name="DivNIT">
														...
														<label class="radio-inline radio-styled" >
															<input type="radio" value="N" name="TipoCompra" onChange="ComprobarTipo(this)" required>
															<span>NIT</span>
														</label>
														<label class="radio-inline radio-styled">
															<input type="radio" value="D" name="TipoCompra" onChange="ComprobarTipo(this)" required>
															<span>DPI</span>
														</label>
														<label class="radio-inline radio-styled" >
															<input type="radio" value="P" name="TipoCompra" onChange="ComprobarTipo(this)" required>
															<span>Pasaporte</span>
														</label>
													</div>
													<input class="form-control" type="hidden" name="TipoDoc" id="TipoDoc"  required/>
														
														</div>
														<div class="row">
															<div class="col-lg-3">
																<div class="form-group" id="DIVCIF" style="display: none">
																...
																<div class="form-group" id="DIVPasaporte" style="display: none">
																	<input class="form-control" type="text" name="Pasaporte" id="Pasaporte" onchange="ComprobarNIT(this.value)" value="Pasaporte" autofocus required/>
																	<label for="NIT">Número de Pasaporte</label>
																	<span id="SpanNIT"></span>
																</div>
																<div class="form-group" id="DIVDPI" style="display: none">
																	<input class="form-control" type="text" name="DPI" id="DPI" onchange="ComprobarNIT(this.value)" value="DPI" autofocus required/>
																	<label for="NIT">Número de DPI</label>
																	<span id="SpanNIT"></span>
																</div>
																<div id="EMNIT"></div>
															</div>
														...
														<div class="row">
															<div class="col-lg-10">
																...
																<h6><select name="EventoPertenece" id="EventoPertenece" class="form-control"  required>
																		...
																		<?php
																			...
																			$QueryCB = "SELECT * FROM Eventos.EVENTO WHERE F_CODIGO IS NULL AND EV_ESTADO=0 AND EV_INFORMATIVO=0 AND EV_CANCELADO=0 AND EV_ENCARGADO = ".$id_user." ORDER BY EV_FECHA_EV";
															...
															<select name="TipoFactura" id="TipoFactura" class="form-control" required >
																<option value="" disabled selected>Selecione Tipo de Factura</option>
														...
														<div class="row">
															<div class="col-lg-10">
																<div class="form-group">
																<table class="table" name="tableDescripcion" id="tableDescripcion">
																<thead>
																	<tr>
																		<th>Cantidad</th>
																		<th>Descripción</th>
																...
																<tbody>
																<tr class="fila-baseD">
						                                                <td><h6><input type="number" class="form-control" name="CantidadDes[]" id="CantidadDes[]" onChange="CalcularTotal()" style="width: 100px" min="0"></h6></td>
						                                                <td><h6><textarea class="form-control" name="DescripcionFac[]" id="DescripcionFac[]" rows="1" cols="60" placeholder="Escribe aquí la descripción de la Factura:"></textarea></h6></td>
						                                                <td><h6><input type="number" class="form-control" name="PrecioDes[]" id="PrecioDes[]"   style="width: 100px" min="0" step="any" style="text-align: right" value="0.00" readonly></h6></td>
						                                                <td><h6><input type="number" class="form-control" name="SubtotalDescr[]" id="SubtotalDescr[]"  onChange="CalcularTotal()" style="width: 100px" min="0" step="any" style="text-align: right" value="0.00"></h6></td>
						                                            	<td class="eliminar">
						                                                    <button type="button" class="btn btn-danger btn-lg" onclick="EliminarLineaD(this)">
																...
																<tr>
						                                                ...
						                                                <td style="text-align: left; vertical-align: text-top; font-size: 20px"><input class="form-control" type="text" id="TotalDescripcion" name="TotalDescripcion" value="0.00" readonly></td>
						                                                <td></td>
						                                            </tr>
																</tfoot>
																</table>
						                                        ...
						                                        <button type="button" class="btn btn-success btn-md" id="agregar" onclick="AgregarLineaD()">
															...
															<table class="table" name="tabla" id="tabla">
																...
																<tbody>
																	<tr class="fila-base">
						                                                ...
						                                                <td><h6><input type="text" class="form-control" name="ProductoNombre[]" id="ProductoNombre[]" style="width: 450px" onchange="BuscarProducto(this)"></h6></td>
						                                                <td><h6><select name="PuntoDeVenta[]" id="PuntoDeVenta[]" class="form-control" style="width: 150px"  required>
																		<option value="" disabled selected>Seleccione una opción</option>
																		<option value="1">Terrazas</option>
																		<option value="4">Café Los Abuelos</option>
																		<option value="3">Helados</option>
																		<option value="2">Souvenirs</option>
																		<option value="8">Pizzería</option>
																		<option value="6">Kiosco Azados</option>
																		</select>
																		</h6></td>
													...
													<div class="tab-pane" id="step2">
														...
														<div class="col-lg-3" class="col-lg-3" class="col-lg-3" id="DivCredito" name="DivCredito" style="display: none">
															<div class="card">
																<div class="card-body no-padding">
																	<div class="alert alert-callout alert-success no-margin">
																		<div>
															...
															<div class="card" >
																<div class="card-body no-padding">
																	<div class="alert alert-callout alert-success no-margin">
																		<div>
																			<div class="text-center">
																				<a style="text-decoration: none; cursor: pointer" onclick="TipoPago(5)" ><h1><span class="text-light">Mixto <i class="fa fa-ticket text-success"></i></span></h1></a>
																			</div>
																		</div>
																	</div>
																</div><!--end .card-body -->
													...
													<div class="tab-pane" id="step3">
														...
														<div class="col-lg-12" id="FormaPagoMixto" style="display: none">
															<div class="row">
																...
																<div class="col-lg-2 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<span class="input-group-addon">Q.</span>
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="TotalFacturaCobrarMixto" name="TotalFacturaCobrarMixto" readonly required>
																				<label for="TotalFacturaCobrarMixto">Total Factura</label>
																			</div>
																		</div>
																	</div>
																</div>
															...
															<div class="col-lg-4 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="EfectivoM" name="EfectivoM" required value="Efectivo Q." readonly>
																	...
																	<div class="form-group">
																		<div class="input-group">
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="MontoME" name="MontoME" onchange="SumaMixto()" value="0.00" step="any" required>
																				<label for="MontoME">Monto Q.</label>
																		</div>
																		</div>
																		</div>
																	</div>
															...
															<div class="col-lg-4 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="TrejetaM" name="TrejetaM" required value="Trjeta de Credito/Debito" readonly>
																	...
																	<div class="form-group">
																		<div class="input-group">
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="MontoMT" name="MontoMT" onchange="SumaMixto()" value="0.00" step="any" required>
																				<label for="MontoMT">Monto Q.</label>
																		</div>
																		</div>
																		</div>
																	</div>
																...
																<div class="col-lg-2 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="AutorizacionM" name="AutorizacionM" required>
																				<label for="AutorizacionM">No. Autorización</label>
																		</div>
																		</div>
																		</div>
																	</div>
														...
														<div class="col-lg-2"></div>
															<div class="col-lg-4 text-center">
																		...
																		<div class="input-group">
																			<div class="input-group-content">
																			<select name="CuentaBancariaM" id="CuentaBancariaM" class="form-control">
																			<option value="" selected disabled>Seleccione una Opción</option>
																			<?php
																...
																<div class="col-lg-2 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="MontoMB" name="MontoMB" onchange="SumaMixto()" value="0.00" step="any" required>
																				<label for="MontoMB">Monto Q.</label>
																		</div>
																		</div>
																		</div>
																	</div>
																<div class="col-lg-2 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="NumeroBoletaM" name="NumeroBoletaM" required>
																				<label for="NumeroBoletaM">No. Boleta</label>
																		</div>
																		</div>
																		</div>
																	</div>
																	<div class="row">	
																	<table class="table" name="tablaAnticipo" id="tablaAnticipo">
																<thead>
																	<tr>
																...
																<tbody>
																	<tr class="fila-base-anticipo">
						                                                <input type="hidden" class="form-control" name="CodigoA[]" id="CodigoA[]">						                                                
						                                                <td><h6><input type="text" class="form-control" name="NombreA[]" id="NombreA[]" style="width: 500px" onFocus="BuscarAnticipo(this)"></h6></td>
						                                                <td><h6><input type="number" class="form-control" name="MontoMA[]" id="MontoMA[]" onChange="CalcularTotal()"  style="width: 100px" min="0" step="any" style="text-align: right" value="0.00"></h6></td>
						                                                <td class="eliminar">
						                                                    <button type="button" class="btn btn-danger btn-lg" onclick="EliminarLineaA(this)">
						                                                        <span class="glyphicon glyphicon-trash"></span>
						                                                    </button>
						                                                </td>
						                                            </tr>
															...
															<div class="col-lg-12" align="left">
						                                        <button type="button" class="btn btn-success btn-md" id="agregar" onclick="AgregarLineaA()">
						                                            <span class="glyphicon glyphicon-plus"></span> Agregar
						                                        </button>
						                                    </div>
																	</div>
																...
																<div class="col-lg-2 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<span class="input-group-addon">Q.</span>
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="TotalEfectivoM" name="TotalEfectivoM" onchange="CalcularCambio2()" required readonly>
																				<label for="TotalEfectivoM">Total Acumulado</label>
																			</div>
																		</div>
																	</div>
																</div>
																...
																<div class="col-lg-2 text-center">
																	<div class="form-group">
																		<div class="input-group">
																			<span class="input-group-addon">Q.</span>
																			<div class="input-group-content">
																				<input type="text" class="form-control" id="CambioM" name="CambioM" readonly required>
																				<label for="CambioM">Cambio en Efectivo</label>
																			</div>
																		</div>
																	</div>
																</div>
												...
												<ul class="pager wizard">
													...
													<button type="button" id="Siguiente" name="Siguiente" class="btn ink-reaction btn-raised btn-lg btn-accent-dark" href="javascript:void(0);" disabled ><li class="next disabled" >Siguiente</li></button>
												</ul>
	...
	<!-- Modal Detalle Pasivo Contingente -->
		...
		<!-- Modal Detalle Pasivo Contingente -->
        <div id="ModalSugerenciasAnticipo" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <!-- Modal content-->
                <div class="modal-content">
                    <div class="modal-header" align="center">
                    ...
                    <div class="modal-body">
                    	<div id="suggestionsAn" class="text-center"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- /Modal Detalle Pasivo Contingente -->
		 ...
		 <!-- Modal Detalle Pasivo Contingente -->
		 <div id="ModalYogurt" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog">
                <!-- Modal content-->
                <div class="modal-content">
                    <div class="modal-header" align="center">
                        <h2 class="modal-title">Elegir Yogurt y Frutas</h2>
                    </div>
                    <div class="modal-body">
                    	<form class="form" role="form" id="FormularioSaboresYogurt">
	                    	<div id="SelectsSaboresYogurt">
                    ...
                    <div class="modal-footer">
                    	<button type="button" class="btn btn-primary" onclick="AgregarSaboresYogurt()">
                    	<span class="glyphicon glyphicon-check"></span> Agregar
                    	</button>
