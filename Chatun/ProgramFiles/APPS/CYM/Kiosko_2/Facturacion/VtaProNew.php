...
<head>
	...
	<script>
		function Imprimir()
		{
			...
			printJS('<?php $_SERVER['DOCUMENT_ROOT']?>/ProgramFiles/APPS/CYM/Kiosko_2/Facturacion/FactImpNew.php?Codigo='+Cod);
	...
	<div id="base">
		...
		<?php
			...
			if($TipoPago == 1)
			{
				...
				$ConsultaOrden = "SELECT F_ORDEN FROM Bodega.FACTURA_KS_2 WHERE F_FECHA_TRANS = CURRENT_DATE ORDER BY F_ORDEN DESC LIMIT 1";
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_KS_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_ORDEN, F_CON_DESCUENTO)
				...
				$sqlAbono = mysqli_query($db,"INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Kiosko Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 9, 1, '".$Uid."')") or die(mysqli_error());
				...
				else
				{
					...
					$Prod = $ProductoXplotado[0];
						...
						$query = mysqli_query($db,"INSERT INTO Bodega.FACTURA_KS_2_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO)
					...
					****************************************************************************************/
					...
					$SqlContable = mysqli_query($db,"INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_CONCEPTO, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_SERIE, TRA_FACTURA, TRA_TOTAL, RES_NUMERO, TRA_CORRELATIVO, PC_CODIGO)
											 VALUES('".$Uid."', CURRENT_DATE(), 'Vta. Kiosko Restaurant Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 2, 24, '".$SerieAutorizada."', '".$FacturaNumero."', ".$TotalFacturaFinal.", '".$NumeroResolucion."', ".$Correlativo.", ".$Periodo.")") or die(mysqli_error());
					...
					$SqlContableD = mysqli_query($db,"INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.01.016', ".number_format($TotalFacturaFinal, 2, ".", "").", 0.00)");
					...
					$SqlContableD2 = mysqli_query($db,"INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '4.01.03.005', 0.00, ".number_format($VentaSinIVA, 3, ".", "").")");
				}
					...
					for($i=1; $i<$Contador; $i++)
				{
						...
						if($rowe["R_CODIGO"]!=NULL){
							...
							while($rower = mysqli_fetch_array($resultr))
							{
								...
								while($roweR = mysqli_fetch_array($resultR))
								{
									...
									$existenciaAnR=$roweR["P_EXISTENCIA_KIOSCO"];
									$existenciaGAnR=$roweR["P_EXISTENCIA_GENERAL"];

									$cantidadgenR = $roweR["P_EXISTENCIA_GENERAL"] - $CanR;

									$nuevaexistenciaR = $roweR["P_EXISTENCIA_KIOSCO"] - $CanR;

								

								$QueryPU = mysqli_query($db, "UPDATE Productos.PRODUCTO SET
								P_EXISTENCIA_KIOSCO = '".$nuevaexistenciaR."',
								...
								VALUES('".$KCod."', '".$Uid."', '".$ProdR."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Kiosco Azados Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 6, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
					...
					$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
					VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de kiosco Azados "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAn."', '".$cantidadgen."', 6, '".$existenciaAn."', '".$nuevaexistencia."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   $QueryUpdateFactura = mysqli_query($db,"UPDATE Bodega.FACTURA_KS_2 SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'");

					   $QueryUpdateTransaccionBodega = mysqli_query($db,"UPDATE Bodega.TRANSACCION SET TRA_OBSERVACIONES = 'Vta. Kiosko Restaurante Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."' WHERE F_CODIGO = '".$Uid."'");

					   $QueryUpdateTransaccionContabilidad = mysqli_query($db,"UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Kiosko Restaurante Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
					}
					else
					{
				      	echo '<br><ul>';
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_KS_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO, F_ORDEN, F_CON_DESCUENTO, F_TIPO_TARJETA)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 2, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.", '".$Observaciones."', '".$NumeroAutorizacionTXT."', '".$NumeroResolucion."', '".$NumeroOrden."', '".$TotalDescuentoFactura."', '".$TipoTarjeta."')");

				$sqlAbono = mysqli_query($db,"INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Kiosko Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 9, 1, '".$Uid."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_KS_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO, F_ORDEN, F_CON_DESCUENTO)
				...
				$sqlAbono = mysqli_query($db,"INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Kiosko Restaurant Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 9, 1, '".$Uid."', '".$TotalDescuentoFactura."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   VALUES ('".$CCod."', '".$Uid."', 1, $NIT,  '".$id_user."', '".$TotalFacturaFinal."', 'AZADOS', 0.00)");
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_KS_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_ORDEN, F_CUENTA_BANCARIA, F_CON_DESCUENTO)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 4, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.",  '".$NumeroBoleta."', '".$Observaciones."', '".$NumeroResolucion."', '".$NumeroOrden."', '".$CuentaBancaria."', '".$TotalDescuentoFactura."')");
