...
//ADULTOS MAYORES NO ASOCIADOS INSERT 

$selectUpdAdultoM = mysqli_query($db, "SELECT SUM(a.INA_ADULTO_MAYOR) AS SUMA, a.INA_FECHA_INGRESO FROM Taquilla.INGRESO_NO_ASOCIADO a 
WHERE MONTH(a.INA_FECHA_INGRESO) = $mes AND YEAR(a.INA_FECHA_INGRESO) =  $year AND a.INA_ADULTO_MAYOR>0 
GROUP BY DAY(a.INA_FECHA_INGRESO) ");
while($rowTarjAdultoM = mysqli_fetch_array($selectUpdAdultoM))
{
		$tarjM = mysqli_query($db, "UPDATE Taquilla.DET_INGRESO SET INA_ADULTO_MAYOR = $rowTarjAdultoM[SUMA]  WHERE DIT_FECHA = '$rowTarjAdultoM[INA_FECHA_INGRESO]'");
}

$selectAdultoM = mysqli_query($db, "SELECT SUM(a.INA_ADULTO_MAYOR) AS SUMA, a.INA_FECHA_INGRESO FROM Taquilla.INGRESO_NO_ASOCIADO a 
WHERE MONTH(a.INA_FECHA_INGRESO) = $mes AND YEAR(a.INA_FECHA_INGRESO) =  $year  AND a.INA_ADULTO_MAYOR>0 AND NOT EXISTS (SELECT *FROM Taquilla.DET_INGRESO B WHERE B.DIT_FECHA = a.INA_FECHA_INGRESO )
GROUP BY DAY(a.INA_FECHA_INGRESO) ");
while($rowMayores = mysqli_fetch_array($selectAdultoM))
{
		$rowMayores = mysqli_query($db, "INSERT INTO Taquilla.DET_INGRESO (DIT_FECHA, INA_ADULTO_MAYOR, COLABORADOR)
		values ('$rowMayores[INA_FECHA_INGRESO]', $rowMayores[SUMA], $id_user)");
}
		...
		<table class="table table-hover" id="TblTicketsHotel">
		    <thead>
		      <tr>
				...
				<th>Adulto Mayor No Asociado</th>						     
		    ...
		    while($TicketHotelResult = mysqli_fetch_array($Tickets))
		    	{ 
		    		$totalNoAsociados = $TicketHotelResult["INA_NINO"]+$TicketHotelResult["INA_ADULTO"]+$TicketHotelResult["INA_ADULTO_MAYOR"]+$TicketHotelResult["IH_ADULTOS"]+$TicketHotelResult["IH_NINOS"]+$TicketHotelResult["ITF_TITULAR"]+$TicketHotelResult["ITF_ADULTOS"]+$TicketHotelResult["ITF_NINOS"];
		      ...
		      <tr>
				...
				<td><?php echo $TicketHotelResult["INA_ADULTO_MAYOR"]?></td>
			...
			$adultoNoAsociadoM += $TicketHotelResult["INA_ADULTO_MAYOR"];
	    	...
	    	<tr>
				...
				<td class="text-center"><?php echo $adultoNoAsociadoM ?></td>
