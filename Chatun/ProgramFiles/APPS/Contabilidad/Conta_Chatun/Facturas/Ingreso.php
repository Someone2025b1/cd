...
function CalcularTotal()
        {
            var CantidadP = document.getElementsByName('CantidadP[]');
            var PrecioU = document.getElementsByName('PrecioU[]');
            var PrecioTo = document.getElementsByName('PrecioTo[]');
            var PrecioP = document.getElementsByName('PrecioP[]');
            var RegimenC = $('#RegimenC').val(); 
            var TotalP = 0; 
            var TotalSI = 0; 
            var TotalPeces = 0;
            var CantidadFinal = 0;
            ...
            { 
                TotalP =  CantidadP[i].value * PrecioU[i].value; 
                TotalSI = (parseFloat(CantidadP[i].value) *  parseFloat(PrecioU[i].value)) / 1.12;  
                if (RegimenC==1) 
                {
                    CantidadFinal = TotalP;
                }
                else
                {
                	CantidadFinal = TotalSI;
                }
                PrecioTo[i].value = TotalP.toFixed(2);
                PrecioP[i].value = CantidadFinal.toFixed(2);
                TotalPeces = TotalPeces + CantidadFinal;
            }  
            $('#BoldTotal').val(TotalPeces.toFixed(2));
            Calcular();
        }
	...
	$(function() {
	    $('#FormAEnviar').submit( function() {
	    	if ($('#Establecimiento').val()=="x") 
	    	{
	    		alertify.error('Seleccione un tipo de Establecimiento');
	    		return false;
	    	}
	    	else
	    	{
	...
	function SelProveedor(x)
		...
		function Calcular()
		{
			...
			if ($('#ProduccionTilpia').is(':checked')) 
				{
					var ValorNeto = TotalCargos / 1.12; 
					if ($("#RegimenC").val()==1) 
					{
					 	var ValorNeto = TotalCargos; 
					}
					else
					{
						var ValorNeto = TotalCargos / 1.12; 
					}
					var TotalP = $("#BoldTotal").val(); 
					// if (ValorNeto.toFixed(2) == parseFloat(TotalP)) 
					// {
					//  $('#btnGuardar').prop("disabled", false); 
					// }
					// else
		...
		{ 
	        if($('#ProduccionTilpia').is(':checked'))
	        { 
	        	$("#DivProduccionTilapia").show();
	        }
	         else
	        { 
	        	$("#DivProduccionTilapia").hide();
	        }
...
<script>
	...
	$('#FormAEnviar').submit( function() {
		...
		if ($('#Establecimiento').val()== 2 ) 
		{
			...
			{ 
			...
			}elseif($('#Inventario').is(':checked') AND $('#ProduccionTilpia').is(':checked')){
				var Cuenta = document.getElementsByName('Cuenta[]');
				...
				for(i=1; i<Cuenta.length; i++)
				{
					...
					if(NombreCuenta.startsWith("1.01.06."))
					{
						return true;

					}else{
						alertify.error("El Inventario debe de Comenzar en 1.01.06.");
						return false;
				...
				for(i=1; i<Cuenta.length; i++)
				{
					...
					if(NombreCuenta.startsWith("6.08."))
					{
						return true;

					}else{
						alertify.error("El Gasto debe de Comenzar en 6.08.");
						return false;
		...
		<div id="content">
							...
							<div class="card-body">
									...
									<div class="col-lg-3">
										<div class="form-group"> 
											...
											<select class="form-control"  name="Establecimiento" id="Establecimiento" required>
												<option selected value="x" >Seleccione</option>
								...
								<div class="row">
									...
									<div class="col-lg-2">
										<div class="checkbox checkbox-styled">
											<label>
												<input onChange="ValidarActivo()" type="checkbox" name="ProduccionTilpia" id="ProduccionTilpia">
												<span>A Producción Tilapia</span>
											</label>
										</div>
									</div>
									<div class="col-lg-2">
			...
			<div class="row">
                ...
                <tbody>
                     <tr class="fila-base1">  
                        <td> 
                            <div class="form-group">
                                <label for="Capacidad">Producto</label>
                                <select name="ProductoTilapia[]" id="ProductoTilapia" class="form-control">
                                    <option value="" disabled selected>Seleccione un Producto</option>
                                    <?php
                                            //Select para obtener los tipos de falta de la base de datos y creación de las opciones del input select
                                    $query = "SELECT * FROM Pisicultura.Producto_Pisicultura where Estado = 1";
                                    $result = mysqli_query($db,$query);
                                    while($row = mysqli_fetch_array($result))
                                    {
                        ...
                        <td> 
                            <div class="form-group">
                                ...
                                <input placeholder=" " step="any" class="form-control" type="number"  step="any"  name="CantidadP[]" id="CantidadP" />
                            </div> 
                        </td>
                        <td> 
                            <div class="form-group">
                                <label for="Capacidad">Precio Unitario</label>
                                <input onchange="CalcularTotal()" placeholder=" " class="form-control" step="any" type="number" name="PrecioU[]" id="PrecioU" />
                            </div> 
                        </td>
                        <td> 
                            <div class="form-group">
                                <label for="Capacidad">Total</label>
                                <input placeholder=" " class="form-control" step="any" type="number"  name="PrecioTo[]" id="PrecioTo" />
                            </div> 
                        </td>
                        <td> 
                            <div class="form-group">
                                <label for="Capacidad">Total sin IVA</label>
                                <input placeholder=" " class="form-control" step="any" type="number"  name="PrecioP[]" id="PrecioP" />
                            </div> 
                        </td>
                        <td class="eliminar1">
                            <button type="button" class="btn btn-danger btn-xs">
            ...
            <tfoot>
            	<tr>
            		...
            		<td> 
                    <label for="Capacidad">Total</label>
                    <input placeholder=" " class="form-control" step="any" type="number" min="1"  id="BoldTotal" />
                    </td>
            	</tr>
            </tfoot>
            </table>
