...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	if ($_GET["accion"] == 1) {
		...
		$insertar = "INSERT INTO Estado_Patrimonial.detalle_valor_acciones (colaborador, clase_titulo, institucion, monto, valor_comercial) VALUES ('$auxi', '$clase_titulo', '$institucion', '$monto', '$valor_comercial')";
		mysqli_query($db, $insertar) or die (mysqli_error());
	}
	if ($_GET["accion"] == 4) {
		$id = $_GET["id"];
		$eliminar = "DELETE FROM Estado_Patrimonial.detalle_valor_acciones WHERE id = '$id'";
	...
	if ($_GET["accion"] == 3) {
		...
		$monto = str_replace(",","",$monto_1);
		$valor_comercial = str_replace(",","",$valor_comercial_1);
		$actualizar = "UPDATE Estado_Patrimonial.detalle_valor_acciones SET colaborador = '$auxi', clase_titulo = '$clase_titulo', institucion = '$institucion', monto = '$monto', valor_comercial = '$valor_comercial' WHERE id = '$id'";
...
<?php
		...
		$id = $_GET["id"];
		$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_valor_acciones WHERE colaborador = '$auxi' and id = '$id'";
