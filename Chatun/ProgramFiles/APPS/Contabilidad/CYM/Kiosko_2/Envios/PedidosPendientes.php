						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								<thead>
									...
									<th><strong>Quien Envía</strong></th>
								...
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Productos.EX_PUNTOS_ENVIOS AS A WHERE A.EPE_ESTADO = 0 AND A.EPE_PUNTO = 'Kiosco Azados' ORDER BY A.EPE_FECHA ";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
										$Codigo = $row["EPE_CODIGO"];
										$EN = $row['EN_ENVIA'];
                                        										echo '<tr>';
										    	echo '<td style="font-size: 10px">'.$Contador.'</td>'; 
                                                echo '<td style="font-size: 10px">'.$row["EPE_CODIGO"].'</td>';
                                                echo '<td style="font-size: 10px">Terrazas</td>';
										    	echo '<td style="font-size: 10px">'.$row["EPE_DESCRIPCION"].'</td>';
                                                echo '<td style="font-size: 10px">'.$row["EPE_FECHA"].'</td>';
										    	echo '<td><a href="RecibirPedido.php?Codigo='.$Codigo.'"><button type="button" class="btn btn-warning">
														     Recibir Pedido
														  </button></a>
														</td>';
