...
$Consulta = "SELECT * FROM Bodega.RECETA_SUBRECETA WHERE RS_CODIGO = '".$Codigo."'";
	...
	while($row = mysqli_fetch_array($Resultado))
	{
		...
		$Categoria = $row["CME_CODIGO"];
		$Tipo      = $row["RS_TIPO"];
		$Precio    = $row["RS_PRECIO"];
		$MostrarMenu = $row["RS_MOSTRAR_MENU"];
		if($Tipo == 2)
		{
			$Consulta1 = "SELECT * FROM Bodega.PRODUCTO WHERE P_CODIGO_SUBRECETA = '".$Codigo."'";
			$Resultado1 = mysqli_query($db, $Consulta1) or die (mysqli_error());
						...
						<div class="card">
							...
							<div class="card-body">
										...
										<div class="form-group">
											<select name="Categoria" id="Categoria" class="form-control" required>
												...
												<?php
                                                    ...
                                                    $query = "SELECT * FROM Bodega.CATEGORIA_MENU_EVENTO ORDER BY CME_NOMBRE";
                                                    $result = mysqli_query($db, $query);
                                                    while($row = mysqli_fetch_array($result))
                                                    {
                                                    	if($Categoria == $row["CME_CODIGO"])
                                                    	{
                                                    		$Selected = 'selected';
                                                    	}
                                                    	else
                                                        ...
                                                        echo '<option value="'.$row["CME_CODIGO"].'" '.$Selected.'>'.$row["CME_NOMBRE"].'</option>';
                                                    }
								...
								<div class="row">
									<div class="col-lg-2 col-lg-8">
										<div class="form-group">
											<select name="MostrarMenu" id="MostrarMenu" class="form-control" required="required">
												<option value="1" <?php if($MostrarMenu == 1){echo 'selected';} ?>>Sí</option>
												<option value="0" <?php if($MostrarMenu == 0){echo 'selected';} ?>>No</option>
											</select>
											<label for="MostrarMenu">Mostrar En Menú</label>
										</div>
									</div>
								</div>
								<?php
						...
						<div class="card">
							...
							<div class="card-body">
								...
								<div class="row">
									<div class="col-lg-4 col-lg-8">
										<div class="form-group">
											<select name="CategoriaMenuEventos" id="CategoriaMenuEventos" class="form-control" required>
											...
											<label for="CategoriaMenuEventos">Categoría</label>
										</div>
									</div>
								</div>
								<div class="row">
									...
									<table class="table table-hover table-condensed" id="tabla">
										...
										<tbody>
											...
											<select name="Producto[]" id="Producto[]" class="form-control">
												...
												<?php
	                                                    //Select para obtener los tipos de falta de la base de datos y creación de las opciones del input select
												$query = "SELECT PRODUCTO.*, UNIDAD_MEDIDA.UM_NOMBRE FROM Bodega.PRODUCTO, Bodega.UNIDAD_MEDIDA WHERE PRODUCTO.UM_CODIGO = UNIDAD_MEDIDA.UM_CODIGO AND CP_CODIGO = 'TR' ORDER BY P_NOMBRE";
												$result = mysqli_query($db, $query);
												while($row = mysqli_fetch_array($result))
												{
											...
											<?php
												...
												$Resultado2 = mysqli_query($db, $Consulta2) or die (mysqli_error());
