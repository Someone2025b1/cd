					...
					<div class="col-lg-12"> 
						<div class="card">
							<div class="card-head style-primary">
								<h4 class="text-center"><strong>HISTORIAL DE VALES PENDIENTES</strong></h4>
							</div>
							<div class="card-body">
								<h2>INSTITUCIÓN: <?php echo $NombreHotel["H_NOMBRE"] ?></h2>
								 <div class="row">
									...
									<table class="table table-hover" id="TblTicketsHotel">
										...
										/* 
									    $Contador = 1;
									   $Tickets = mysqli_query($db, "SELECT SUM(a.IH_ADULTOS) AS ADULTOS, SUM(a.IH_ADULTO_MAYOR) AS ADULTOSM, SUM(a.IH_NINOS) AS NINOS, DATE(a.IH_FECHA) AS Fecha, SUM(a.IH_TOTAL_ADULTO) AS TotalAdulto, SUM(a.IH_TOTAL_ADULTO_MAYOR) AS TotalAdultoM, SUM(a.IH_TOTAL_NINO) AS TotalNino
									    ...
									    while($TicketHotelResult = mysqli_fetch_array($Tickets))
									    	...
									    	$total =   $TicketHotelResult["TotalAdulto"]+$TicketHotelResult["TotalAdultoM"]+$TicketHotelResult["TotalNino"];
									      ...
									      <tr>
											...
											<td align="center"><?php echo $TicketHotelResult["ADULTOSM"] ?></td>
									  ...
									  $Contador++; 
											...
											$totalAdultosM += $TicketHotelResult["ADULTOSM"];
									 ...
									 $Tickets2 = mysqli_query($db, "SELECT DATE(b.CH_Fecha) as Fecha, SUM(a.DC_Adultos) AS ADULTOS, SUM(a.DC_Ninos) AS NINO 
								    	...
								    	*/
										...
										/* ESTO ES PARA QUE SALGA TODO EL RESUMEN
								    	...
								    	<!--<tr>
											...
											<td class="text-center"><?php echo $totalAdultosM ?> </td> 
											...
											<td class="text-center"><?php echo $totalAdultosM1 ?> </td> 
								...
								<div class="col-lg-12">
									...
									##### ESTO MODIFIQUE 07/12/2022
									    $Contador = 1;
									   $Tickets = mysqli_query($db, "SELECT a.*, b.H_NOMBRE
											FROM Taquilla.INGRESO_HOTEL a
											INNER JOIN Taquilla.HOTEL b ON a.H_CODIGO = b.H_CODIGO
											WHERE a.H_CODIGO = $Id AND a.IH_VALE > 0 AND NOT EXISTS (SELECT *FROM Taquilla.DETALLE_VALE_FACTURA c 
											WHERE a.IH_VALE = c.DVF_Vale AND a.H_CODIGO = c.DVF_Hotel AND c.DVF_Estado = 2)
											");
									    
									    while($TicketHotelResult = mysqli_fetch_array($Tickets))
									    	{
												$TotalTiket=$TicketHotelResult["IH_TOTAL_ADULTO"]+$TicketHotelResult["IH_TOTAL_ADULTO_MAYOR"]+$TicketHotelResult["IH_TOTAL_NINO"];
									    	$totalpend += $TotalTiket;
											$totalAdultospend += $TicketHotelResult['IH_ADULTOS'];
											$totalAdultospendM += $TicketHotelResult['IH_ADULTO_MAYOR'];
								    		$totalNinospend += $TicketHotelResult['IH_NINOS'];

											?>
											<tr>
									    		<td align="center"><?php echo abs($TicketHotelResult['IH_VALE']) ?></td>
									    		<td align="center"><?php echo $TicketHotelResult['IH_FECHA'] ?></td>
									    		<td align="center"><?php echo abs($TicketHotelResult['IH_ADULTOS']) ?></td>
												<td align="center"><?php echo abs($TicketHotelResult['IH_ADULTO_MAYOR']) ?></td>
									    		<td align="center"><?php echo abs($TicketHotelResult['IH_NINOS']) ?></td>
									    		<td align="center">Q. <?php echo number_format($TotalTiket,2) ?></td>
												</tr>
												<?php
											}
									      ?>
											...
											$TotalAduM = $totalAdultosM - $totalAdultosM1;
									    	$TotalNino = $totalNinos - $totalNinos1; 
									    	$totalEntrada =   abs($TotalAdu+$TotalNino);
										?>

											<tr>
									    		<td colspan="2" align="center">TOTALES</td>
									    		<td align="center"><?php echo abs($totalAdultospend) ?></td>
												<td align="center"><?php echo abs($totalAdultospendM) ?></td>
									    		<td align="center"><?php echo abs($totalNinospend) ?></td>
									    		<td align="center">Q. <?php echo number_format($totalpend,2) ?></td>
												</tr>

												</tbody>
												</table>
<!-- esto sE QUITI EL 16/11/2024
									...
									<table class="table table-hover" id="TblTicketsHotelTotal">
									    ...
									    <tbody>
									    	<tr>
												...
												<td align="center"><?php echo abs($TotalAduM) ?></td>
