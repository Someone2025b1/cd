...
<body class="menubar-hoverable header-fixed menubar-pin">
	...
	if($id_user==53711 | $id_user==141797 | $id_user==435849 | $id_user==10345 | $Usuar==1801788) {
		$Filtro="";
	}else{
...
<div class="col-lg-12">
									...
									<h3 class="text-center">RESUMEN ANTICIPOS PENDIENTES</h3><br>
									<table class="table table-hover" id="TblTicketsHotelTotal">
									    <thead>
									      <tr> 
									        <th class="text-center">COLABORADOR</th>
											<th class="text-center">MONTO</th>
									        <th class="text-center">CANTIDAD DE ANTICIPOS</th>
									      </tr>
									    </thead>
									    <tbody>
									    	<?php 
									    	$Query2 = mysqli_query($db, "SELECT COUNT(*) AS CONT, sum(AE_MONTO) as MONTO, A.*, C.TRA_CONCEPTO, C.TRA_FECHA_HOY
											FROM Contabilidad.ANTICIPO_EVENTOS AS A,
											Contabilidad.TRANSACCION AS C
											WHERE  C.TRA_CODIGO = A.AE_CODIGO
											AND A.AE_ESTADO= 0
											$Filtro
											GROUP BY A.AE_USER");
									    	while($Fila2 = mysqli_fetch_array($Query2))
											{
												$Usuario   = $Fila2["AE_USER"];
...
<div class="col-lg-12">
	...
	$Query = mysqli_query($db, "SELECT A.*, C.TRA_CONCEPTO, C.TRA_FECHA_HOY
    FROM Contabilidad.ANTICIPO_EVENTOS AS A,
    Contabilidad.TRANSACCION AS C
    WHERE C.TRA_CODIGO = A.AE_CODIGO
    AND A.AE_ESTADO=0
				...
				<div id="myTable_wrapper" class="dataTables_wrapper no-footer"> <table id="TblTicketsHotel1" class="table table-bordered table-striped dataTable no-footer" role="grid" aria-describedby="myTable_info"> 
					<thead>
						<tr>
							...
							<th><h5><strong>FECHA ANTICIPO</strong></h5></th>
					...
					<tbody>
						<?php
							...
							while($Fila = mysqli_fetch_array($Query))
							{
								...
								$MontoFac        =  $Fila["AE_MONTO"];

                                $CodigoFac = $Fila["AE_CODIGO"]; 
                                $Usuario   = $Fila["AE_USER"];
								$Nombre  = $Fila["AE_NOMBRE_CLIENTE"];           
