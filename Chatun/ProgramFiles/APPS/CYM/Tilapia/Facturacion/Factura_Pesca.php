...
<head>
	...
	<script>
		...
		function CalcularTotal()
		{
			...
			{
				SubTotalCalculado = parseFloat(CantidadFE[i].value) * parseFloat(Precio[i].value) - parseFloat(Descuento[i].value);
...
<body class="menubar-hoverable header-fixed menubar-pin ">
	...
	$ConsultaApertura = "SELECT ACC_CODIGO FROM Bodega.APERTURA_CIERRE_CAJA WHERE ACC_FECHA = CURRENT_DATE AND ACC_TIPO = 9";
	...
	$ConsultaCierre = "SELECT ACC_CODIGO FROM Bodega.APERTURA_CIERRE_CAJA WHERE ACC_FECHA = CURRENT_DATE AND ACC_TIPO = 9 AND ACC_ESTADO = 2";
	$ResultCierre = mysqli_query($db, $ConsultaCierre);
	$RegistrosCierre = mysqli_num_rows($ResultCierre);
	if($RegistrosCierre > 1)
	{
		?>
		<script>
			CierreCaja();
								...
								<div class="card">
									<div class="card-body ">
										<div id="rootwizard2" class="form-wizard form-wizard-horizontal">
											<form class="form form-validation" role="form" novalidate="novalidate" action="Factura_Pesca_Pro.php" method="POST" role="form" id="FormularioPrincipal">
													...
													<div class="tab-pane active" id="step1">
														...
														<div class="row">
 															...
 															<table class="table" name="tabla" id="tabla">
																...
																<tbody>
																	<tr >
						                                                <td><h6><input type="number" class="form-control" name="CantidadFE[]" id="CantidadFE[]" onChange="CalcularTotal()" style="width: 100px" min="0"></h6></td> 
						                                                <td><h6><input type="number" step="any" class="form-control" name="UnidadesVendidas[]" id="UnidadesVendidas[]" value="0" min="1"></h6></td> 
																		<td><h6><input type="number" step="any" class="form-control" name="Estanque[]" id="Estanque[]" value="0" min="1"></h6></td>
						                                                <td>
						                                                <select name="Producto[]" id="Producto[]" class="form-control" onchange="SaberPrecio(this)">
						                                                		<option selected disabled>Seleccione</option>
						                                                		<?php 
						                                                		$ListPescado = mysqli_query($db, "SELECT a.P_NOMBRE, a.P_CODIGO FROM Bodega.PRODUCTO a WHERE a.TP_ID = 3");
						                                                		while ($RowPescado = mysqli_fetch_array($ListPescado)) 
						                                                		{
						                                                		?>
						                                                		<option value="<?php echo $RowPescado["P_CODIGO"]?>"><?php echo $RowPescado["P_NOMBRE"] ?></option>
						                                                		<?php 
						                                                		}
						                                                		?> 
						                                                	</select>
