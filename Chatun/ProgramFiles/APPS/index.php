...
$pdf->ezText("Existencia de Productos",16,array('justification'=>'center'));
...
$ResultCuentas = mysql_query($QueryCuentas, $db);
while($row = mysql_fetch_array($ResultCuentas))
{
    $ExistenciaMost = 0;
    ...
    while($fila = mysql_fetch_array($rest))
    {
        ...
        $ResCosto = mysql_query($QueryCosto,$db);
        while($filacosto = mysql_fetch_array($ResCosto))
        {
