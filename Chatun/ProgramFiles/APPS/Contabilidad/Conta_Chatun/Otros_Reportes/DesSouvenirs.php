                                   ...
                                   data-filter-control="true">
									...
									<tbody>
										<?php
											...
											JOIN Bodega.FACTURA_SV_DETALLE AS C ON  C.F_CODIGO = A.F_CODIGO
											...
											while($FilaDetalle = mysqli_fetch_array($QueryDescuentos))
											{
												...
												if($codf != $FilaDetalle["F_CODIGO"]){
													...
													FROM Bodega.FACTURA_SV_DETALLE AS H
													JOIN Bodega.PRODUCTO AS I ON H.P_CODIGO = I.P_CODIGO 
													...
													$preciocon = ($FilaDetalledes["FD_SUBTOTAL"]-$FilaDetalledes["FD_DESCUENTO"]) / $FilaDetalledes["FD_CANTIDAD"];
													$subtotalsin =  $FilaDetalledes["FD_SUBTOTAL"];
													$descuentototal += $FilaDetalledes["FD_DESCUENTO"];
															
													#totales por cada venta
													$totalconpv += ($FilaDetalledes["FD_SUBTOTAL"]-$FilaDetalledes["FD_DESCUENTO"]);
												...
												if($codf != $FilaDetalle["F_CODIGO"]){
													...
													while($FilaDetalledes = mysqli_fetch_array($QueryDetalleDescuentos))
													{
														if($encabezado==0){
															...
															$preciocon = ($FilaDetalledes["FD_SUBTOTAL"]-$FilaDetalledes["FD_DESCUENTO"]) / $FilaDetalledes["FD_CANTIDAD"];
                                                            $subtotalsin =  $FilaDetalledes["FD_SUBTOTAL"];
                                                            $subtotalcon = ($FilaDetalledes["FD_SUBTOTAL"]-$FilaDetalledes["FD_DESCUENTO"]);
															...
															<tr>
																...
																<td align="center">Q.<?php echo $subtotalcon ?></td> 
																</tr>
