...
<head>
	...
	function Imprimir()
	{
		...
		printJS('<?php $_SERVER['DOCUMENT_ROOT']?>/ProgramFiles/APPS/CYM/Helados/Facturacion/FactImpNew.php?Codigo='+Cod);
...
<body class="menubar-hoverable header-fixed menubar-pin " >
	...
	<div id="base">
		...
		<?php
			...
			$PaisEmisor = 'GT';
			$CorreoReceptor = 'parquechatun81@gmail.com';
			...
			if($TipoPago == 1)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HS (F_CODIGO, F_TIPO, CLI_NIT, F_CON_DESCUENTO, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_SERIE, F_ORDEN)
												VALUES ('".$Uid."', 1, '".$NIT."', ".$TotalFacturaConDescuento.", ".$TotalFacturaFinal.", CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user.", ".$Moneda.", ".$Efectivo.", ".$Cambio.", '".$Observaciones."', '".$NumeroResolucion."', '".$SerieAutorizada."', '".$NumeroOrden."')");
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Helados Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 11, 3, '".$Uid."')") or die(mysqli_error($sqlAbono));
				...
				else
				{
					for($i=0; $i<$Contador; $i++)
					{
						...
						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HS_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, RS_NOMBRE, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO)
											VALUES('".$Uid."', '".$UidD."', ".$Can.", '".$Pro."', '".$Nom."', ".$Pre.", ".$SubFac.", ".$Des.")");
					...
					****************************************************************************************/
					...
					$SqlContable = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_CONCEPTO, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_SERIE, TRA_FACTURA, TRA_TOTAL, RES_NUMERO, TRA_CORRELATIVO, PC_CODIGO)
											 VALUES('".$Uid."', CURRENT_DATE(), 'Vta. Helados Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 2, 6, '".$SerieAutorizada."', '".$FacturaNumero."', ".$TotalFacturaFinal.", '".$NumeroResolucion."', ".$Correlativo.", ".$Periodo.")") or die(mysqli_error($SqlContable));
					...
					for($i=1; $i<$Contador; $i++)
				{
					...
					WHERE P_CODIGO = ".$Prod);
						...
						if($rowe["R_CODIGO"]!=NULL){
							...
							while($rower = mysqli_fetch_array($resultr))
							{
								...
								VALUES('".$KCod."', '".$Uid."', '".$ProdR."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Helados Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 3, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
					...
					$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
					VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Helados Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAn."', '".$cantidadgen."', 3, '".$existenciaAn."', '".$nuevaexistencia."')");
							...
							if($RegistrosSelSabores > 0)
							{
								while($FilaSelSabores = mysqli_fetch_array($ResultSelSabores))
								{
									...
									if($CodProdSabor==1205 | $CodProdSabor==1204 | $CodProdSabor==1203){
										$CanR=1;
								...
								$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
								VALUES('".$KCod."', '".$Uid."', '".$CodProdSabor."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Helados Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 3, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					...
					else
					{
							...
							SELECT FACTURA_HS.F_CODIGO, '3', FACTURA_HS.F_ORDEN, FACTURA_HS.F_SERIE, FACTURA_HS.F_NUMERO, FACTURA_HS.F_TIPO, FACTURA_HS.CLI_NIT, FACTURA_HS.D_CODIGO, FACTURA_HS.F_CON_DESCUENTO, FACTURA_HS.F_TOTAL, FACTURA_HS.F_EFECTIVO, FACTURA_HS.F_CAMBIO, FACTURA_HS.F_FECHA_TRANS, FACTURA_HS.F_HORA, FACTURA_HS.F_USUARIO, FACTURA_HS.F_MONEDA, FACTURA_HS.F_ESTADO, FACTURA_HS.F_RAZON_ANULACION, FACTURA_HS.F_NOMBRE_CREDITO, FACTURA_HS.F_NO_BOLETA, FACTURA_HS.F_OBSERVACIONES, FACTURA_HS.F_NO_AUTORIZACION, FACTURA_HS.RES_NUMERO, FACTURA_HS.F_CUENTA_BANCARIA, FACTURA_HS.F_REALIZADA, FACTURA_HS.F_CAE, FACTURA_HS.F_DTE, FACTURA_HS.F_FECHA_CERTIFICACION, FACTURA_HS.F_TIPO_TARJETA FROM Bodega.FACTURA_HS
							WHERE FACTURA_HS.F_CODIGO = '".$Uid."'")or die(mysqli_error());

							$QueryBitacora1 = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA_DETALLE
							SELECT * FROM Bodega.FACTURA_HS_DETALLE
							WHERE FACTURA_HS_DETALLE.F_CODIGO = '".$Uid."'")or die(mysqli_error());
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO, F_CON_DESCUENTO, F_TIPO_TARJETA, F_ORDEN)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 2, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.", '".$Observaciones."', '".$NumeroAutorizacionTXT."', '".$NumeroResolucion."', '".$TotalFacturaConDescuento."', '".$TipoTarjeta."', '".$NumeroOrden."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO, F_CON_DESCUENTO, F_ORDEN)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 3, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.",  '".$NombreCredito."', '".$Observaciones."', '".$NumeroResolucion."', '".$TotalFacturaConDescuento."', '".$NumeroOrden."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Helados Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
						...
						VALUES ('".$CCod."', '".$Uid."', 1, $NIT,  '".$id_user."', '".$TotalFacturaFinal."', 'HELADOS', 0.00)");
					}
					else
					{
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA, F_CON_DESCUENTO, F_ORDEN)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 4, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.",  '".$NumeroBoleta."', '".$Observaciones."', '".$NumeroResolucion."', '".$CuentaBancaria."', '".$TotalFacturaConDescuento."', '".$NumeroOrden."')");
