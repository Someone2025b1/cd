...
while($NuevaFecha <= $FechaFin)
{
    ...
    $Query = "SELECT * FROM Bodega.TRANSACCION
                WHERE (TT_CODIGO = 1 OR TT_CODIGO = 9 OR TT_CODIGO = 7)
    ...
    while($row = mysqli_fetch_array($Result))
    {
        ...
        if($row["TT_CODIGO"] == 1 OR $row["TT_CODIGO"] == 7)
        {
            ...
            while($Fila = mysqli_fetch_array($ResultDetalle))
            {
                ...
                $CostoUnitarioExistencia = $SumaCostoTotal / $SumaTotalEntrada;

                $CostoTotalExistencia = $SumaTotalEntrada * $CostoUnitarioExistencia;

                $Data[] = array('col1'=>date('d-m-Y', strtotime($NuevaFecha)), 'col2'=>number_format($UnidadesEntrada, 2, '.', ','), 'col3'=>number_format($PrecioEntrada, 2, '.', ','), 'col4'=>number_format($TotalEntrada, 2, '.', ','), 'col5'=>'', 'col6'=>'', 'col7'=>'', 'col8'=>number_format($SumaTotalEntrada, 2, '.', ','), 'col9'=>number_format($CostoUnitarioExistencia, 2, '.', ','), 'col10'=>number_format($CostoTotalExistencia, 2, '.', ','));
        ...
        else
        {
            ...
            while($Fila = mysqli_fetch_array($ResultDetalle))
            {
                ...
                $Data[] = array('col1'=>date('d-m-Y', strtotime($NuevaFecha)), 'col2'=>'', 'col3'=>'', 'col4'=>'', 'col5'=>number_format($UnidadesSalida, 2, '.', ','), 'col6'=>number_format($PrecioSalida, 2, '.', ','), 'col7'=>number_format($TotalSalida, 2, '.', ','), 'col8'=>number_format($SumaTotalEntrada, 2, '.', ','), 'col9'=>number_format($CostoUnitarioExistencia, 2, '.', ','), 'col10'=>number_format($CostoTotalExistencia, 2, '.', ','));
