						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								<thead>
									...
									<th><strong>Quien Pidio</strong></th>
									<th><strong>Observaciones</strong></th>
                                    <th><strong>FECHA NECESIDAD</strong></th>
                                    <th><strong>FECHA SOLICITADO</strong></th>
								...
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM CompraVenta.PEDIDO_INVENTARIO AS A WHERE A.PI_ESTADO = 1 OR A.PI_ESTADO = 2 ORDER BY A.PI_FECHA_NECESIDAD ";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
										$Codigo = $row["PI_CODIGO"];
                                         ...
                                         $Estado = $row['PI_ESTADO'];
											...
											while($rowUsu = mysqli_fetch_array($ResultUsu))
											{
                                                ...
                                                echo '<td style="font-size: 10px">'.$row["PI_CODIGO"].'</td>';
                                                ...
                                                echo '<td style="font-size: 10px">'.$NombreSolito.'</td>';
										    	echo '<td style="font-size: 10px">'.$row["PI_DESCRIPCION"].'</td>';
                                                echo '<td style="font-size: 10px">'.$row["PI_FECHA_NECESIDAD"].'</td>';
                                                echo '<td style="font-size: 10px">'.$row["PI_FECHA_PEDIDO"].'a las: '.$row["PI_HORA_PEDIDO"].' horas</td>';
                                                echo '<td style="font-size: 10px">'.$row["PI_TOTAL_PRODUCTOS"].'</td>';
