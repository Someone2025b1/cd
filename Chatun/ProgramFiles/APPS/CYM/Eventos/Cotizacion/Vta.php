...
<head>
	...
	<script>
		...
		function AgregarLineaPersonalizado()
		{
			$("#TBLPersonalizado tbody tr:eq(0)").clone().removeClass('fila-base').appendTo("#TBLPersonalizado tbody");
		}
		...
		function EliminarLineaPersonalizado(x)
		{
			var parent = $(x).parents().get(1);
                $(parent).remove();
		...
		function ComprobarNIT(x)
		{
			$.ajax({
				...
				success: function(opciones)
				{
					...
					{
						var Separado = opciones.split('&/.');

						$('#Nombre').val(Separado[1]);
						$('#Direccion').val(Separado[2]);
		...
		{
			var Precio   = document.getElementsByName('PrecioPersonalizado[]');
			var Cantidad = document.getElementsByName('CantidadPersonalizado[]');
			var SubTotal = document.getElementsByName('SubTotalPersonalizado[]');
			var Total = 0;
			var SubTotalCalculado = 0;
			for(i = 1; i < Precio.length; i++)
			{
			...
			$('#BoldTotalPersonalizado').val(Total.toFixed(2));
		}
		...
		{
			if($('input:radio[name=TipoImpresion]:checked').val() == 2)
			{
				var Precio   = document.getElementsByName('Precio[]');
				var Cantidad = document.getElementsByName('Cantidad[]');
				var SubTotal = document.getElementsByName('SubTotal[]');
				...
				var PrecioPersonalizado   = document.getElementsByName('PrecioPersonalizado[]');
				var CantidadPersonalizado = document.getElementsByName('CantidadPersonalizado[]');
				var SubTotalPersonalizado = document.getElementsByName('SubTotalPersonalizado[]');
				var TotalPersonalizado = 0;
				var SubTotalCalculadoPersonalizado = 0;
				for(i = 1; i < PrecioPersonalizado.length; i++)
				{
					SubTotalCalculadoPersonalizado = parseFloat(CantidadPersonalizado[i].value) * parseFloat(PrecioPersonalizado[i].value);
					SubTotalPersonalizado[i].value = SubTotalCalculadoPersonalizado.toFixed(2);
					TotalPersonalizado = TotalPersonalizado + SubTotalCalculadoPersonalizado;
				}

				if(parseFloat(TotalPersonalizado) == parseFloat(Total))
				{
				...
				{
					alertify.error('El total de la Factura Detalle, no coincide con el total de la Factura Perzonalizada o viceversa');
				}
			}
			else
			{
		...
		function TipoImpresionFnc(x)
		{
			if(x == 2)
			{
				$('#DIVDetallePersonalizado').show();
			}
			else
			{
				$('#DIVDetallePersonalizado').hide();	
			}
		}
	</script>
...
<body class="menubar-hoverable header-fixed menubar-pin " onload="CalcularTotal()">
	...
	$ConsultaSerie = "SELECT RES_SERIE, RES_NUMERO FROM Bodega.RESOLUCION WHERE RES_ESTADO = 1 AND RES_TIPO = 'TR'";
	...
	$ConsultaNumeroFactura = "SELECT F_NUMERO FROM Bodega.FACTURA_EV WHERE F_SERIE = '".$Serie."' ORDER BY F_NUMERO DESC LIMIT 1";
	...
	{
		$ConsultaNumeroFacturaInicial = "SELECT RES_DEL FROM Bodega.RESOLUCION WHERE RES_ESTADO = 1 AND RES_TIPO = 'EV'";
	...
	$ConsultaOrden = "SELECT F_ORDEN FROM Bodega.FACTURA_EV WHERE F_FECHA_TRANS = CURRENT_DATE ORDER BY F_ORDEN DESC LIMIT 1";
	...
	$ConsultaApertura = "SELECT ACC_CODIGO FROM Bodega.APERTURA_CIERRE_CAJA WHERE ACC_FECHA = CURRENT_DATE AND ACC_TIPO = 6";
	...
	$ConsultaCierre = "SELECT ACC_CODIGO FROM Bodega.APERTURA_CIERRE_CAJA WHERE ACC_FECHA = CURRENT_DATE AND ACC_TIPO = 6 AND ACC_ESTADO = 2";
												...
												<div class="tab-content clearfix">
													<div class="tab-pane active" id="step1">
														<div class="row">
															...
															<div class="col-lg-1">
																<div class="form-group floating-label">
																	<input class="form-control" type="hidden" name="NumeroResolucion" id="NumeroResolucion" value="<?php echo $NumeroResolucion; ?>" required readonly />
																	<input class="form-control" type="text" name="FacturaSerie" id="FacturaSerie" value="<?php echo $Serie; ?>" required readonly />
																	<label for="FacturaSerie">Serie</label>
																</div>
															</div>
															<div class="col-lg-2">
																<div class="form-group floating-label">
																	<input class="form-control" type="text" name="FacturaNumero" id="FacturaNumero" value="<?php echo $NumeroFactura; ?>" required readonly/>
																	<label for="FacturaNumero">Número</label>
																</div>
															</div>
														</div>
														<div class="row">
														...
														<div class="row">
															...
															<div class="col-lg-2 text-center">
																<div class="form-group floating-label">
																	<input class="form-control" type="int" name="NumeroOrden" id="NumeroOrden" value="<?php echo $NumeroOrden; ?>" required readonly/>
																	<label for="NumeroOrden"># Orden</label>
																</div>
															</div>
														</div>
														<?php
															$QueryDatosCliente = mysqli_query($db, "SELECT * FROM Bodega.COTIZACION WHERE C_CODIGO = ".$_GET[Codigo]);
															$FilaDatosCliente = mysqli_fetch_array($QueryDatosCliente);

														?>
														<div class="row">
															<div class="col-lg-3">
																<div class="form-group" id="DIVCIF">
																	<input class="form-control" type="text" name="NIT" id="NIT" onchange="ComprobarNIT(this.value)" value="<?php echo $FilaDatosCliente[CE_NIT] ?>" autofocus required/>
															...
															<div class="col-lg-3">
																<div class="form-group" id="DIVCIF">
																	<input class="form-control" type="text" name="Nombre" id="Nombre" value="<?php echo $FilaDatosCliente[CE_NOMBRE] ?>" required/>
														...
														<div class="row">
															<div class="col-lg-6">
																<div class="form-group" id="DIVCIF">
																	<input class="form-control" type="text" name="Direccion" id="Direccion" value="<?php echo $FilaDatosCliente[CE_DIRECCION] ?>"  required/>
														...
														<div class="row">
															<div class="col-lg-10">
																<div class="form-group">
																	<input class="form-control" type="text" name="Observaciones" id="Observaciones" value="Evento con código de cotización #<?php echo $_GET[Codigo] ?>"/>
														...
														<div class="form-group" style="display: none">
															<div class="col-lg-3">
																<div class="input-group">
																	<span class="input-group-addon"><i id="NombreDescunto"><h3><b>Sin Descuento</b></h3></i></span>
																	<div class="input-group-content">
																		<input type="text" class="form-control" id="PorcientoDescuento" value="0.00" name="PorcientoDescuento" readonly>
																		<label for="PorcientoDescuento">Descuento</label>
																	</div>
														...
														<div class="row">
															<div class="form-group">
																<label class="col-sm-3 control-label">Tipo Impresión Factura</label>
																<div class="col-sm-9">
																	<div class="radio radio-styled">
																		<label>
																			<input type="radio" name="TipoImpresion" value="1" checked="" onchange="TipoImpresionFnc(this.value)">
																			<span>Factura con Detalle Cotización</span>
																		</label>
																	</div>
																	<div class="radio radio-styled">
																		<label>
																			<input type="radio" name="TipoImpresion" value="2" onchange="TipoImpresionFnc(this.value)">
																			<span>Factura Personalizada</span>
																		</label>
																	</div>
																</div><!--end .col -->
															</div>
														...
														<div class="row" >
															<table class="table" name="tabla" id="tabla">
																...
																<tbody>
						                                            ...
						                                            <?php
						                                            	$QueryPlatillos = mysqli_query($db, "SELECT A.RS_CODIGO, A.PE_PRECIO, A.PE_CANTIDAD, A.PE_SUBTOTAL, A.PE_DESCUENTO, B.RS_NOMBRE
																										FROM Bodega.PLATILLO_EVENTO AS A 
																										INNER JOIN Bodega.RECETA_SUBRECETA AS B ON A.RS_CODIGO = B.RS_CODIGO
																										WHERE A.C_REFERENCIA = '".$FilaDatosCliente[C_REFERENCIA]."'");
						                                            	while($FilaPlatillos = mysqli_fetch_array($QueryPlatillos))
						                                            	{
						                                            		?>
																				<tr>
									                                                <td><h6><input type="number" readonly class="form-control" name="Cantidad[]" id="Cantidad[]" onChange="CalcularTotal()" style="width: 100px" min="0" value="<?php echo $FilaPlatillos[PE_CANTIDAD] ?>"></h6></td>
									                                                <td><h6><input type="hidden" class="form-control" name="Producto[]" id="Producto[]" value="<?php echo $FilaPlatillos[RS_CODIGO] ?>"></h6>
									                                                	<h6><input type="text" class="form-control" name="ProductoNombre[]" id="ProductoNombre[]" style="width: 500px" value="<?php echo $FilaPlatillos[RS_NOMBRE] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="Precio[]" id="Precio[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaPlatillos[PE_SUBTOTAL] / $FilaPlatillos[PE_CANTIDAD] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="SubTotal[]" id="SubTotal[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaPlatillos[PE_SUBTOTAL] ?>" readonly></h6></td>
									                                            </tr>
						                                            		<?php

						                                            		$TotalDescuentosFactura = $TotalDescuentosFactura + $FilaPlatillos[PE_DESCUENTO];
						                                            	}

						                                            	$QueryMobiliario = mysqli_query($db, "SELECT *
						                                            	...
						                                            	while($FilaMobiliario = mysqli_fetch_array($QueryMobiliario))
						                                            	{
						                                            		?>
																				<tr>
									                                                <td><h6><input type="number" readonly class="form-control" name="Cantidad[]" id="Cantidad[]" onChange="CalcularTotal()" style="width: 100px" min="0" value="<?php echo $FilaMobiliario[M_CANTIDAD] ?>"></h6></td>
									                                                <td><h6><input type="hidden" class="form-control" name="Producto[]" id="Producto[]" value="M" readonly></h6>
									                                                	<h6><input type="text" class="form-control" name="ProductoNombre[]" id="ProductoNombre[]" style="width: 500px" value="<?php echo $FilaMobiliario[M_NOMBRE].' '.$FilaMobiliario[TM_NOMBRE] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="Precio[]" id="Precio[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaMobiliario[M_SUBTOTAL] / $FilaMobiliario[M_CANTIDAD] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="SubTotal[]" id="SubTotal[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaMobiliario[M_SUBTOTAL] ?>" readonly></h6></td>
									                                            </tr>
						                                            		<?php
						                                            	}
						                                            	...
						                                            	$QueryMobiliario = mysqli_query($db, "SELECT *
																										...
																										INNER JOIN Bodega.MOBILIARIO_ALQUILER AS B ON A.MA_CODIGO = B.MA_CODIGO
																										INNER JOIN Bodega.TIPO_MOBILIARIO AS C ON B.TM_CODIGO = C.TM_CODIGO
																										WHERE A.C_REFERENCIA = '".$FilaDatosCliente[C_REFERENCIA]."'");
						                                            	while($FilaMobiliario = mysqli_fetch_array($QueryMobiliario))
						                                            	{
						                                            		?>
																				<tr>
									                                                <td><h6><input type="number" readonly class="form-control" name="Cantidad[]" id="Cantidad[]" onChange="CalcularTotal()" style="width: 100px" min="0" value="<?php echo $FilaMobiliario[MEA_CANTIDAD] ?>"></h6></td>
									                                                <td><h6><input type="hidden" class="form-control" name="Producto[]" id="Producto[]" value="MA"></h6>
									                                                	<h6><input type="text" class="form-control" name="ProductoNombre[]" id="ProductoNombre[]" style="width: 500px" value="<?php echo $FilaMobiliario[MA_NOMBRE].' '.$FilaMobiliario[TM_NOMBRE] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="Precio[]" id="Precio[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaMobiliario[MEA_SUBTOTAL] / $FilaMobiliario[MEA_CANTIDAD] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="SubTotal[]" id="SubTotal[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaMobiliario[MEA_SUBTOTAL] ?>" readonly></h6></td>
									                                            </tr>
						                                            		<?php
						                                            	}
						                                            	...
						                                            	while($FilaServicios = mysqli_fetch_array($QueryServicios))
						                                            	{
						                                            		?>
																				<tr>
									                                                <td><h6><input type="number" readonly class="form-control" name="Cantidad[]" id="Cantidad[]" onChange="CalcularTotal()" style="width: 100px" min="0" value="<?php echo $FilaServicios[SEC_CANTIDAD] ?>"></h6></td>
									                                                <td><h6><input type="hidden" class="form-control" name="Producto[]" id="Producto[]" value="SE"></h6>
									                                                	<h6><input type="text" class="form-control" name="ProductoNombre[]" id="ProductoNombre[]" style="width: 500px" value="<?php echo $FilaServicios[SE_NOMBRE] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="Precio[]" id="Precio[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaServicios[SEC_SUBTOTAL] / $FilaServicios[SEC_CANTIDAD] ?>" readonly></h6></td>
									                                                <td><h6><input type="number" class="form-control" name="SubTotal[]" id="SubTotal[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaServicios[SEC_SUBTOTAL] ?>" readonly></h6></td>
									                                            </tr>
						                                            		<?php
						                                            	}

						                                            	$QueryDegustacion = mysqli_query($db, "SELECT A.D_CODIGO
																											FROM Bodega.COTIZACION AS A
																											WHERE A.C_CODIGO = ".$_GET[Codigo]);
						                                            	$FilaDegustacion = mysqli_fetch_array($QueryDegustacion);

						                                            	if($FilaDegustacion[D_CODIGO] != '')
						                                            	{
						                                            		$QueryDetalleDegustacion = mysqli_query($db, "SELECT C.PD_CANTIDAD, D.RS_CODIGO, D.RS_NOMBRE, D.RS_PRECIO
																														FROM Bodega.DEGUSTACION AS A
																														INNER JOIN Bodega.COTIZACION AS B ON A.D_REFERENCIA = B.D_CODIGO
																														INNER JOIN Bodega.PLATILLO_DEGUSTACION AS C ON A.D_REFERENCIA = C.D_REFERENCIA
																														INNER JOIN Bodega.RECETA_SUBRECETA AS D ON C.RS_CODIGO = D.RS_CODIGO
																														WHERE B.C_CODIGO = ".$_GET[Codigo]);
						                                            		while($FilaDetalleDegustacion = mysqli_fetch_array($QueryDetalleDegustacion))
							                                            		{
							                                            		?>
																					<tr>
										                                                <td><h6><input type="number" readonly class="form-control" name="Cantidad[]" id="Cantidad[]" onChange="CalcularTotal()" style="width: 100px" min="0" value="<?php echo $FilaDetalleDegustacion[PD_CANTIDAD] ?>"></h6></td>
										                                                <td><h6><input type="hidden" class="form-control" name="Producto[]" id="Producto[]" value="<?php echo $FilaDetalleDegustacion[RS_CODIGO] ?>"></h6>
										                                                	<h6><input type="text" class="form-control" name="ProductoNombre[]" id="ProductoNombre[]" style="width: 500px" value="<?php echo $FilaDetalleDegustacion[RS_NOMBRE] ?>" readonly></h6></td>
										                                                <td><h6><input type="number" class="form-control" name="Precio[]" id="Precio[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo $FilaDetalleDegustacion[RS_PRECIO] ?>" onChange="CalcularTotal()"></h6></td>
										                                                <td><h6><input type="number" class="form-control" name="SubTotal[]" id="SubTotal[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="<?php echo ($FilaDetalleDegustacion[PD_CANTIDAD] * $FilaDetalleDegustacion[RS_PRECIO]) ?>" readonly></h6></td>
										                                            </tr>
							                                            		<?php
							                                            	}
						                                            	}
																...
																<tfoot>
																	<tr style="display: none">
						                                                ...
						                                                <td style="text-align: left; vertical-align: text-top; font-size: 18px"><input class="form-control" type="text" id="BoldDescuento" name="TotalFacturaConDescuento" value="<?php echo number_format($TotalDescuentosFactura, 2) ?>" readonly></td>
						                                                <td></td>
						                                            </tr>
																	<tr>
						                                                <td></td>
														...
														<div class="row" id="DIVDetallePersonalizado" style="display: none">
															<table class="table table-hover table-condensed" id="TBLPersonalizado">
																<thead>
																	<tr>
																		<th>Cantidad</th>
																		<th>Producto</th>
																...
																<tbody>
																	<tr class="fila-base">
						                                                <td><h6><input type="number" class="form-control" name="CantidadPersonalizado[]" id="CantidadPersonalizado[]" onChange="CalcularTotalPersonalizado()" style="width: 100px" min="0"></h6></td>
						                                                <td><h6><input type="text" class="form-control" name="ProductoPersonalizado[]" id="ProductoPersonalizado[]"></h6></td>
						                                                <td><h6><input type="number" class="form-control" name="PrecioPersonalizado[]" id="PrecioPersonalizado[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="0.00" onChange="CalcularTotalPersonalizado()"></h6></td>
						                                                <td><h6><input type="number" class="form-control" name="SubTotalPersonalizado[]" id="SubTotalPersonalizado[]"  style="width: 100px" min="0" step="any" style="text-align: right" value="0.00" readonly></h6></td>
						                                                <td class="eliminarPersonalizado">
						                                                    <button type="button" class="btn btn-danger btn-lg" onclick="EliminarLineaPersonalizado(this)">
						                                                        <span class="glyphicon glyphicon-trash"></span>
						                                                    </button>
						                                                </td>
						                                            </tr>
																...
																<tfoot>
																	<tr>
						                                                ...
						                                                <td></td>
						                                                <td style="text-align: right; vertical-align: text-top; font-size: 20px"><b>Total Q.</b></td>
						                                                <td style="text-align: left; vertical-align: text-top; font-size: 20px"><input class="form-control" type="text" id="BoldTotalPersonalizado" name="TotalFacturaFinalPersonalizado" value="0.00" readonly></td>
						                                                <td></td>
						                                            </tr>
																</tfoot>
															</table>
															<div class="col-lg-12" align="left">
						                                        <button type="button" class="btn btn-success btn-md" id="agregar" onclick="AgregarLineaPersonalizado()">
						                                            <span class="glyphicon glyphicon-plus"></span> Agregar
						                                        </button>
						                                    </div>
														</div>
