...
$sql = "SELECT * FROM Bodega.FACTURA_EV AS A LEFT JOIN Bodega.RESOLUCION AS B ON A.RES_NUMERO = B.RES_NUMERO WHERE A.F_CODIGO = '".$Factura."'";
...
if($DescripcionFactura==NULL & $VariasDes==0){
 
$sqlD = "SELECT * FROM Bodega.FACTURA_EV_DETALLE WHERE F_CODIGO = '".$Factura."'";
...
while($rowD = mysqli_fetch_array($resultD))
{
    if($rowD["FD_CANTIDAD"] != 0)
    {
        ...
        {
            $QueryNombreProducto = mysqli_query($db, "SELECT P_NOMBRE FROM Productos.PRODUCTO WHERE P_CODIGO = '".$CodigoReceta."'");
...
EOD;
    ...
    $sqlD = "SELECT * FROM Bodega.FACTURA_EV_DESCRIPCION WHERE F_CODIGO = '".$Factura."'";
$resultD = mysqli_query($db, $sqlD);
while($rowD = mysqli_fetch_array($resultD))
{
    if($rowD["FD_CANTIDAD"] != 0)
...
EOD;
    ...
    $TablaProducto .= <<<EOD
    <tr>
<td align="left" style="font-size: 30px; width: 10%;">$CantidadDes</td>
<td align="left" style="font-size: 30px; width: 30%">$DescripcionFactura</td>
<td align="right" style="font-size: 30px; width: 20%;">$PrecioDes</td>
<td align="right" style="font-size: 30px; width: 20%;">$Descuenton</td>
<td align="right" style="font-size: 30px; width: 20%;">$TotalFactura</td>
</tr>
EOD;
}
