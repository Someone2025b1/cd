...
while($fila = mysqli_fetch_array($asociado))
{
	...
	while($FilaAcompananteTemporal = mysqli_fetch_array($QueryConsultaTemporal))
	{
		...
		if($QueryIngresoAcompaniante){
			$Cantidad=$Cantidad+1;
		}
	}

	//obtener variables menores 

	$QueryConsultaTemporalMenor = mysqli_query($db, "SELECT * FROM Taquilla.INGRESO_ACOMPANIANTE_MENORES_TEMPORAL WHERE USUARIO = ".$id_user." AND ASOCIADO = ".$CIFAsociado)or die('Error 3: '.mysqli_error($QueryConsultaTemporal));
	while($FilaAcompananteTemporalMenor = mysqli_fetch_array($QueryConsultaTemporalMenor))
	{
		$CantidadMenor=$FilaAcompananteTemporalMenor["NM_CANTIDAD"];
	}
$NombreNoAsociado = "Ingreso Menor con Asociado CIF $CIFAsociado";
$PaisNoAsociado = 73;
$DepartamentoNoAsociado = 19;	
$MunicipioNoAsociado = 1;
	
		
$SelectVisitaEsquipulas = 1;
$EnteradoNoAsociado = 1;
$EdadNoAsociado =0;

$SelectAsisteconNoAsociado =1;
$FrecuenciaVisitaNoAsociado =1;
$CorreoNoAsociado ="";
$SelectConociaParque = 0;
$BuscaNoAsociado =0;
$NumeroTelefonoNoAsociado = 0; 
$trozos = explode(", ", $PaisNoAsociado);
$municipio= 1;
$depto =  19;

if($QueryConsultaTemporalMenor){
$GuardarNoAsociado = mysqli_query($db, "INSERT INTO Taquilla.INGRESO_NO_ASOCIADO VALUES (NULL, '$NombreNoAsociado', 73, $depto, $municipio, $SelectVisitaEsquipulas, $EnteradoNoAsociado, $EdadNoAsociado, $SelectAsisteconNoAsociado, $FrecuenciaVisitaNoAsociado, '$CorreoNoAsociado', '$NumeroTelefonoNoAsociado', $SelectConociaParque, $BuscaNoAsociado, $id_user, CURDATE(), 0, 0, $CantidadMenor, 0) ");
}
if($GuardarNoAsociado){
	$Cantidad=$Cantidad+$CantidadMenor;
}
...
$QueryLimpieza2 = mysqli_query($db, "DELETE FROM Taquilla.INGRESO_ACOMPANIANTE_MENORES_TEMPORAL WHERE USUARIO = ".$id_user)or die('Error 6: '.mysqli_error($QueryLimpieza2));	
