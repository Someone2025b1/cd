...
$sql = "SELECT * FROM Bodega.FACTURA_TQ_2 AS A LEFT JOIN Bodega.RESOLUCION AS B ON A.RES_NUMERO = B.RES_NUMERO WHERE A.F_CODIGO = '".$Factura."'";
...
while($row = mysqli_fetch_array($result))
{
    ...
    $FechaHora = date('d-m-Y H:i:s', strtotime($row["F_FECHA_TRANS"].$row["F_HORA"]));
...
$sqlD = "SELECT * FROM Bodega.FACTURA_TQ_2_DETALLE WHERE F_CODIGO = '".$Factura."'";
...
if($rowD["RS_CODIGO"]==2629){
    $contJuego=1;
}elseif($rowD["RS_CODIGO"]==2628){
    $contJuego=1;
}elseif($rowD["RS_CODIGO"]==2627){
    $contJuego=1;
}elseif($rowD["RS_CODIGO"]==2626){
    $contJuego=1;
}elseif($rowD["RS_CODIGO"]==2625){
    $contJuego=1;
}elseif($rowD["RS_CODIGO"]==2624){
    $contJuego=1;
}elseif($rowD["RS_CODIGO"]==2623){
    $contJuego=1;
}elseif($rowD["RS_CODIGO"]==2630){
    $contJuego=1;
}
...
while($rowD = mysqli_fetch_array($resultD))
{
    ...
    if($codigopr==2629|$codigopr==2628|$codigopr==2627|$codigopr==2626|$codigopr==2625|$codigopr==2624|$codigopr==2623 |$codigopr==2630){
...
while($rowD1 = mysqli_fetch_array($resultD1))
{
        ...
        <table border="0">  
            ...
            <tr style="font-size: 30px">
                <td align="left"><b>PS:</b> Taquilla #2 </td> 
            </tr>   
            ...
            <tr style="font-size: 34px">
                <td colspan="8" align="center"> 1 $NombreProdu </td>
            </tr>   
            <tr style="font-size: 34px">
