					...
					<div class="card">
						...
						<div class="card-body">
							<?php
								...
								$sql = mysqli_query($db,"UPDATE Bodega.FACTURA_PIZZA SET F_ESTADO = 2, F_RAZON_ANULACION = '".$_POST["Razon"]."', F_TOTAL = 0.00 WHERE F_CODIGO = '".$_POST["Codigo"]."'") or die(mysqli_error());
								if(!$sql)
								{
									$Centinela = false;
								...
								$sqlDetalle = mysqli_query($db,"DELETE FROM Bodega.FACTURA_PIZZA_DETALLE WHERE F_CODIGO = '".$_POST["Codigo"]."'") or die(mysqli_error());
								if(!$sqlDetalle)
								{
									$Centinela = false;
								...
								$Consulta = mysqli_query($db,"UPDATE Contabilidad.TRANSACCION SET TRA_ESTADO = 2  WHERE TRA_CODIGO = '".$_POST["Codigo"]."'") or die(mysqli_error());
								if(!$Consulta)
								{
									$Centinela = false;
