					...
					<div class="card">
						...
						<div class="card-body">
							<table class="table table-hover table-condensed" id="tbl_resultados">
								...
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Bodega.SERVICIO_EVENTO";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
										$Codigo = $row["SE_CODIGO"];
										echo '<tr>';
										    	echo '<td style="font-size: 10px">'.$Contador.'</td>';               
										    	echo '<td style="font-size: 10px">'.$row["SE_NOMBRE"].'</td>';
										    	echo '<td style="font-size: 10px">'.number_format($row["SE_PRECIO_UNITARIO"], 2).'</td>';
										    	echo '<td><a href="ServiciosMod.php?Codigo='.$Codigo.'"><button type="button" class="btn btn-warning btn-xs">
