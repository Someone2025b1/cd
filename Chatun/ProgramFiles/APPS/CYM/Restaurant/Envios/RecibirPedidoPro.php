						...
						<div class="card">
							...
							while($rowe = mysqli_fetch_array($resultsa))
							{
								...
								if($Envia=="BODEGA GENERAL"){
									...
									{
										$existenciabode=$rowpr["P_EXISTENCIA_BODEGA"];
										$newcantbo=$rowpr["P_EXISTENCIA_BODEGA"]-$CAN;
									...
									}elseif($Envia=="KIOSCO AZADOS"){
	
										$querypr = "SELECT * FROM Productos.PRODUCTO WHERE P_CODIGO = ".$Pro;
									$resultpr = mysqli_query($db, $querypr);
									while($rowpr = mysqli_fetch_array($resultpr))
									{
										$existenciabode=$rowpr["P_EXISTENCIA_KIOSCO"];
										$newcantbo=$rowpr["P_EXISTENCIA_KIOSCO"]-$CAN;
										$newcantsou=$rowpr["P_EXISTENCIA_TERRAZAS"]+$CAN;
										$existenciasou=$rowpr["P_EXISTENCIA_TERRAZAS"];
										$existenciagen=$rowpr["P_EXISTENCIA_GENERAL"];
										$ponderado = $rowpr["P_PRECIO_COMPRA_PONDERADO"];
									...
									}elseif($Envia=="PIZZERIA"){
	
										$querypr = "SELECT * FROM Productos.PRODUCTO WHERE P_CODIGO = ".$Pro;
									$resultpr = mysqli_query($db, $querypr);
									while($rowpr = mysqli_fetch_array($resultpr))
									{
										$existenciabode=$rowpr["P_EXISTENCIA_PIZZERIA"];
										$newcantbo=$rowpr["P_EXISTENCIA_PIZZERIA"]-$CAN;
										$newcantsou=$rowpr["P_EXISTENCIA_TERRAZAS"]+$CAN;
										$existenciasou=$rowpr["P_EXISTENCIA_TERRAZAS"];
										$existenciagen=$rowpr["P_EXISTENCIA_GENERAL"];
										$ponderado = $rowpr["P_PRECIO_COMPRA_PONDERADO"];
									...
									$QueryPU = mysqli_query($db, "UPDATE Productos.PRODUCTO SET
														P_EXISTENCIA_PIZZERIA = '".$newcantbo."',
														P_EXISTENCIA_TERRAZAS = '".$newcantsou."'
														WHERE P_CODIGO = ".$Pro);
								...
								VALUES('".$KCod."', '".$CodEN."', '".$Pro."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Envío de producto interno de ".$Envia." a Terrazas', '".$existenciagen."', '".$existenciagen."', 1, '".$existenciasou."', '".$newcantsou."', '-', '-', '-')");
								...
								VALUES('".$KCod."', '".$CodEN."', '".$Pro."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Envío de producto interno de ".$Envia." a Terrazas', '".$existenciagen."', '".$existenciagen."', '".$Pun."', '".$existenciabode."', '".$newcantbo."', '-', '-', '".$ponderado."')");
