					...
					<div class="col-lg-12">
						...
						<div class="card">
							<div class="card-head style-primary">
								<h4 class="text-center"><strong>Nota de Cargo</strong></h4>
							</div>
							<div class="card-body">
								<?php
								$UI           = uniqid('tra_');
								...
								$sql = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO)
									VALUES('".$UI."', CURRENT_DATE(), '".$Observaciones."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$UserID .", 1, 7, ".$Bodega.")");
									...
									if(!$sql) //Si el query tuvo algún problema que despligue error y que la ejecución del código se detenga
									{
										...
										<h2 class="text-light">Lo sentimos, no se pudo realizar la nota de cargo a bodega.</h2>
								...
								else
								{
									for($i=0; $i<$Contador; $i++)
									{
										...
										$query = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION_DETALLE (TRAD_CODIGO, TRA_CODIGO, P_CODIGO, TRAD_CARGO_PRODUCTO, TRAD_PRECIO_UNITARIO, TRAD_SUBTOTAL)
											VALUES('".$UID."', '".$UI."', ".$Pro.", ".$Can.", ".$Unitario.", ".$Cos.")");
								...
								if($Centinela == true)
								{
									...
									<h2 class="text-light">La nota de cargo se realizó correctamente.</h2>
									<div class="row">
										<div class="col-lg-6 text-left"><a href="NC.php"><button type="button" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-chevron-left"></span> Regresar</a></div>
									</div>';
								}
							}
