...
function operar() {
	if (document.form_bienes_inmuebles.descripcion.value != ""){
			if (form_bienes_inmuebles.monto.value > 0){
					form_bienes_inmuebles.submit()
						}else{
			alert("El Monto debe contener valores numericos")
			}
	}else{
...
function operar1() {
	if (document.form_bienes_inmuebles_editar.descripcion.value != ""){
...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	if ($_GET["accion"] == 1) {
		...
		$insertar = "INSERT INTO Estado_Patrimonial.detalle_otros_ingresos (colaborador, descripcion, monto, mes, anio) VALUES ('$auxi', '$descripcion', '$monto', $mes_periodo, $anio_periodo)";
	mysqli_query($db, $insertar) or die (mysqli_error());
	}
	if ($_GET["accion"] == 4) {
		$id = $_GET["id"];
		$eliminar = "DELETE FROM Estado_Patrimonial.detalle_otros_ingresos WHERE id = '$id' and mes=$mes_periodo and anio=$anio_periodo ";
	...
	if ($_GET["accion"] == 3) {
		...
		$monto_1 = $_POST["monto"];
		$monto = str_replace(",","",$monto_1);
		$actualizar = "UPDATE Estado_Patrimonial.detalle_otros_ingresos SET colaborador = '$auxi', descripcion = '$descripcion', monto = '$monto' WHERE id = '$id' and mes=$mes_periodo and anio=$anio_periodo ";
...
<form action="otros_ingresos.php?accion=1" method="post" name="form_bienes_inmuebles" id="form_bienes_inmuebles">
  <tr>
    <td colspan="2" align="center"><b>DETALLE OTROS INGRESOS</b>
      <label for="descripcion"></label></td>
    </tr>
  <tr>
    <td width="222">Descripcion Ingreso:</td>
...
<?php
		...
		$id = $_GET["id"];
		$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_otros_ingresos WHERE colaborador = '$auxi' and id = '$id' and mes=$mes_periodo and anio=$anio_periodo ";
	$query_historial = mysqli_query($db, $sql_historial);
	while ($editar=mysqli_fetch_array($query_historial)) 	{ 	
?>
<table width="600" border="0" align="center" id="tabla_bienes_inmuebles" class="Tamaño12">
<form action="otros_ingresos.php?accion=3&id=<?php echo $editar['id'] ?>" method="post" name="form_bienes_inmuebles_editar" id="form_bienes_inmuebles_editar">
  <tr>
    <td colspan="2" align="center"><b>DETALLE OTROS INGRESOS</b></td>
    </tr>
  <tr>
    <td width="222">Descripcion Ingreso:</td>
    <td><label for="descripcion"></label>
      <input name="descripcion" type="text" id="descripcion" value="<?php echo $editar["descripcion"] ?>"></td>
    </tr>
  <tr>
    <td>Monto mensual:</td>
...
<tr> 
	<td width="64%" bgcolor="#000000" class="LetraBlanca" align="center"><b>Descripci&oacute;n</b></td>
	<td width="28%" bgcolor="#000000" class="LetraBlanca" align="center"><b>Monto:</b></td>
	<td width="8%" bgcolor="#000000" class="LetraBlanca" align="center">&nbsp;</td>
  </tr>
  <?php
	$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_otros_ingresos WHERE colaborador = '$auxi' and mes=$mes_periodo and anio=$anio_periodo ";
	$query_historial = mysqli_query($db, $sql_historial);
	while ($historial=mysqli_fetch_array($query_historial)) 	{ 
?>
<tr> 
<td><? echo $historial ["descripcion"] ?></font></td>
<td><? echo $historial["monto"] ?></font></td>
<td><a href="otros_ingresos.php?accion=2&id=<?php echo $historial['id']?>"><img src="../Imagenes/edit.png" alt="Editar..." width="16" height="16" border="0"></a><a href="otros_ingresos.php?accion=4&id=<?php echo $historial['id']?>"><img src="../Imagenes/borrar.png" alt="Borrar..." width="16" height="16" border="0"></a></td>
<?php 
	}	//end while
?>
</tr>
