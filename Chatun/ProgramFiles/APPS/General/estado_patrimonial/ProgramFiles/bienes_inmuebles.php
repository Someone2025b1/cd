...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	if ($_GET["accion"] == 1) {
		...
		$insertar = "INSERT INTO Estado_Patrimonial.detalle_bienes_inmuebles (id_tipo_inmueble, colaborador, localizacion, finca, folio, libro, departamento, valor_mercado, mes, anio) VALUES ('$tipo_inmueble', '$auxi', '$localizacion', '$finca', '$folio', '$libro', '$departamento', '$valor_mercado', $mes_periodo, $anio_periodo)";
	mysqli_query($db, $insertar) or die (mysqli_error());
	}
	if ($_GET["accion"] == 4) {
		$id = $_GET["id"];
		$eliminar = "DELETE FROM Estado_Patrimonial.detalle_bienes_inmuebles WHERE id = '$id' and mes=$mes_periodo and anio=$anio_periodo ";
	...
	if ($_GET["accion"] == 3) {
		...
		$valor_mercado_ingresado = $_POST["valor_mercado"];
		$valor_mercado = str_replace(",","",$valor_mercado_ingresado);
		$actualizar = "UPDATE Estado_Patrimonial.detalle_bienes_inmuebles SET id_tipo_inmueble = '$tipo_inmueble', colaborador = '$auxi', localizacion = '$localizacion', finca = '$finca', libro = '$libro', departamento = '$departamento', valor_mercado = '$valor_mercado' WHERE id = '$id' and mes=$mes_periodo and anio=$anio_periodo  ";
...
<form action="bienes_inmuebles.php?accion=1" method="post" name="form_bienes_inmuebles" id="form_bienes_inmuebles">
  <tr>
    <td colspan="2" align="center"><b>DETALLE DE BIENES INMUEBLES</b>
      <label for="localizacion"></label></td>
    </tr>
  <tr>
    <td width="222">Tipo de Inmueble:</td>
    <td>
<?php
	$sql1="SELECT * FROM Estado_Patrimonial.tipo_inmueble";
	$res1=mysqli_query($db, $sql1) or die(mysqli_error());
?>
    <select name="tipo_inmueble" size="1" id="tipo_inmueble">
	<option value = "0">::Seleccione una opcion::</option>
<?php
	while ($rw1=mysqli_fetch_array($res1)) {
?>
	<option value="<?php echo $rw1["id_inmueble"] ?>"> <?php echo $rw1["tipo_inmueble"]?> </option>
<?php
	}
?>
	</select>
  ...
  <tr>
    <td width="222">Localizaci&oacute;n:</td>
    <td><label for="localizacion"></label>
      <input type="text" name="localizacion" id="localizacion"></td>
    </tr>
  <tr>
    <td>Finca:</td>
    <td><input type="text" name="finca" id="finca"></td>
  </tr>
  <tr>
    <td>Folio:</td>
  ...
  <tr>
    ...
    <td><input type="text" name="valor_mercado" id="valor_mercado"></td>
  </tr>
  <tr>
...
<?php
		...
		$id = $_GET["id"];
		$auxi=$_SESSION["iduser"];
		$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_bienes_inmuebles WHERE colaborador = '$auxi' and id = '$id' and mes=$mes_periodo and anio=$anio_periodo ";
	$query_historial = mysqli_query($db, $sql_historial);
	while ($editar=mysqli_fetch_array($query_historial)) 	{ 	
?>
<table width="600" border="0" align="center" id="tabla_bienes_inmuebles" class="Tamaño12">
<form action="bienes_inmuebles.php?accion=3&id=<?php echo $editar['id'] ?>" method="post" name="form_bienes_inmuebles_editar" id="form_bienes_inmuebles_editar">
  <tr>
    <td colspan="2" align="center"><b>DETALLE DE BIENES INMUEBLES</b>
      <label for="localizacion"></label></td>
    </tr>
...
<?php
	...
	while ($rw1=mysqli_fetch_array($res1)) {
		if ($rw1["id_inmueble"] == $editar["id_tipo_inmueble"]) {
					...
					?>
                         ...
                         <option value="<?php echo $rw1["id_inmueble"] ?>" <?php echo $seleccionado ?>> <?php echo $rw1["tipo_inmueble"]?> </option>
                          <?php
					  }
					 ?>
                        </select>    
  ...
  <tr>
    <td width="222">Localizaci&oacute;n:</td>
    <td><label for="localizacion"></label>
      <input name="localizacion" type="text" id="localizacion" value="<?php echo $editar["localizacion"] ?>"></td>
    </tr>
  <tr>
    <td>Finca:</td>
    <td><input name="finca" type="text" id="finca" value="<?php echo $editar["finca"] ?>"></td>
  </tr>
  <tr>
    <td>Folio:</td>
    <td><input name="folio" type="text" id="folio" value="<?php echo $editar["folio"] ?>"></td>
  </tr>
  <tr>
    <td>Libro:</td>
    <td><input name="libro" type="text" id="libro" value="<?php echo $editar["libro"] ?>"></td>
  </tr>
  <tr>
    <td>Departamento:</td>
    <td><input name="departamento" type="text" id="departamento" value="<?php echo $editar["departamento"] ?>"></td>
  </tr>
  <tr>
    <td>Valor de Mercado:</td>
    <td><input name="valor_mercado" type="text" id="valor_mercado" value="<?php echo $editar["valor_mercado"] ?>"></td>
  </tr>
  <tr>
    <td colspan="2" align="center">
...
<tr> 
  ...
  <?php
  $auxi=$_SESSION["iduser"];
	$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_bienes_inmuebles WHERE colaborador = '$auxi' and mes=$mes_periodo and anio=$anio_periodo ";
	$query_historial = mysqli_query($db, $sql_historial);
...
<td><a href="bienes_inmuebles.php?accion=2&id=<?php echo $historial['id']?>"><img src="../Imagenes/edit.png" alt="Editar..." width="16" height="16" border="0"></a><a href="bienes_inmuebles.php?accion=4&id=<?php echo $historial['id']?>"><img src="../Imagenes/borrar.png" alt="Borrar..." width="16" height="16" border="0"></a></td>
<?php 
	}	//end while
?>
</tr>
