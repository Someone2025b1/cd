...
<body class="menubar-hoverable header-fixed menubar-pin ">
	...
	<div id="base">
		...
		<div id="content">
			<?php
				...
				SUM(CIERRE_DETALLE.CD_Q_200) AS  CD_Q_200, SUM(CIERRE_DETALLE.CD_Q_100) AS  CD_Q_100, SUM(CIERRE_DETALLE.CD_Q_50) AS  CD_Q_50, SUM(CIERRE_DETALLE.CD_Q_20) AS  CD_Q_20, SUM(CIERRE_DETALLE.CD_Q_10) AS  CD_Q_10, SUM(CIERRE_DETALLE.CD_Q_5) AS  CD_Q_5, SUM(CIERRE_DETALLE.CD_Q_1) AS  CD_Q_1, SUM(CIERRE_DETALLE.CD_M_1) AS  CD_M_1, SUM(CIERRE_DETALLE.CD_M_50) AS  CD_M_50, SUM(CIERRE_DETALLE.CD_M_25) AS  CD_M_25, SUM(CIERRE_DETALLE.CD_M_10) AS  CD_M_10,  SUM(CIERRE_DETALLE.CD_M_5) AS  CD_M_5, SUM(CIERRE_DETALLE.CD_TOTAL_Q) AS  CD_TOTAL_Q, SUM(CIERRE_DETALLE.CD_D_100) AS  CD_D_100, SUM(CIERRE_DETALLE.CD_D_50) AS  CD_D_50,  SUM(CIERRE_DETALLE.CD_D_20) AS  CD_D_20, SUM(CIERRE_DETALLE.CD_D_10) AS  CD_D_10,  SUM(CIERRE_DETALLE.CD_D_5) AS  CD_D_5, SUM(CIERRE_DETALLE.CD_D_1) AS  CD_D_1, SUM(CIERRE_DETALLE.CD_TOTAL_D) AS  CD_TOTAL_D, SUM(CIERRE_DETALLE.CD_L_500) AS  CD_L_500,  SUM(CIERRE_DETALLE.CD_L_100) AS  CD_L_100, SUM(CIERRE_DETALLE.CD_L_50) AS  CD_L_50, SUM(CIERRE_DETALLE.CD_L_20) AS  CD_L_20, SUM(CIERRE_DETALLE.CD_L_10) AS  CD_L_10, SUM(CIERRE_DETALLE.CD_L_5) AS  CD_L_5, SUM(CIERRE_DETALLE.CD_TOTAL_L) AS  CD_TOTAL_L,  SUM(CIERRE_DETALLE.CD_TOTAL_FALTANTE) AS  CD_TOTAL_FALTANTE, SUM(CIERRE_DETALLE.CD_TOTAL_SOBRANTE) AS  CD_TOTAL_SOBRANTE 
					...
					AND APERTURA_CIERRE_CAJA.ACC_TIPO = 3
				...
				$QueryMinMaxFacturas = "SELECT MAX(F_NUMERO) AS MAXIMO, MIN(F_NUMERO) AS MINIMO, F_SERIE FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."'";
				...
				$Query = "SELECT COUNT(F_CODIGO) AS FACTURAS_EMITIDAS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."'";
				...
				$Query1 = "SELECT COUNT(F_CODIGO) AS FACTURAS_ANULADAS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_ESTADO = 2";
				...
				$Query3 = "SELECT SUM(F_TOTAL) AS TOTAL_CREDITO FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 2 AND F_ESTADO = 1";
				...
				$Query4 = "SELECT SUM(F_TOTAL) AS TOTAL_DOLARES FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 1 AND F_MONEDA = 2 AND F_ESTADO = 1";
				...
				$Query4 = "SELECT SUM(F_TOTAL) AS TOTAL_LEMPIRAS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 1 AND F_MONEDA = 3 AND F_ESTADO = 1";
				$Result4 = mysqli_query($db, $Query4);
				while($row4 = mysqli_fetch_array($Result4))
				{
				...
				$Query5 = "SELECT SUM(F_TOTAL) AS TOTAL_DEPOSITOS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 4 AND F_ESTADO = 1";
				...
				$Query6 = "SELECT SUM(F_TOTAL) AS TOTAL_CREDITOS FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 3 AND F_ESTADO = 1";
				...
				$Query7 = "SELECT SUM(F_TOTAL) AS TOTAL_EFECTIVO FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_MONEDA = 1 AND F_ESTADO = 1";
				...
				$Query8 = "SELECT ACC_SALDO_INICIAL AS TOTAL_APERTURA FROM Bodega.APERTURA_CIERRE_CAJA WHERE ACC_FECHA = '".$Fecha."' AND ACC_TIPO = 3";
				...
				$Query9 = "SELECT SUM(F_CAMBIO) AS TOTAL_CAMBIO_LEMPIRA FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_MONEDA = 3 AND F_ESTADO = 1";
				...
				$Query10 = "SELECT SUM(F_CAMBIO) AS TOTAL_CAMBIO_DOLARES FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$Fecha."' AND F_MONEDA = 2 AND F_ESTADO = 1";
				...
				$Query11 = "SELECT SUM(F_TOTAL) AS TOTAL_FACTURADO FROM Bodega.FACTURA_HS WHERE F_ESTADO = 1 AND F_FECHA_TRANS = '".$Fecha."'";
				...
				$QueryNotaEfectivo = mysqli_query($db, "SELECT A.TRA_TOTAL
													...
													INNER JOIN Bodega.FACTURA_HS AS B ON A.TRA_FACTURA_NOTA_CREDITO = B.F_CODIGO
								...
								<div id="accordion6-3" class="collapse" aria-expanded="false" style="height: 0px;">
									<div class="card-body">
										<table class="table" id="tbl_resultados">
											...
											<tbody>
												<?php
													$SqlFacturas = "SELECT * FROM Bodega.FACTURA_HS WHERE F_FECHA_TRANS = '".$_POST["FechaInicio"]."' ORDER BY F_SERIE, F_NUMERO";
		...
		<form method="POST" target="_blank" action="<?php echo $CierreDet?>" name="FormCorte">
			...
			<input type="hidden" name="TipoCorte" id="TipoCorte" value="Helados Sarita"> 
