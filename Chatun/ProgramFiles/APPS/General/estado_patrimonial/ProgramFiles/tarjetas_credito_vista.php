...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	if ($_GET["accion"] == 1) {
		...
		$insertar = "INSERT INTO Estado_Patrimonial.detalle_tarjetas_credito (colaborador, acreedor, vencimiento, monto_original, saldo_actual) VALUES ('$auxi', '$acreedor', '$vencimiento', '$monto_original', '$saldo_actual')";
		mysqli_query($db, $insertar) or die (mysqli_error());
	}
	if ($_GET["accion"] == 4) {
		$id = $_GET["id"];
		$eliminar = "DELETE FROM Estado_Patrimonial.detalle_tarjetas_credito WHERE id = '$id'";
	...
	if ($_GET["accion"] == 3) {
		...
		$saldo_actual_1 = $_POST["saldo_actual"];
		$monto_original = str_replace(",","",$monto_original_1);
		$saldo_actual = str_replace(",","",$saldo_actual_1);		$actualizar = "UPDATE Estado_Patrimonial.detalle_tarjetas_credito SET colaborador = '$auxi', acreedor = '$acreedor', vencimiento = '$vencimiento', monto_original = '$monto_original', saldo_actual = '$saldo_actual' WHERE id = '$id'";
...
<?php
		...
		$id = $_GET["id"];
		$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_tarjetas_credito WHERE colaborador = '$auxi' and id = '$id'";
