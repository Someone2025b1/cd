								...
								<?php
									...
									if($Hotel==$HotelV){
										echo '<div class="col-lg-12 text-center">
												...
												<h2 class="text-light">Los Hoteles no pueden ser iguales!</h2>
												
											'; 
									}else{
										$HotelA = mysqli_query($db, "SELECT *FROM Taquilla.ASIGNACION_TALONARIO_TICKET a WHERE ATT_ESTADO = 0 AND ATT_AL = '".$Al."'");
									    while($HotVi = mysqli_fetch_array($HotelA))
									    	{

											$HotelViejo=$HotVi["H_CODIGO"];

								    		}

										if($HotelViejo!=$HotelV){
											echo '<div class="col-lg-12 text-center">
												<h1><span class="text-xxxl text-light">ERROR <i class="fa fa-exclamation-circle text-danger"></i></span></h1>
												<h2 class="text-light">El Hotel Selecionado no posee esos tikets!</h2>
