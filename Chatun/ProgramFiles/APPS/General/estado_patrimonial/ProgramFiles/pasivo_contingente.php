...
function empezar() {
	document.form_bienes_inmuebles.fiador_de.focus();
}
function operar() {
...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	if ($_GET["accion"] == 1) {
		$fiador_de = $_POST["fiador_de"];
		$institucion = $_POST["institucion"];
		$monto = $_POST["monto"];
		$vencimiento = cambio_fecha_usa($_POST["vencimiento"]);
		$monto = str_replace(",","",$monto);
		$insertar = "INSERT INTO Estado_Patrimonial.detalle_pasivo_contingente (colaborador, fiador_de, institucion, monto, vencimiento) VALUES ('$auxi', '$fiador_de', '$institucion', '$monto', '$vencimiento')";
	mysqli_query($db, $insertar) or die (mysqli_error());
	}
	if ($_GET["accion"] == 4) {
		$id = $_GET["id"];
		$eliminar = "DELETE FROM Estado_Patrimonial.detalle_pasivo_contingente WHERE id = '$id'";
	...
	if ($_GET["accion"] == 3) {
		...
		$vencimiento = cambio_fecha_usa($_POST["vencimiento"]);
		$monto = str_replace(",","",$monto);
		$actualizar = "UPDATE Estado_Patrimonial.detalle_pasivo_contingente SET colaborador = '$auxi', fiador_de = '$fiador_de', institucion = '$institucion', monto = '$monto', vencimiento = '$vencimiento' WHERE id = '$id'";
...
<form action="pasivo_contingente.php?accion=1" method="post" name="form_bienes_inmuebles" id="form_bienes_inmuebles">
  <tr>
    <td colspan="2" align="center"><b>DETALLE DE PASIVO CONTINGENTE</b>
      <label for="fiador_de"></label></td>
    </tr>
  <tr>
    <td width="222">Fiador, Codeudor Avalista de:</td>
    <td><label for="fiador_de"></label>
      <input type="text" name="fiador_de" id="fiador_de"></td>
    </tr>
  <tr>
...
<?php
		...
		$id = $_GET["id"];
		$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_pasivo_contingente WHERE colaborador = '$auxi' and id = '$id'";
	$query_historial = mysqli_query($db, $sql_historial);
	while ($editar=mysqli_fetch_array($query_historial)) 	{ 	
?>
<table width="600" border="0" align="center" id="tabla_bienes_inmuebles" class="Tamaño12">
<form action="pasivo_contingente.php?accion=3&id=<?php echo $editar['id'] ?>" method="post" name="form_bienes_inmuebles_editar" id="form_bienes_inmuebles_editar">
  <tr>
    <td colspan="2" align="center"><b>DETALLE DE PASIVO CONTINGENTE</b></td>
    </tr>
  <tr>
    <td width="222">Fiador, Codeudor Avalista de:</td>
    <td><label for="fiador_de"></label>
      <input name="fiador_de" type="text" id="fiador_de" value="<?php echo $editar["fiador_de"] ?>"></td>
    </tr>
  <tr>
    <td>Instituci&oacute;n:</td>
...
<tr> 
	<td width="45%" bgcolor="#000000" class="LetraBlanca" align="center"><b>Fiador, Codeudor o Avalista de</b></td>
	<td width="35%" bgcolor="#000000" class="LetraBlanca" align="center"><b>Institucion</b></td>
  ...
  <?php
	$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_pasivo_contingente WHERE colaborador = '$auxi'";
	$query_historial = mysqli_query($db, $sql_historial);
	while ($historial=mysqli_fetch_array($query_historial)) 	{ 
?>
<tr> 
<td><? echo $historial ["fiador_de"] ?></font></td>
<td><? echo $historial["institucion"] ?></font></td>
<td><? echo poner_comas($historial["monto"]) ?></font></td>
<td><a href="pasivo_contingente.php?accion=2&id=<?php echo $historial['id']?>"><img src="../Imagenes/edit.png" alt="Editar..." width="16" height="16" border="0"></a><a href="pasivo_contingente.php?accion=4&id=<?php echo $historial['id']?>"><img src="../Imagenes/borrar.png" alt="Borrar..." width="16" height="16" border="0"></a></td>
<?php 
	}	//end while
?>
</tr>
