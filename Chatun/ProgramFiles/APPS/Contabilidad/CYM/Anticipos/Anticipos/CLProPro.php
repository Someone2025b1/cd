					...
					<div class="card">
						...
						<?php
							...
							if($Estado == 2)
							{
								...
								$sql = mysqli_query($db,"UPDATE Contabilidad.TRANSACCION SET TRANSACCION.E_CODIGO = 2, TRANSACCION.TRA_NO_HOJA = '".$Comprobante."', TRANSACCION.TRA_CORRELATIVO = ".$Correlativo.", TRANSACCION.PC_CODIGO = ".$Periodo." WHERE TRANSACCION.TRA_CODIGO = '".$CodigoTransaccion."'");
							}
							else
							{
							...
							if(!$sql) //Si el query tuvo algún problema que despligue error y que la ejecución del código se detenga
							{
								echo '<div class="col-lg-12 text-center">
										...
										<h2 class="text-light">Lo sentimos, no se pudo operar la liquidación.</h2>
							...
							else
							{
								...
								<h2 class="text-light">La liquidación se operó correctamente.</h2>
								<div class="row">
									<div class="col-lg-6 text-right"><a href="IngresoImp.php?Codigo='.$CodigoTransaccion.'" target="_blank"><button type="button" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-print"></span> Imprimir</a></div>
									<div class="col-lg-6 text-left"><a href="CL.php"><button type="button" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-chevron-left"></span> Regresar</a></div>
								</div>';
							}
						?>
						</div>
