...
<body class="menubar-hoverable header-fixed menubar-pin " onload="Imprimir()">
	...
	<div id="base">
		...
		<?php
			...
			$UidD      = uniqid("FD_");
			$Centinela = true;
			$Contador  = count($_POST["CantidadFE"]);
			...
			$Cantidad       = $_POST["CantidadFE"];
			$Producto       = $_POST["Producto"];
			$Precio         = $_POST["Precio"];
			$Descuento      = $_POST["Descuento"];
			$SubTotal       = $_POST["SubTotal"]; 
			$UnidVn    = $_POST["UnidadesVendidas"];
			...
			$sql = mysqli_query($db, "INSERT INTO Bodega.CLIENTE (CLI_NIT, CLI_NOMBRE, CLI_DIRECCION, CLI_EMAIL) VALUES ('".$NIT."', '".$Nombre."', '".$Direccion."', '".$Email."')ON DUPLICATE KEY UPDATE CLI_NOMBRE = '".$Nombre."', CLI_DIRECCION = '".$Direccion."', CLI_EMAIL = '".$Email."'");
			...
			if($TipoPago == 1)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_PS (F_CODIGO, F_TIPO, CLI_NIT, F_CON_DESCUENTO, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_SERIE)
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Tilapia Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 17, 9, '".$Uid."')") or die(mysqli_error($sqlAbono));
				...
				else
				{
					...
					for($i=0; $i<$Contador; $i++)
					{
						...
						$NomP = mysqli_fetch_array(mysqli_query($db, "SELECT a.P_NOMBRE  FROM Bodega.PRODUCTO a WHERE a.P_CODIGO = $Producto[$i]"));
						$Nom = $NomP['P_NOMBRE'];
						$Can = $Cantidad[$i];
						$Pro = $Producto[$i];
						$Pre = $Precio[$i];
						...
						$SubFac = ($Pre*$Can) - $Des;
						$UnidVn1 = $UnidVn[$i];
						$Estanque1 = $Estanque[$i];

						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_PS_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, RS_NOMBRE, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO, FD_UNIDADES, FD_ESTANQUE)
											VALUES('".$Uid."', '".$UidD."', ".$Can.", '".$Pro."', '".$Nom."', ".$Pre.", ".$SubFac.", ".$Des.", '".$UnidVn1."', '".$Estanque1."')"); 
						...
						$SubNeto = $Sub - $IVAProducto;
							...
							$XMLEnviar .= '
					          <dte:Item BienOServicio="B" NumeroLinea="'.$j.'">
					...
					****************************************************************************************/
					...
					$SqlContable = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_CONCEPTO, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_SERIE, TRA_FACTURA, TRA_TOTAL, RES_NUMERO, TRA_CORRELATIVO, PC_CODIGO)
											 VALUES('".$Uid."', CURRENT_DATE(), 'Vta. Tilapia Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 2, 32, '".$SerieAutorizada."', '".$FacturaNumero."', ".$TotalFacturaFinal.", '".$NumeroResolucion."', ".$Correlativo.", ".$Periodo.")") or die(mysqli_error($SqlContable));
					...
					$SqlContableD = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.01.011', ".number_format($TotalFacturaFinal, 2, ".", "").", 0.00)");
					...
					$SqlContableD2 = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '4.01.07.001', 0.00, ".number_format($VentaSinIVA, 2, ".", "").")");
				...
				if($response->body->resultado == 1)
				{
					...
					$uri = "https://certificador.feel.com.gt/fel/certificacion/v2/dte/";
					   ...
					   <?php 
						   ...
						   $QueryUpdateFactura = mysqli_query($db, "UPDATE Bodega.FACTURA_PS SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'");

						   $QueryUpdateTransaccionBodega = mysqli_query($db, "UPDATE Bodega.TRANSACCION SET TRA_OBSERVACIONES = 'Vta. Tilapia Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."' WHERE F_CODIGO = '".$Uid."'");

						   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Tilapia Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."', TRA_SERIE = '".$responseCerti->body->serie."', TRA_CAE = '".$responseCerti->body->numero."', TRA_DTE = '".$responseCerti->body->uuid."', TRA_FACTURA = '".$responseCerti->body->numero."' WHERE TRA_CODIGO = '".$Uid."'");
						}
						else
						{
					      	echo '<br><ul>';
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_PS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO, F_CON_DESCUENTO, F_TIPO_TARJETA)
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Tilapia Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 17, 9, '".$Uid."')");
				...
				else
				{
					...
					****************************************************************************************/
					...
					$SqlContable = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_CONCEPTO, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_SERIE, TRA_FACTURA, TRA_TOTAL, RES_NUMERO, TRA_CORRELATIVO, PC_CODIGO)
											 VALUES('".$Uid."', CURRENT_DATE(), 'Vta. Tilapia Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 2, 23, '".$SerieAutorizada."', '".$FacturaNumero."', ".$TotalFacturaFinal.", '".$NumeroResolucion."', ".$Correlativo.", ".$Periodo.")") or die(mysqli_error($SqlContable));
				...
				if($response->body->resultado == 1)
				{
					...
					if($responseCerti->body->cantidad_errores == 0)
					{
					   ...
					   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Tilapia Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
					}
					else
					{
				      	echo '<br><ul>';
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_PS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO, F_CON_DESCUENTO)
				...
				else
				{
					...
					****************************************************************************************/
					...
					$SqlContableD = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.03.011', ".number_format($TotalFacturaFinal, 2, ".", "").", 0.00)");
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_PS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA, F_CON_DESCUENTO)
