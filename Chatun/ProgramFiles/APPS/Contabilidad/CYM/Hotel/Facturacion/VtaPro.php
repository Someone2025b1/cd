...
<body class="menubar-hoverable header-fixed menubar-pin "  onload="Imprimir()">
	...
	<div id="base">
		...
		<?php
			...
			$Centinela = true; 
			$Contador  = mysqli_num_rows(mysqli_query($db, "SELECT * FROM Taquilla.DETALLE_FAC_HOTEL A WHERE A.CH_Id = $IdCorte"));
			$TotalDescuentoFactura = 0; 
			$FechaHoraHoy = date('Y-m-d', strtotime('now')).'T'.date('H:i:s', strtotime('now'));
			$CodigoEstablecimiento = 1;
			$CorreoEmisor = 'sjextkposadas@micoope.coosajo.com';
			$NITEmisor = '92066097';
			...
			if($TipoPago == 1)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HC (F_CODIGO, F_TIPO, CLI_NIT, F_CON_DESCUENTO, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_SERIE)
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Venta Boletos en Hoteles Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 16, 3, '".$Uid."')");
				...
				if(!$QueryEncabezado)
				{
					...
					echo mysqli_error($QueryEncabezado);
					$Centinela = false;
				...
				else
				{
					...
					$Det = mysqli_query($db, "SELECT SUM(C.IH_ADULTOS)AS Adulto, SUM(C.IH_TOTAL_ADULTO) AS TotalAdulto, C.IH_PRECIO_ADULTO
					FROM Taquilla.DETALLE_FAC_HOTEL A 
					INNER JOIN Taquilla.DETALLE_VALE_FACTURA B ON A.DC_Id = B.DC_Id
					INNER JOIN Taquilla.INGRESO_HOTEL C ON C.IH_VALE = B.DVF_Vale
					WHERE A.CH_Id = $IdCorte
					GROUP BY C.IH_PRECIO_ADULTO");
					$i = 1; 
					while($DetIngreso = mysqli_fetch_array($Det))
					{ 
						$Can = $DetIngreso["Adulto"];
						$Pro = "SERVICIO";
						$Sub = $DetIngreso["TotalAdulto"];
						$Des = 0;
						$Pre =  $DetIngreso["IH_PRECIO_ADULTO"]; 
						$Nom = "INGRESO ADULTOS"; 
						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HC_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, RS_NOMBRE, FD_PRECIO_UNITARIO, FD_SUBTOTAL)
											VALUES('".$Uid."', '".$UidD."', ".$Can.", '".$Pro."', '".$Nom."', ".$Pre.", ".$Sub.")");

						// $ConsultaProdReceta = "SELECT * FROM Bodega.RECETA_SUBRECETA_DETALLE WHERE RS_CODIGO = '".$Pro."'";
						// $ResultProdReceta = mysqli_query($ConsultaProdReceta);
					    ...
					    $Total = $MontoGravable + $MontoImpuesto; 
							...
							$XMLEnviar .= '
					          <dte:Item BienOServicio="S" NumeroLinea="'.$i.'">
					            ...
					            <dte:PrecioUnitario>'.number_format($Pre, 3, ".", "").'</dte:PrecioUnitario>
					            <dte:Precio>'.number_format($Sub, 3, ".", "").'</dte:Precio> 
					            <dte:Descuento>0</dte:Descuento><dte:Impuestos>
					              <dte:Impuesto>
					                <dte:NombreCorto>IVA</dte:NombreCorto>
					                <dte:CodigoUnidadGravable>1</dte:CodigoUnidadGravable>
					                <dte:MontoGravable>'.number_format($MontoGravable, 3, ".", "").'</dte:MontoGravable>
					                <dte:MontoImpuesto>'.number_format($MontoImpuesto, 3, ".", "").'</dte:MontoImpuesto>
					              </dte:Impuesto>
					            </dte:Impuestos>
					            <dte:Total>'.number_format($Total, 3, ".", "").'</dte:Total>
					           </dte:Item>';
					...
					$Det = mysqli_query($db, "SELECT SUM(C.IH_ADULTO_MAYOR)AS AdultoM, SUM(C.IH_TOTAL_ADULTO_MAYOR) AS TotalAdultoM, C.IH_PRECIO_ADULTO_MAYOR
					FROM Taquilla.DETALLE_FAC_HOTEL A 
					INNER JOIN Taquilla.DETALLE_VALE_FACTURA B ON A.DC_Id = B.DC_Id
					INNER JOIN Taquilla.INGRESO_HOTEL C ON C.IH_VALE = B.DVF_Vale
					WHERE A.CH_Id = $IdCorte
					GROUP BY C.IH_PRECIO_ADULTO_MAYOR");
					$i = 1; 
					while($DetIngreso = mysqli_fetch_array($Det))
					{ 
						$Can = $DetIngreso["AdultoM"];
						$Pro = "SERVICIO";
						$Sub = $DetIngreso["TotalAdultoM"];
						$Des = 0;
						$Pre =  $DetIngreso["IH_PRECIO_ADULTO_MAYOR"]; 
						$Nom = "INGRESO ADULTOS MAYORES"; 
						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HC_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, RS_NOMBRE, FD_PRECIO_UNITARIO, FD_SUBTOTAL)
											VALUES('".$Uid."', '".$UidD."', ".$Can.", '".$Pro."', '".$Nom."', ".$Pre.", ".$Sub.")");
					...
					########################## NIÑO
					$Det = mysqli_query($db, "SELECT SUM(C.IH_NINOS)AS Ninos, SUM(C.IH_TOTAL_NINO) AS TotalNinos, C.IH_PRECIO_NINO
					FROM Taquilla.DETALLE_FAC_HOTEL A 
					INNER JOIN Taquilla.DETALLE_VALE_FACTURA B ON A.DC_Id = B.DC_Id
					INNER JOIN Taquilla.INGRESO_HOTEL C ON C.IH_VALE = B.DVF_Vale
					WHERE A.CH_Id = $IdCorte
					GROUP BY C.IH_PRECIO_NINO");
					$i = 1;
					...
					while($DetIngreso = mysqli_fetch_array($Det))
					{
						...
						$Can = $DetIngreso["Ninos"];
						$Pro = "SERVICIO";
						$Sub = $DetIngreso["TotalNinos"];
						$Des = 0;
						$Pre =  $DetIngreso["IH_PRECIO_NINO"]; 
						$Nom = "INGRESO NIÑOS"; 
						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HC_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, RS_NOMBRE, FD_PRECIO_UNITARIO, FD_SUBTOTAL)
											VALUES('".$Uid."', '".$UidD."', ".$Can.", '".$Pro."', '".$Nom."', ".$Pre.", ".$Sub.")");
					...
					****************************************************************************************/
					...
					$SqlContable = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_CONCEPTO, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_SERIE, TRA_FACTURA, TRA_TOTAL, RES_NUMERO, TRA_CORRELATIVO, PC_CODIGO)
											 VALUES('".$Uid."', CURRENT_DATE(), 'Venta Boletos en Hoteles Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 2, 22, '".$SerieAutorizada."', '".$FacturaNumero."', ".$TotalFacturaFinal.", '".$NumeroResolucion."', ".$Correlativo.", ".$Periodo.")");
					...
					$SqlContableD = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.01.015', ".number_format($TotalFacturaFinal, 2, ".", "").", 0.00)");
					...
					$SqlContableD2 = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '4.01.01.002', 0.00, ".number_format($VentaSinIVA, 2, ".", "").")");
					...
					if($response->body->resultado == 1)
					{
						...
						{
							$Insert = mysqli_query($db, "UPDATE Taquilla.FAC_HOTEL SET CH_Estado = 2, CH_Total = $TotalFacturaFinal, CH_Factura = '".$responseCerti->body->numero."'  WHERE CH_Id = $IdCorte");
							$Detalle = mysqli_query($db, "SELECT A.DC_Id FROM Taquilla.DETALLE_FAC_HOTEL A WHERE A.CH_Id = $IdCorte");
						   ...
						   $QueryUpdateFactura = mysqli_query($db, "UPDATE Bodega.FACTURA_HC SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'");

						   $QueryUpdateTransaccionBodega = mysqli_query($db, "UPDATE Bodega.TRANSACCION SET TRA_OBSERVACIONES = 'Venta Boletos en Hoteles Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."' WHERE F_CODIGO = '".$Uid."'");

						   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Venta Boletos en Hoteles Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
						}
						else
						{
					      	echo '<br><ul>';
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HC (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO)
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HC (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO)
				...
				if($response->body->resultado == 1)
				{
					...
					if($responseCerti->body->cantidad_errores == 0)
					{
					   ...
					   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Venta Boletos en Hoteles Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
						...
						VALUES ('".$CCod."', '".$Uid."', 1, $NIT,  '".$id_user."', '".$TotalFacturaFinal."', 'HOTELES', 0.00)");
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_HC (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA)
