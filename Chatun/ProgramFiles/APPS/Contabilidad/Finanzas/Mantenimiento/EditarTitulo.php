...
<head>
	...
	function CambiarNombreTitulo(Id)
	{
		var Nombre = $("#Nombre").val();
		NOM = Nombre;
		$.ajax({
			url: 'Ajax/ModificarTitulo.php',
			type: 'POST',
			dataType: 'html',
			data: {Id: Id, NOM: NOM},
			success:function(data)
			{
				if (data==1) 
				{
					$('#NombreResultado').html('Nombre Actualizado Correctamente');
...
<body class="menubar-hoverable header-fixed menubar-pin ">
	...
	<div id="base">
		...
		<div id="content">
		<td><div style="height: 10px" id="ResultadoPartida" role="alert"><strong id="NombreResultado" style="background:green; font-size: 24px; margin: 12em; height: 4em; "></strong></div></td>

		<?php
        $NombreTitulo         = $_GET["Codigo"];
        
        $titulo = mysqli_query($db, "SELECT T_CODIGO, T_NOMBRE FROM Finanzas.TITULO
											WHERE TITULO.T_Codigo='$NombreTitulo'");
        while($row = mysqli_fetch_array($titulo))
        {
			...
			<label class="text-center">Nombre Titulo: <input class="form-control" type="text" name="Nombre" id="Nombre"  value="<?php echo $NomTitulo;?>" required></label>  
			</div>
			<div>
			<div class="col-lg-12 text-right"><a class="cam" onclick="CambiarNombreTitulo('<?php echo $CodTitulo?>')">Actualizar Nombre</a></div>
			</div>
			</div>
			</div>
