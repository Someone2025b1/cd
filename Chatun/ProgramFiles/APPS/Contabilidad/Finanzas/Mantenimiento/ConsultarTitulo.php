						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								<thead>
                                    ...
                                    <th><strong>CANTIDAD DE NOMENCLATURAS ASIGNADAS</strong></th>
								</thead>
                                
								<tbody>
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Finanzas.TITULO AS A ORDER BY A.T_NOMBRE ";
									...
									while($row = mysqli_fetch_array($Resultado))
									{
										$Codigo = $row["T_CODIGO"];
                                                ...
                                                WHERE a.T_CODIGO = $Codigo"));

										    	echo '<td style="font-size: 10px">'.$row["T_NOMBRE"].'</td>';
										    	echo '<td style="font-size: 10px">'.$row["T_CODIGO"].'</td>';
                                                echo '<td style="font-size: 10px">'.$Contador.'</td>';
										    	echo '<td><a href="EditarTitulo.php?Codigo='.$Codigo.'"><button type="button" class="btn btn-warning btn-xs">
