						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								<thead>
                                    ...
                                    <th colspan="2"><strong  align="center"><h6 align="center">Ación</h1></strong></th>
								</thead>
                                
								<tbody>
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Productos.INVENTARIO AS A WHERE A.I_ESTADO = 1 ORDER BY A.I_FECHA ";
									...
									while($row = mysqli_fetch_array($Resultado))
									{
										...
										$EN = $row['I_PUNTO'];
										    	...
										    	echo '<td><a href="IFProEditar.php?Codigo='.$Codigo.'&Punto='.$EN.'"><button type="button" class="btn btn-warning">
														     Ver Diferencias
														  </button></a>
														</td>';
									...
									$Consulta = "SELECT * FROM Productos.INVENTARIO_RECONTEO AS A WHERE A.I_ESTADO = 1 ORDER BY A.I_FECHA ";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
										...
										$EN = $row['I_PUNTO'];
										    	...
										    	echo '<td style="font-size: 10px">'.$row["I_OBSERVACIONES"].'</td>';
                                                echo '<td style="font-size: 10px">'.$row["I_FECHA"].' a las: '.$row["I_HORA"].' horas</td>';
										    	echo '<td><a href="IFProEditarReconteo.php?Codigo='.$Codigo.'&Punto='.$EN.'"><button type="button" class="btn btn-warning">
														     Ver Diferencias
														  </button></a>
														</td>';
