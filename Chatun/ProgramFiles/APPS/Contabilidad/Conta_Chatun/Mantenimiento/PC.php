	...
	<div id="base">
		...
		<div id="content">
			<div class="container">
				...
				<br>
				<div class="col-lg-12">
					<form action="PCAddPro.php" method="POST">
					<div class="card">
						...
						<div class="card-body">
							<table class="table table-hover table-condensed" id="tbl_resultados">
								...
								<tbody>
									<?php
									$Consulta = "SELECT PERIODO_CONTABLE.*, ESTADO_PERIODO_CONTABLE.EPC_NOMBRE 
												FROM Contabilidad.PERIODO_CONTABLE, Contabilidad.ESTADO_PERIODO_CONTABLE 
												WHERE PERIODO_CONTABLE.EPC_CODIGO = ESTADO_PERIODO_CONTABLE.EPC_CODIGO
												ORDER BY PERIODO_CONTABLE.PC_ANHO, PERIODO_CONTABLE.PC_MES";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{	
										$Estado = $row["EPC_CODIGO"];
										$Codigo = $row["PC_CODIGO"];

										echo '<tr>';
												echo '<td style="font-size: 12px">'.$row["PC_MES"].' - '.$row["PC_ANHO"].'</td>';               
												echo '<td style="font-size: 12px">'.$row["EPC_NOMBRE"].'</td>';
												echo '<td><a href="PCMod.php?Codigo='.$Codigo.'"><button type="button" class="btn btn-primary btn-xs">
												 Consultar
													</button></a></td>';
										echo '</tr>';
									}
