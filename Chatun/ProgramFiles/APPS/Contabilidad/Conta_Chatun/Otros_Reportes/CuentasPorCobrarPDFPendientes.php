...
while($FilaAnterior = mysqli_fetch_array($ResultAnterior))
{
    ...
    $MontoSaldo=$FilaAnterior["CARGOS"];
    $Cliente = $FilaAnterior["CLIENTE"];

    if ($MontoSaldo>2700){
        $retencion=number_format(((($MontoSaldo/1.12)*0.12)*0.15), 2, '.', ',');
        $MontoSaldo=$MontoSaldo-$retencion;
    }else{
        $MontoSaldo=$MontoSaldo;
    }
    

    $QueryAnterior1 = "SELECT TRANSACCION_DETALLE.TRAD_ABONO_CONTA
                    FROM Contabilidad.TRANSACCION_DETALLE, Contabilidad.TRANSACCION
                    WHERE TRANSACCION_DETALLE.TRA_CODIGO = TRANSACCION.TRA_CODIGO
                    AND (TRANSACCION.TRA_FECHA_TRANS BETWEEN '".$FechaIni."' AND '".$FechaFin."')
                    AND TRANSACCION_DETALLE.N_CODIGO = '".$Nombre."'
                    AND TRANSACCION.TRA_CODIGO LIKE '%tra_%'
                    AND TRANSACCION.E_CODIGO = 2 AND TRANSACCION.TRA_ESTADO = 1
                    AND TRANSACCION_DETALLE.TRAD_ABONO_CONTA = $MontoSaldo";
                    $ResultAnterior1 = mysqli_query($db, $QueryAnterior1);
                    while($FilaAnterior1 = mysqli_fetch_array($ResultAnterior1))
                    {
                        $Pagada=$FilaAnterior1["TRAD_ABONO_CONTA"];
                    }
