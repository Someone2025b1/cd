...
$Cod = mysqli_fetch_array(mysqli_query($db, "SELECT a.ACC_CODIGO  FROM Bodega.APERTURA_CIERRE_CAJA a WHERE a.ACC_FECHA = CURDATE() AND a.ACC_TIPO = 2"));
$Insert = mysqli_query($db, "INSERT INTO Bodega.CUADRE_CAJA (Usuario, Fecha, Tipo, ACC_CODIGO, Hora, EfectivoQ, EfectivoD, EfectivoL, EfectivoNota, EfectivoDeb, EfectivoTotal, Credito, CreditoNota, CreditoDeb, CreditoTotal, Tarjeta, TarjetaNota, TarjetaDebito, TarjetaTotal, Deposito, DepositoNota, DepositoDeb, DepositoTotal, TotalDatos, TotalNotas, TotalNotasC, TotalIngresos, TotalFacturado, TotalNotasEmitidas, Sobrante, Faltante) 
    VALUES ('$Usuario', CURRENT_TIME, 2, '".$Cod["ACC_CODIGO"]."', CURRENT_TIMESTAMP, '".$_POST["EfectivoCorte"]."', '".$_POST["DolaresCorte"]."', '".$_POST["LempirasCorte"]."', '".$_POST["NotaCreditoEfectivo"]."', '".$_POST["NotaDebitoEfectivo"]."', '".$EfectivoQuetzalizadoT."', '".$_POST["CreditoCorte"]."', '".$_POST["NotaCreditoCredito"]."', '".$_POST["NotaDebitoCredito"]."', '".$_POST["TotalCredito"]."', '".$TCCorte."', '".$_POST["NotaCreditoTarjeta"]."', '".$_POST["NotaDebitoTarjeta"]."', '".$_POST["TotalTarjetaCredito"]."', '".$_POST["DepositosCorte"]."', '".$_POST["NotaDebitoDeposito"]."', '".$_POST["NotaCreditoDeposito"]."', '".$_POST["TotalDepositos"]."', '".$TotalDatosPreliminares1."', '".$_POST["TotalNotasDebito"]."', '".$_POST["TotalNotasCredito"]."',  '".$TotalIngresosT."', '".$_POST["FacturadoCorte"]."', '".$TotalNotasEmitidasTotalT."', '$Sobra', '$Falta')");//****************** CUSTOMIZACION **************************
//***********************************************************
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0,0, "CAJA SOUVENIRS" ,0,1,'C');
    }
}
//***********************************************************
//***********************************************************
