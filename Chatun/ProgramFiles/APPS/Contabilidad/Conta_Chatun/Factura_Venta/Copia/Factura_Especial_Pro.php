					...
					$url_servicio_firma = "https://signer-emisores.feel.com.gt/sign_solicitud_firmas/firma_xml";
								...
								if($response->body->resultado == 1)
								{
									...
									if($responseCerti->body->cantidad_errores == 0)
									{
										...
										$sql = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_NO_HOJA, TRA_FECHA_TRANS, TRA_SERIE, TRA_FACTURA, TRA_CONCEPTO, TRA_TOTAL, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_CORRELATIVO, TRA_SALDO, TRA_TIPO_FACTURA_VENTA, PC_CODIGO, TRA_DTE, TRA_CAE, TRA_FECHA_CERTIFICACION, P_CODIGO, TRA_ISR, TRA_IVA)
														VALUES('".$UI."', '".$Comprobante."', '".$FechaFactura."', '".$responseCerti->body->serie."', '".$responseCerti->body->numero."', '".$Descripcion."', ".$TotalFactura.", CURRENT_DATE(), CURRENT_TIMESTAMP(), '".$id_user."', 2, 19, ".$Correlativo.", ".$TotalFactura.", 'FES', ".$Periodo.", '".$responseCerti->body->uuid."', '".$responseCerti->body->numero."', '".$responseCerti->body->fecha."', '".$CodigoProveedor."', $ISR, $IVA)");
