							...
							<div >
								<?php
										...
										if($Ar==1){
											...
											$QueryUsuC = "SELECT * FROM RRHH.COLABORADOR WHERE C_USUARIO = ".$id_user; 
											$ResultUsuC = mysqli_query($db, $QueryUsuC);
											while ($rowUsuC = mysqli_fetch_array($ResultUsuC)) {
												
												$CodigoCOlaboradoNew = $rowUsuC["C_CODIGO"];
											}

											$Query = "SELECT 
												A.R_CODIGO, A.R_FECHA, A.R_HORA, A.U_CODIGO, B.*
											FROM 
												CompraVenta.REQUISICION AS A
											JOIN 
												CompraVenta.REQUISICION_DETALLE AS B ON A.R_CODIGO = B.R_CODIGO
											JOIN 
												RRHH.COLABORADOR AS C ON C.C_USUARIO = A.U_CODIGO
											WHERE 
												B.RD_ESTADO NOT IN (10, 11)
												AND (C.C_JEFE = '$CodigoCOlaboradoNew' OR A.U_CODIGO = $id_user)
											ORDER BY 
												B.R_CODIGO";
