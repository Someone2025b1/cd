...
<head>
	...
	<script>
		function Imprimir()
		{
			...
			printJS('<?php $_SERVER['DOCUMENT_ROOT']?>/ProgramFiles/APPS/CYM/Juegos/Facturacion/<?php echo $Imprimir?>'+Cod);
...
<body class="menubar-hoverable header-fixed menubar-pin " onload="Imprimir()">
	...
	<div id="base">
		...
		<?php
			...
			$TotalFacturaConDescuento = $_POST["TotalDescuentoFactura"];
			...
			if($TipoPago == 1)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_JG (F_CODIGO, F_TIPO, CLI_NIT, F_CON_DESCUENTO, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_SERIE)
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Juegos Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 18, 2, '".$Uid."')") or die(mysqli_error($sqlAbono));
				...
				else
				{
					...
					for($i=0; $i<$Contador; $i++)
					{
						...
						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_JG_DETALLE (F_CODIGO, FD_CODIGO, FD_TIPO, FD_CANTIDAD, RS_CODIGO, RS_NOMBRE, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO)
											VALUES('".$Uid."', '".$UidD."', ".$Tip.", ".$Can.", '".$Pro."', '".$Nom."', ".$Pre.", ".$SubFac.", ".$Des.")");
						...
						$SubNeto = $Sub - $IVAProducto;
							...
							$XMLEnviar .= '
					          <dte:Item BienOServicio="B" NumeroLinea="'.$i.'">
					            <dte:Cantidad>'.number_format($Can,2).'</dte:Cantidad>
					            ...
					            <dte:Descuento>'.number_format($Des,2).'</dte:Descuento>
					            <dte:Impuestos>
					              <dte:Impuesto>
							...
							if ($Tip==1) 
							{
								...
								$Cuenta = mysqli_fetch_array(mysqli_query($db, "SELECT a.N_CODIGO FROM Bodega.PRODUCTO a WHERE a.P_CODIGO = $Pro"));
								...
								$SqlContableD2 = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '$CuentaContable', 0.00, ".number_format($SubNeto, 2, ".", "").")");
							} 
							elseif ($Tip==2) 
							{
					...
					****************************************************************************************/
					...
					$SqlContable = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_CONCEPTO, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_SERIE, TRA_FACTURA, TRA_TOTAL, RES_NUMERO, TRA_CORRELATIVO, PC_CODIGO)
											 VALUES('".$Uid."', CURRENT_DATE(), 'Vta. Juegos Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 2, 26, '".$SerieAutorizada."', '".$FacturaNumero."', ".$TotalFacturaFinal.", '".$NumeroResolucion."', ".$Correlativo.", ".$Periodo.")") or die(mysqli_error($SqlContable));
					...
					$SqlContableD = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.01.005', ".number_format($TotalFacturaFinal, 2, ".", "").", 0.00)");
				...
				if($response->body->resultado == 1)
				{
					...
					$uri = "https://certificador.feel.com.gt/fel/certificacion/v2/dte/";
						...
						if($responseCerti->body->cantidad_errores == 0)
						{
						   ...
						   $QueryUpdateFactura = mysqli_query($db, "UPDATE Bodega.FACTURA_JG SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'");

						   $QueryUpdateTransaccionBodega = mysqli_query($db, "UPDATE Bodega.TRANSACCION SET TRA_OBSERVACIONES = 'Vta. Juegos Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."' WHERE F_CODIGO = '".$Uid."'");

						   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Juegos Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
						}
						else
						{
					      	echo '<br><ul>';
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_JG (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO, F_CON_DESCUENTO, F_TIPO_TARJETA)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 2, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.", '".$Observaciones."', '".$NumeroAutorizacionTXT."', '".$NumeroResolucion."', '".$TotalFacturaConDescuento."', '".$TipoTarjeta."')");

				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Juegos Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 18, 2, '".$Uid."')");
					...
					for($i=0; $i<$Contador; $i++)
					{
						...
						$SubNeto = $Sub - $IVAProducto;
							...
							elseif ($Tip==3) 
							{
								...
								$QueryCombo = mysqli_fetch_array(mysqli_query($db, "SELECT b.P_CODIGO, a.Precio FROM Bodega.ESCALA_PRODUCTO a
							        INNER JOIN Bodega.PRODUCTO  b ON a.P_CODIGO = b.P_CODIGO
							        WHERE IdEscala = $Pro"));
							    $Cantidad = $QueryCombo["Cantidad"]; 
							    for ($i=0; $i < $Cantidad; $i++)   
							    { 
							    	$IVAProducto = ($QueryCombo['Precio']/ 1.12)*0.12;
									$SubNeto = $QueryCombo['Precio'] - $IVAProducto;
								    $Cuenta = mysqli_fetch_array(mysqli_query($db, "SELECT a.N_CODIGO FROM Bodega.PRODUCTO a WHERE a.P_CODIGO = $QueryCombo[P_CODIGO]"));
									$CuentaContable = $Cuenta["N_CODIGO"];  
									$SqlContableD2 = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '$CuentaContable', 0.00, ".number_format($SubNeto, 3, ".", "").")");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_JG (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO, F_CON_DESCUENTO)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 3, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.",  '".$NombreCredito."', '".$Observaciones."', '".$NumeroResolucion."', '".$TotalFacturaConDescuento."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   VALUES ('".$CCod."', '".$Uid."', 1, $NIT,  '".$id_user."', '".$TotalFacturaFinal."', 'JUEGOS', 0.00)");
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_JG (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA, F_CON_DESCUENTO)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 4, '".$NIT."', ".$TotalFacturaFinal.", 0.00, 0.00, CURRENT_DATE(), CURRENT_TIMESTAMP(),  ".$id_user.",  '".$NumeroBoleta."', '".$Observaciones."', '".$NumeroResolucion."', '".$CuentaBancaria."', '".$TotalFacturaConDescuento."')");
