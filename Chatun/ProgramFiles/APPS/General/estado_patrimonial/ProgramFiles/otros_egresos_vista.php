...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	if ($_GET["accion"] == 1) {
		...
		$insertar = "INSERT INTO Estado_Patrimonial.detalle_otros_egresos (colaborador, descripcion, monto) VALUES ('$auxi', '$descripcion', '$monto')";
	mysqli_query($db, $insertar) or die (mysqli_error());
	}
	if ($_GET["accion"] == 4) {
		$id = $_GET["id"];
		$eliminar = "DELETE FROM Estado_Patrimonial.detalle_otros_egresos WHERE id = '$id'";
	...
	if ($_GET["accion"] == 3) {
		...
		$monto_1 = $_POST["monto"];
		$monto = str_replace(",","",$monto_1);
		$actualizar = "UPDATE Estado_Patrimonial.detalle_otros_egresos SET colaborador = '$auxi', descripcion = '$descripcion', monto = '$monto' WHERE id = '$id'";
