...
<head>
	...
	<script>
		...
		function Imprimir()
		{
			...
			printJS('<?php $_SERVER['DOCUMENT_ROOT']?>/ProgramFiles/APPS/CYM/Pizzeria/Facturacion/FactImpNew.php?Codigo='+Cod);
...
<body class="menubar-hoverable header-fixed menubar-pin ">
	...
	<div id="base">
		...
		<?php
			...
			if($TipoPago == 1)
			{
				$ConsultaOrden = "SELECT F_ORDEN FROM Bodega.FACTURA_PIZZA WHERE F_FECHA_TRANS = CURRENT_DATE()
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_PIZZA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_ORDEN, F_CON_DESCUENTO)
				...
				else
				{
					for($i=0; $i<$Contador; $i++)
					{
						...
						$query = mysqli_query($db,"INSERT INTO Bodega.FACTURA_PIZZA_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO)
					...
					****************************************************************************************/
					...
					$SqlContableD = mysqli_query($db,"INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.01.018', ".number_format($TotalFacturaFinal, 2, ".", "").", 0.00)");
					...
					$SqlContableD2 = mysqli_query($db,"INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '4.01.03.011', 0.00, ".number_format($VentaSinIVA, 3, ".", "").")");
				}
					...
					for($i=1; $i<$Contador; $i++)
				{
						...
						if($rowe["R_CODIGO"]!=NULL){
							...
							while($rower = mysqli_fetch_array($resultr))
							{
								...
								while($roweR = mysqli_fetch_array($resultR))
								{
									...
									$existenciaAnR=$roweR["P_EXISTENCIA_PIZZERIA"];
									$existenciaGAnR=$roweR["P_EXISTENCIA_GENERAL"];

									$cantidadgenR = $roweR["P_EXISTENCIA_GENERAL"] - $CanR;

									$nuevaexistenciaR = $roweR["P_EXISTENCIA_PIZZERIA"] - $CanR;

								

								$QueryPU = mysqli_query($db, "UPDATE Productos.PRODUCTO SET
								P_EXISTENCIA_PIZZERIA = '".$nuevaexistenciaR."',
								...
								VALUES('".$KCod."', '".$Uid."', '".$ProdR."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Pizzería Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 8, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
					...
					$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
					VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Pizzería Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAn."', '".$cantidadgen."', 8, '".$existenciaAn."', '".$nuevaexistencia."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   $QueryUpdateFactura = mysqli_query($db,"UPDATE Bodega.FACTURA_PIZZA SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'")or die(mysqli_error());

					   $QueryUpdateTransaccionBodega = mysqli_query($db,"UPDATE Bodega.TRANSACCION SET TRA_OBSERVACIONES = 'Vta. Pizzería Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."' WHERE F_CODIGO = '".$Uid."'");

					   $QueryUpdateTransaccionContabilidad = mysqli_query($db,"UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Pizzería Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
					}
					else
					{
							$QueryBitacora = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA 
						SELECT FACTURA.F_CODIGO, '1', FACTURA.F_ORDEN, FACTURA.F_SERIE, FACTURA.F_NUMERO, FACTURA.F_TIPO, FACTURA.CLI_NIT, FACTURA.D_CODIGO, FACTURA.F_CON_DESCUENTO, FACTURA.F_TOTAL, FACTURA.F_EFECTIVO, FACTURA.F_CAMBIO, FACTURA.F_FECHA_TRANS, FACTURA.F_HORA, FACTURA.F_USUARIO, FACTURA.F_MONEDA, FACTURA.F_ESTADO, FACTURA.F_RAZON_ANULACION, FACTURA.F_NOMBRE_CREDITO, FACTURA.F_NO_BOLETA, FACTURA.F_OBSERVACIONES, FACTURA.F_NO_AUTORIZACION, FACTURA.RES_NUMERO, FACTURA.F_CUENTA_BANCARIA, FACTURA.F_REALIZADA, FACTURA.F_CAE, FACTURA.F_DTE, FACTURA.F_FECHA_CERTIFICACION, FACTURA.F_TIPO_TARJETA FROM Bodega.FACTURA_PIZZA 
						WHERE  FACTURA.F_CODIGO = '".$Uid."'")or die(mysqli_error());

							$QueryBitacora1 = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA_DETALLE
							SELECT * FROM Bodega.FACTURA_PIZZA_DETALLE
							WHERE FACTURA_PIZZA_DETALLE.F_CODIGO = '".$Uid."'")or die(mysqli_error());
					 		
							$QueryUnoRestaurante = mysqli_query($db, "DELETE 
																FROM Bodega.FACTURA_PIZZA 
																WHERE FACTURA_PIZZA.F_CODIGO = '".$Uid."'")or die(mysqli_error());
				...
				else
				{
					$QueryBitacora = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA 
						...
						WHERE  FACTURA.F_CODIGO = '".$Uid."'")or die(mysqli_error());
							...
							$QueryBitacora1 = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA_PIZZA_DETALLE
							SELECT * FROM Bodega.FACTURA_PIZZA_DETALLE
							WHERE FACTURA_PIZZA_DETALLE.F_CODIGO = '".$Uid."'")or die(mysqli_error());
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_PIZZA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO, F_ORDEN, F_TIPO_TARJETA, F_CON_DESCUENTO)
...
if($EventoPertenece!="SINEVENTO"){
	...
	$QueryEvento= mysqli_query($db, "UPDATE Eventos.EVENTO SET
					   ...
					   $QueryUpdateFactura = mysqli_query($db,"UPDATE Bodega.FACTURA_PIZZA SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'");
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_PIZZA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO, F_ORDEN, F_CON_DESCUENTO)
...
if($EventoPertenece!="SINEVENTO"){
	...
	$QueryEvento= mysqli_query($db, "UPDATE Eventos.EVENTO SET
					   ...
					   $QueryUpdateTransaccionContabilidad = mysqli_query($db,"UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Pizzería Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
						...
						VALUES ('".$CCod."', '".$Uid."', 1, $NIT,  '".$id_user."', '".$TotalFacturaFinal."', 'PIZZERÍA', 0.00)");
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA_PIZZA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA, F_ORDEN, F_CON_DESCUENTO)
