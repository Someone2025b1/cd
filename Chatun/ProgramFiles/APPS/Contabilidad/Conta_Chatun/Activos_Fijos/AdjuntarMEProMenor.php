...
<body class="menubar-hoverable header-fixed menubar-pin ">
	...
	$query = "SELECT TRA_TOTAL, TRA_SALDO, TRA_FECHA_TRANS FROM Contabilidad.TRANSACCION WHERE TRA_CODIGO = '".$_GET["CodigoPoliza"]."'";
	$result = mysqli_query($db,$query);
	while($row = mysqli_fetch_array($result))
	{	
		$Total = $row["TRA_TOTAL"];
		$Saldo = $row["TRA_SALDO"];
		$FechaCompra = $row["TRA_FECHA_TRANS"];
	}

	$Anho = date('Y', strtotime($FechaCompra));
	$Anho2 = date('y', strtotime($FechaCompra));

	$query = "SELECT * FROM Contabilidad.ACTIVO_FIJO_MENOR WHERE YEAR(AF_FECHA_ADQUISICION) = ".$Anho." 
	ORDER BY AF_CODIGO DESC
	LIMIT 1";
	$result = mysqli_query($db,$query);
	while($row = mysqli_fetch_array($result))
	{	
		$CodigoAnt = $row["AF_CODIGO"];
	}
if($CodigoAnt==NULL){

	$CodigoNuevo="CHATM-".$Anho2."-01";
}else{

	$correlativoante = explode("CHATM-$Anho2-", $CodigoAnt);
	$correlatinuev=$correlativoante[1]+1;
	$CodigoNuevo="CHATM-".$Anho2."-".sprintf("%04d",$correlatinuev);
	...
	<div id="base">
		...
		<div id="content">
			<div class="container">
				<form class="form" action="AdjuntarMEProProMenor.php" method="POST" role="form">
					<div class="col-lg-12">
						<br>
						<div class="card">
							...
							<div class="card-body">
								...
								<div class="row">
									<div class="col-lg-3">
										<div class="form-group">
											<input class="form-control" type="text" name="CodigoActivo" id="CodigoActivo"  value="<?php echo $CodigoNuevo ?>" required readonly/>
						...
						<div class="card">
							...
							<div class="card-body">
								<table class="table table-hover table-condensed">
									...
									<tbody>
										<?php
											$query = "SELECT ACTIVO_FIJO_MENOR.*, AREA_GASTO.AG_NOMBRE, TIPO_ACTIVO.TA_NOMBRE
														FROM Contabilidad.ACTIVO_FIJO_MENOR, Contabilidad.AREA_GASTO, Contabilidad.TIPO_ACTIVO 
														WHERE ACTIVO_FIJO_MENOR.AF_AREA = AREA_GASTO.AG_CODIGO
														AND ACTIVO_FIJO_MENOR.TA_CODIGO = TIPO_ACTIVO.TA_CODIGO
														AND ACTIVO_FIJO_MENOR.AF_ESTADO = 1
											...
											while($row = mysqli_fetch_array($result))
											{
												...
												?>
													<tr>
														...
														<td>
															<h6><a href="EliminarActivoMenor.php?CodigoActivo=<?php echo $row["AF_CODIGO"] ?>&Valor=<?php echo $row["AF_VALOR"] ?>&Saldo=<?php echo $Saldo ?>&Transaccion=<?php echo $_GET["CodigoPoliza"] ?>"><button type="button" class="btn btn-danger btn-xs">
