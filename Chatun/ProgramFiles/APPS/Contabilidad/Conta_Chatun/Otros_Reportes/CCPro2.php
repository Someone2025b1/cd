	...
	<div id="base">
		...
		<div id="content">
			<?php
				...
				$QueryMinMaxFacturas = "SELECT MAX(F_NUMERO) AS MAXIMO, MIN(F_NUMERO) AS MINIMO, F_SERIE FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."'";
				...
				$Query = "SELECT COUNT(F_CODIGO) AS FACTURAS_EMITIDAS FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."'";
				...
				$Query1 = "SELECT COUNT(F_CODIGO) AS FACTURAS_ANULADAS FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_ESTADO = 2";
				$Result1 = mysqli_query($db, $Query1);
				while($row1 = mysqli_fetch_array($Result1))
				{
					$FacturasAnula = $row1["FACTURAS_ANULADAS"];
				}


				$Query3 = "SELECT SUM(F_TOTAL) AS TOTAL_CREDITO FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 2 AND F_ESTADO = 1";
				...
				$Query4 = "SELECT SUM(F_TOTAL) AS TOTAL_DOLARES FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 1 AND F_MONEDA = 2 AND F_ESTADO = 1";
				...
				$Query4 = "SELECT SUM(F_TOTAL) AS TOTAL_LEMPIRAS FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 1 AND F_MONEDA = 3 AND F_ESTADO = 1";
				$Result4 = mysqli_query($db, $Query4);
				while($row4 = mysqli_fetch_array($Result4))
				{
				...
				$Query5 = "SELECT SUM(F_TOTAL) AS TOTAL_DEPOSITOS FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 4 AND F_ESTADO = 1";
				...
				$Query6 = "SELECT SUM(F_TOTAL) AS TOTAL_CREDITOS FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_TIPO = 3 AND F_ESTADO = 1";
				...
				$Query7 = "SELECT SUM(F_TOTAL) AS TOTAL_EFECTIVO FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_MONEDA = 1 AND F_ESTADO = 1";
				...
				$Query9 = "SELECT SUM(F_CAMBIO) AS TOTAL_CAMBIO_LEMPIRA FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_MONEDA = 3 AND F_ESTADO = 1";
				...
				$Query10 = "SELECT SUM(F_CAMBIO) AS TOTAL_CAMBIO_DOLARES FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."' AND F_MONEDA = 2 AND F_ESTADO = 1";
				...
				$Query11 = "SELECT SUM(F_TOTAL) AS TOTAL_FACTURADO FROM Bodega.FACTURA_2 WHERE F_ESTADO = 1 AND F_FECHA_TRANS = '".$Fecha."'";
				...
				$QueryNotaEfectivo = mysqli_query($db, "SELECT A.TRA_TOTAL
													...
													INNER JOIN Bodega.FACTURA_2 AS B ON A.TRA_FACTURA_NOTA_CREDITO = B.F_CODIGO
				...
				$FaltaSobra = $TCQ + $TotalCreditos + $TotalTarjetaCredito + $TotalDolaresQuetzalisados + $TotalLempirasQuetzalisados + $TotalDeposito - $TotalFacturado;
				
				
				$QueryDocumentosEmitidos = mysqli_query($db, "SELECT F_CODIGO FROM Bodega.FACTURA_2 WHERE F_FECHA_TRANS = '".$Fecha."'");
				...
				$FilaNotasC = $FilaNotaEfectivo[TRA_TOTAL] + $FilaNotaCredito[TRA_TOTAL] + $FilaNotaTarjetaCredito[TRA_TOTAL] + $FilaNotaDepositos[TRA_TOTAL] + $FilaNotaDolares[TRA_TOTAL] + $FilaNotaLempiras[TRA_TOTAL];
												...
												<table class="table table-hover table-condensed">
													<tbody>
														...
														<tr>
															...
															<td colspan="4" align="right"><h5><?php if($FaltaSobra < 0){echo 'Q. '.number_format($FaltaSobra, 2, '.', ',');}elseif($FaltaSobra == 0){echo 'Q. '.number_format(0, 2, '.', ',');}else{echo 'Q. '.number_format(0, 2, '.', ',');} ?></h5></td>
														</tr>
														<tr>
											...
											<tbody>
												<?php
													$SqlFacturas = "SELECT FACTURA_2.* FROM Bodega.FACTURA_2
																	WHERE FACTURA_2.F_FECHA_TRANS = '".$_POST["FechaInicio"]."' 
		...
		<form method="POST" target="_blank" action="<?php echo $CierreDet?>" name="FormCorte">
			...
			<input type="hidden" name="TipoCorte" id="TipoCorte" value="Restaurante Terrazas #2">
			...
			<input type="hidden" name="FacturasAnul" id="FacturasAnul" value="<?php $FacturasAnula ?>">
			...
			<input type="hidden" name="Falta" id="Falta" value="<?php echo $Falta ?>">
			<input type="hidden" name="Sobra" id="Sobra" value="<?php echo $Sobra ?>">
			...
			<input type="hidden" name="FilaNotasC" id="FilaNotasC" value="<?php echo $FilaNotasC ?>">
