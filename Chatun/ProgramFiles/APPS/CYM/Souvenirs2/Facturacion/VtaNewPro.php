...
<head>
	...
	<script>
		...
		function Imprimir()
		{
			...
			printJS('<?php $_SERVER['DOCUMENT_ROOT']?>/ProgramFiles/APPS/CYM/Souvenirs2/Facturacion/<?php echo $Imprimir?>'+Cod);
	...
	<div id="base">
		...
		<?php
			...
			if($TipoPago == 1)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_SV_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_CON_DESCUENTO)
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Souvenirs No.2 Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 10, 2, '".$Uid."')");
				...
				else
				{
					for($i=1; $i<$Contador; $i++)
					{
						...
						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_SV_2_DETALLE (F_CODIGO, FD_CODIGO, FD_TIPO,  FD_CANTIDAD, P_CODIGO, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO)
...
#######################################################################
				...
				if($response->body->resultado == 1)
				{
						...
						for($i=1; $i<$Contador; $i++)
					{
						...
						VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Souvenir Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAn."', '".$cantidadgen."', 2, '".$existenciaAn."', '".$nuevaexistencia."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   $QueryUpdateFactura = mysqli_query($db, "UPDATE Bodega.FACTURA_SV_2 SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'");

					   $QueryUpdateTransaccionBodega = mysqli_query($db, "UPDATE Bodega.TRANSACCION SET TRA_OBSERVACIONES = 'Vta. Souvenirs No.2 Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."' WHERE F_CODIGO = '".$Uid."'");

					   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Souvenirs No.2 Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
					}
					else
					{
						$QueryBitacora = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA 
						SELECT FACTURA_SV_2.F_CODIGO, '2', FACTURA_SV_2.F_ORDEN, FACTURA_SV_2.F_SERIE, FACTURA_SV_2.F_NUMERO, FACTURA_SV_2.F_TIPO, FACTURA_SV_2.CLI_NIT, FACTURA_SV_2.D_CODIGO, FACTURA_SV_2.F_CON_DESCUENTO, FACTURA_SV_2.F_TOTAL, FACTURA_SV_2.F_EFECTIVO, FACTURA_SV_2.F_CAMBIO, FACTURA_SV_2.F_FECHA_TRANS, FACTURA_SV_2.F_HORA, FACTURA_SV_2.F_USUARIO, FACTURA_SV_2.F_MONEDA, FACTURA_SV_2.F_ESTADO, FACTURA_SV_2.F_RAZON_ANULACION, FACTURA_SV_2.F_NOMBRE_CREDITO, FACTURA_SV_2.F_NO_BOLETA, FACTURA_SV_2.F_OBSERVACIONES, FACTURA_SV_2.F_NO_AUTORIZACION, FACTURA_SV_2.RES_NUMERO, FACTURA_SV_2.F_CUENTA_BANCARIA, FACTURA_SV_2.F_REALIZADA, FACTURA_SV_2.F_CAE, FACTURA_SV_2.F_DTE, FACTURA_SV_2.F_FECHA_CERTIFICACION, FACTURA_SV_2.F_TIPO_TARJETA FROM Bodega.FACTURA_SV_2 
						WHERE  FACTURA_SV_2.F_CODIGO = '".$Uid."'")or die(mysqli_error());

							$QueryBitacora1 = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA_DETALLE
							SELECT * FROM Bodega.FACTURA_SV_2_DETALLE
							WHERE FACTURA_SV_2_DETALLE.F_CODIGO = '".$Uid."'")or die(mysqli_error());
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_SV_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO, F_CON_DESCUENTO, F_TIPO_TARJETA)
...
#######################################################################
				...
				if($response->body->resultado == 1)
				{
						...
						for($i=1; $i<$Contador; $i++)
					{
						...
						$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
						VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Vta. Souvenirs No.2 Según Fact. ".$SerieAutorizada." ".$FacturaNumero."', '".$existenciaGAn."', '".$cantidadgen."', 2, '".$existenciaAn."', '".$nuevaexistencia."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_SV_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO, F_CON_DESCUENTO)
...
#######################################################################
				...
				if($response->body->resultado == 1)
				{
						...
						for($i=1; $i<$Contador; $i++)
					{
						...
						$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
						VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Souvenir ".$SerieAutorizada." ".$FacturaNumero."', '".$existenciaGAn."', '".$cantidadgen."', 2, '".$existenciaAn."', '".$nuevaexistencia."')");

						}
					}
######################## FIN KARDEX ####################
#$ConsultaOrden = "SELECT F_ORDEN FROM Bodega.BOLETOS
				#ORDER BY F_ORDEN DESC LIMIT 1";
				...
				#else
				#{
				#	$filaO = mysqli_fetch_array($ResultOrden);

				#	$NumeroOrden = $filaO["B_NUMERO"] + 1;
				#}
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   VALUES ('".$CCod."', '".$Uid."', 1, $NIT,  '".$id_user."', '".$TotalFacturaFinal."', 'Terrazas', 0.00)");
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_SV_2 (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA, F_CON_DESCUENTO)
