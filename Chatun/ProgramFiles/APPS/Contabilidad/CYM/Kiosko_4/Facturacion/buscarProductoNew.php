<?php
    ...
    include("../../../../../Script/funciones.php");
      ...
      $query_services = mysqli_query($db, "(SELECT PRODUCTO.*, '0' as Cantidad, '0' as Descuento, '1' as Tipo FROM Productos.PRODUCTO WHERE ((PRODUCTO.P_NOMBRE LIKE '%".$search."%') OR (PRODUCTO.P_CODIGO_BARRAS = '%".$search."%') OR (PRODUCTO.P_CODIGO = '%".$search."%')) AND PRODUCTO.P_ESTADO = 1 AND PRODUCTO.P_HELADOS = 1 AND PRODUCTO.P_TERRAZAS = 0 AND PRODUCTO.R_CODIGO IS NULL
) UNION ALL
SELECT PRODUCTO.*, '0' as Cantidad, R_DESCUENTO as Descuento, '0' as Tipo FROM Productos.PRODUCTO, Productos.RECETA WHERE PRODUCTO.R_CODIGO=RECETA.R_CODIGO AND ((PRODUCTO.P_NOMBRE LIKE '%".$search."%') OR (PRODUCTO.P_CODIGO_BARRAS = '%".$search."%') OR (PRODUCTO.P_CODIGO = '%".$search."%')) AND PRODUCTO.P_ESTADO = 1 AND PRODUCTO.P_TERRAZAS = 1 AND RECETA.R_TERRAZAS = 1
UNION ALL
