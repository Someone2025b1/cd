...
<head>
	...
	<script>
		...
		function Imprimir()
		{
			...
			printJS('<?php $_SERVER['DOCUMENT_ROOT']?>/ProgramFiles/APPS/CYM/Restaurant/Facturacion/FactImpNew.php?Codigo='+Cod);
	...
	<div id="base">
		...
		<?php
			...
			if($TipoPago == 1)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO, F_ORDEN, F_CON_DESCUENTO)
				...
				else
				{
					for($i=0; $i<$Contador; $i++)
					{
						...
						$query = mysqli_query($db,"INSERT INTO Bodega.FACTURA_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO)
					...
					for($i=1; $i<$Contador; $i++)
				{
					...
					WHERE P_CODIGO = ".$Prod);
						...
						if($rowe["R_CODIGO"]!=NULL){
							...
							while($rower = mysqli_fetch_array($resultr))
							{
								...
								VALUES('".$KCod."', '".$Uid."', '".$ProdR."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Terazas Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 1, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
					...
					$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
					VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Terazas Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAn."', '".$cantidadgen."', 1, '".$existenciaAn."', '".$nuevaexistencia."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   $QueryUpdateFactura = mysqli_query($db,"UPDATE Bodega.FACTURA SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'")or die(mysqli_error());
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO, F_ORDEN, F_TIPO_TARJETA, F_CON_DESCUENTO)
...
if($EventoPertenece!="SINEVENTO"){
	...
	$QueryEvento= mysqli_query($db, "UPDATE Eventos.EVENTO SET
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO, F_ORDEN, F_CON_DESCUENTO)
...
if($EventoPertenece!="SINEVENTO"){
	...
	$QueryEvento= mysqli_query($db, "UPDATE Eventos.EVENTO SET
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db,"INSERT INTO Bodega.FACTURA (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA, F_ORDEN, F_CON_DESCUENTO)
