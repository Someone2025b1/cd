			...
			<div class="card">
				<div class="card-body">
					<?php
						...
						$QueryExisteCliente = mysqli_query($db, "SELECT CE_CODIGO FROM Bodega.CLIENTE_EVENTO WHERE CE_CODIGO = ".$CodigoCliente)or die(mysqli_error());
						...
						if($RegistroExisteCliente > 0)
						{
							$QueryUpdateCliente = mysqli_query($db, "UPDATE Bodega.CLIENTE_EVENTO SET CE_NOMBRE = '".$Nombre."',
																								...
																								CE_EMAIL = '".$Email."'
																								WHERE CE_CODIGO = ".$CodigoCliente)or die(mysqli_error());
						}
						else
						{
							$QueryInsertCliente = mysqli_query($db, "INSERT INTO Bodega.CLIENTE_EVENTO(CE_CUI, CE_NIT, CE_NOMBRE, CE_DIRECCION, CE_CELULAR, CE_TELEFONO, CE_EMAIL)VALUES('".$CUI."', '".$NIT."', '".$Nombre."', '".$Direccion."', ".$Celular.", ".$Telefono.", '".$Email."')")or die(mysqli_error());
						}


						$Query = mysqli_query($db, "INSERT INTO Bodega.DEGUSTACION(D_REFERENCIA, D_FECHA, D_HORA, CE_CODIGO, D_CUI, D_NIT, D_NOMBRE, D_DIRECCION, D_CELULAR, D_TELEFONO, D_EMAIL, D_FECHA_INGRESO, D_HORA_INGRESO, D_ESTADO, D_COLABORADOR)VALUES('".$CodigoUnico."', '".$Fecha."', '".$HoraInicio."', '".$CodigoCliente."', '".$CUI."', '".$NIT."', '".$Nombre."', '".$Direccion."', '".$Celular."', '".$Telefono."', '".$Email."', CURRENT_DATE(), CURRENT_TIME(), 1, ".$id_user.")")or die(mysqli_error());
						...
						for ($i=1; $i <= $Contador ; $i++)
						{
							$CodigoPlatillo = uniqid();
								...
								{
									$QueryPlatillo = mysqli_query($db, "INSERT INTO Bodega.PLATILLO_DEGUSTACION(PD_CODIGO, D_REFERENCIA, RS_CODIGO, PD_CANTIDAD)VALUES('".$CodigoPlatillo."', '".$CodigoUnico."', '".$Receta[$i-1]."', ".$Cantidad[$i].")")or die(mysqli_error());
								}
						}
						...
						if($EnvioEmail == 1)
						{
			                ...
			                $mail->Subject = utf8_decode('Degustación en Chatun'); // Asunto
...
<h3 style="color: #2e6c80;">Es un gusto poder saludarle, le recordamos que el d&iacute;a '.date("d-m-Y", strtotime($Fecha)).' a las '.$HoraInicio.' tenemos reservada su degustaci&oacute;n.</h3>';
					...
					<div class="col-lg-12 text-center">
						...
						<h2 class="text-light">La degustación se ingresó correctamente.</h2>
					</div>
			</div>
			<!-- END CONTENT -->
