...
$sql = "SELECT * FROM Bodega.FACTURA_PS AS A LEFT JOIN Bodega.RESOLUCION AS B ON A.RES_NUMERO = B.RES_NUMERO WHERE A.F_CODIGO = '".$Factura."'";
...
$sqlD = "SELECT FACTURA_PS_DETALLE.*, PRODUCTO.P_NOMBRE 
                FROM Bodega.FACTURA_PS_DETALLE, Bodega.PRODUCTO 
                WHERE FACTURA_PS_DETALLE.RS_CODIGO = PRODUCTO.P_CODIGO
                AND FACTURA_PS_DETALLE.F_CODIGO = '".$Factura."'";
$resultD = mysqli_query($db, $sqlD);
while($rowD = mysqli_fetch_array($resultD))
{
    if($rowD["P_CODIGO"]==307){
        $locker=1;
    }
    ...
    $TablaProducto .= <<<EOD
    <tr>
    <td align="left" style="font-size: 30px; width: 100px;">$Cantidad</td>
    <td align="left" style="font-size: 30px; width: 130px;">$NombreProd</td>
