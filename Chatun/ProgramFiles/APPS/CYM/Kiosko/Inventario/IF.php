					...
					<div class="card">
						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								...
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Productos.INVENTARIO AS A WHERE A.I_ESTADO = 0 AND A.I_PUNTO = 4 ORDER BY A.I_FECHA ";
									...
									$Consulta = "SELECT * FROM Productos.INVENTARIO_RECONTEO AS A WHERE A.I_ESTADO = 0 AND A.I_PUNTO = 4 ORDER BY A.I_FECHA ";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
