...
function empezar() {
	document.form_bienes_inmuebles.entidad_financiera.focus();
}
function Verificar() {
...
function Verificar2() {
	if (document.form_bienes_inmuebles_editar.entidad_financiera.value == 'Bancos') {
		document.form_bienes_inmuebles_editar.acreedor.style.backgroundColor="#FFFFFF";
		document.form_bienes_inmuebles_editar.acreedor.value = ''
		document.form_bienes_inmuebles_editar.acreedor.disabled = false;
		document.form_bienes_inmuebles_editar.acreedor.readOnly = false;
		document.form_bienes_inmuebles_editar.acreedor.focus();
	}
	if (document.form_bienes_inmuebles_editar.entidad_financiera.value != 'Bancos') {
		document.form_bienes_inmuebles_editar.acreedor.style.backgroundColor="#999";
		document.form_bienes_inmuebles_editar.acreedor.value = "Coosajo R.L.";
		document.form_bienes_inmuebles_editar.acreedor.readOnly = true;
		document.form_bienes_inmuebles_editar.garantia.focus();
	}
}
function operar() {
...
function verificar_monto() {
	...
	dato2 = parseFloat(dato2);
		...
		if (dato2 > dato1){
			...
			document.form_bienes_inmuebles.saldo_actual.select();
			return false;
		}
}
function verificar_monto1() {
	var dato1 = document.form_bienes_inmuebles_editar.monto_original.value;
	var dato2 = document.form_bienes_inmuebles_editar.saldo_actual.value;			
	dato1 = parseFloat(dato1);
	dato2 = parseFloat(dato2);
	
		if (dato2 > dato1){
			alert("El Saldo actual debe ser menor o igual al monto original..VERIFIQUE");
			document.form_bienes_inmuebles_editar.saldo_actual.select();
			return false;
		}
}

function operar1() {
	if (document.form_bienes_inmuebles_editar.acreedor.value != ""){
		if (document.form_bienes_inmuebles_editar.vencimiento.value != ""){
		if (document.form_bienes_inmuebles_editar.monto_original.value > 0){
			if (document.form_bienes_inmuebles_editar.saldo_actual.value > 0){
					if (document.form_bienes_inmuebles_editar.monto_amortizacion.value > 0){
						if (document.form_bienes_inmuebles_editar.saldo_actual.value <= document.form_bienes_inmuebles_editar.monto_original.value){
								if (document.form_bienes_inmuebles_editar.garantia.value != ""){
										if (document.form_bienes_inmuebles_editar.frecuencia.value != ""){
...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	...
	if ($_GET["accion"] == 3) {
		...
		$saldo_actual = str_replace(",","",$saldo_actual_1);
		$monto_amortizacion = str_replace(",","",$monto_amortizacion_1);		
		$actualizar = "UPDATE Estado_Patrimonial.detalle_obligaciones_corto_plazo SET colaborador = '$auxi', entidad_financiera = '$entidad_financiera', acreedor = '$acreedor', garantia = '$garantia', vencimiento = '$vencimiento', monto_original = '$monto_original', saldo_actual = '$saldo_actual', frecuencia = '$frecuencia', monto_amortizacion = '$monto_amortizacion' WHERE id = '$id' and mes=$mes_periodo and anio=$anio_periodo ";
...
<form action="obligaciones_cortoplazo.php?accion=1" method="post" name="form_bienes_inmuebles" id="form_bienes_inmuebles">
  <tr>
    <td colspan="2" align="center"><b>DETALLE DE OBLIGACIONES A CORTO PLAZO (hasta 1 A&ntilde;o)</b></td>
    </tr>
  <tr>
  ...
  <tr>
    ...
    <td width="348"><label for="acreedor"></label>
      <input name="acreedor" type="text" id="acreedor" style="background-color:#999" value="Coosajo R.L." readonly></td>
    </tr>
  <tr>
    <td>Garant&iacute;a:</td>
    <td><label for="garantia"></label>
...
<?php
		...
		$id = $_GET["id"];
		$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_obligaciones_corto_plazo WHERE colaborador = '$auxi' and id = '$id' and mes=$mes_periodo and anio=$anio_periodo ";
	$query_historial = mysqli_query($db, $sql_historial);
	while ($editar=mysqli_fetch_array($query_historial)) 	{ 	
?>
<table width="600" border="0" align="center" id="tabla_bienes_inmuebles" class="Tamaño12">
<form action="obligaciones_cortoplazo.php?accion=3&id=<?php echo $editar['id'] ?>" method="post" name="form_bienes_inmuebles_editar" id="form_bienes_inmuebles_editar">
  <tr>
    <td colspan="2" align="center"><b>DETALLE DE OBLIGACIONES A CORTO PLAZO (hasta 1 A&ntilde;o)</b></td>
    </tr>
  <tr>
...
<tr> 
  ...
  <?php
	$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_obligaciones_corto_plazo WHERE colaborador = '$auxi' and mes=$mes_periodo and anio=$anio_periodo ";
	$query_historial = mysqli_query($db, $sql_historial);
...
<td><a href="obligaciones_cortoplazo.php?accion=2&id=<?php echo $historial['id']?>"><img src="../Imagenes/edit.png" alt="Editar..." width="16" height="16" border="0"></a><a href="obligaciones_cortoplazo.php?accion=4&id=<?php echo $historial['id']?>"><img src="../Imagenes/borrar.png" alt="Borrar..." width="16" height="16" border="0"></a></td>
<?php 
	}	//end while
?>
</tr>
