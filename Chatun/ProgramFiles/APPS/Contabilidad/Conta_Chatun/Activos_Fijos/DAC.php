...
$QueryColaboradores = "SELECT AF_RESPONSABLE FROM Contabilidad.ACTIVO_FIJO GROUP BY AF_RESPONSABLE";
$ResultColaboradores = mysqli_query($db,$QueryColaboradores);
while($FilaColaboradores = mysqli_fetch_array($ResultColaboradores))
{
	$i = 0;
	$TotalValorAdquisicion = 0;
	$TotalValorActual = 0;
	unset($Data);
	$pdf->ezText("Responsable: ".$FilaColaboradores["AF_RESPONSABLE"]."     ".utf8_decode(saber_nombre_colaborador($FilaColaboradores["AF_RESPONSABLE"])),10,array('justification'=>'left'));
	$pdf->ezText("", 15);
	...
	$query = "SELECT ACTIVO_FIJO.*, AREA_GASTO.AG_NOMBRE, TIPO_ACTIVO.TA_NOMBRE#, TRANSACCION.TRA_FECHA_TRANS
			...
			AND ACTIVO_FIJO.AF_RESPONSABLE = ".$FilaColaboradores["AF_RESPONSABLE"];
	...
	while($row = mysqli_fetch_array($result))
	{
	    ...
	    $Data[] = array('col1'=>$row["AF_CODIGO"], 'col2'=>utf8_decode($row["AF_NOMBRE"]), 'col3'=>utf8_decode($row["AG_NOMBRE"]), 'col4'=>utf8_decode($row["TA_NOMBRE"]), 'col5'=>date('d-m-Y', strtotime($row["AF_FECHA_ADQUISICION"])), 'col6'=>number_format($row["AF_VALOR"], 2, '.', ','), 'col7'=>number_format($row["AF_VALOR_ACTUAL"], 2, '.', ','));
	    $i++;
	}
