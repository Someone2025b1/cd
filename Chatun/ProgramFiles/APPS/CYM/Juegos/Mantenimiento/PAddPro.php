				...
				<form class="form"  method="POST" role="form">
					<?php
						...
						$PrecioEspecial  = $_POST["PrecioEspecial"];
					 	$CodigoContable = $_POST["Nomenclatura"];
					 	//Loop para recorrer todos los archivos
			                        for($i=0; $i<count($_FILES['Icono']['name']); $i++) 
			                        {
			                            //Obtener el nombre temporal del archivo
			                            $tmpFilePath = $_FILES['Icono']['tmp_name'][$i];
			                            ...
			                            if ($tmpFilePath != "")
			                            {
			                                ...
			                                $archivo = basename( $_FILES['Icono']['name'][$i]); 
			                                ...
			                                { 
											$sql = mysqli_query($db, "INSERT INTO Bodega.PRODUCTO (P_NOMBRE,  CP_CODIGO,  P_PRECIO, N_CODIGO, P_PRECIO_ESPECIAL, P_ICONO)
											VALUES ('".$Nombre."', '".$Categoria."',  '".$Precio."', '".$CodigoContable."', '$PrecioEspecial', '". $newFilePath ."')");
												if(!$sql)
												{
