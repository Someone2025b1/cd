...
$sql = "SELECT * FROM Bodega.FACTURA_JG AS A LEFT JOIN Bodega.RESOLUCION AS B ON A.RES_NUMERO = B.RES_NUMERO WHERE A.F_CODIGO = '".$Factura."'";
...
$sqlD = "SELECT FACTURA_JG_DETALLE.*, FACTURA_JG_DETALLE.RS_NOMBRE AS RS_NOMBRE
                FROM Bodega.FACTURA_JG_DETALLE, Bodega.PRODUCTO
                WHERE FACTURA_JG_DETALLE.RS_CODIGO = PRODUCTO.P_CODIGO
                AND FACTURA_JG_DETALLE.F_CODIGO = '".$Factura."'";
...
$sqlD = "SELECT FACTURA_JG_DETALLE.*, FACTURA_JG_DETALLE.RS_NOMBRE AS RS_NOMBRE, PRODUCTO.P_ICONO
                FROM Bodega.FACTURA_JG_DETALLE, Bodega.PRODUCTO
                WHERE FACTURA_JG_DETALLE.RS_CODIGO = PRODUCTO.P_CODIGO
                AND FACTURA_JG_DETALLE.F_CODIGO = '".$Factura."'";
$resultD = mysqli_query($db, $sqlD);
...
if ($rowD["FD_TIPO"]==1) 
{
    ...
    <table border="0">  
        ...
        <tr style="font-size: 30px">
            <td align="left"><b>PS:</b> Juegos </td> 
        </tr>  
...
{
    $QueryCombo = mysqli_query($db, "SELECT B.P_NOMBRE, B.P_ICONO FROM Bodega.COMBO_DETALLE A 
    INNER JOIN Bodega.PRODUCTO B ON A.P_CODIGO = B.P_CODIGO
    WHERE A.Combo_Id = $rowD[RS_CODIGO]");
    $i = 0;
    while ($RoC = mysqli_fetch_array($QueryCombo)) 
    { 
    $Transac  = $rowD["FD_CORRELATIVO"]."-".$i; 
    $NombreCombo  = $NombreProd."-".$RoC["P_NOMBRE"]; 
    $Icono  = $RoC["P_ICONO"];
    $pdf->AddPage();
    $pdf->SetMargins(5,0,10, true);
    $pdf->SetFont('Helvetica', '', 20);  
...
elseif ($rowD["FD_TIPO"]==3) 
{
    $QueryCombo = mysqli_fetch_array(mysqli_query($db, "SELECT a.Precio, a.Cantidad, b.P_NOMBRE, a.IdEscala, b.P_ICONO FROM Bodega.ESCALA_PRODUCTO a
        INNER JOIN Bodega.PRODUCTO  b ON a.P_CODIGO = b.P_CODIGO
        WHERE IdEscala = $rowD[RS_CODIGO]"));
    $Cantidad = $QueryCombo["Cantidad"]; 
    for ($i=0; $i < $Cantidad; $i++)   
    { 
    $Icono  = $QueryCombo["P_ICONO"];
    $Transac  = $rowD["FD_CORRELATIVO"]."-".$i; 
