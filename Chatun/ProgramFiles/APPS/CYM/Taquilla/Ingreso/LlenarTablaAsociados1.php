...
if($RegistrosAsociados > 0)
{
	while($fila = mysqli_fetch_array($query))
	{
		...
		$QueryAhorros = sqlsrv_query($db_sql, "SELECT * FROM OPENQUERY(BANKWORKS, 'SELECT CAPSALDACTUAL FROM CAPCUENTAS WHERE CAPCODCLIENTE = ''".$CIF_ASOCIADO."'' GROUP BY CAPSALDACTUAL HAVING SUM(CAPSALDACTUAL) >= 500')", $params, $options);
		...
		$Variable .= '<tr>';
			$Variable .= '<td>'.$i.'</td>';
			$Variable .= '<td>'.$fila["IAT_CIF_ASOCIADO"].'</td>';
			...
			{
				$Variable .= '<td align="left"><button class="btn btn-success btn-sm"  value="'.$fila["IAT_CIF_ASOCIADO"].'" disabled><span class="fa fa-group"></span></button></td>';	
				$Variable .= '<td align="left"><button class="btn btn-success btn-sm"  value="'.$fila["IAT_CIF_ASOCIADO"].'" disabled><span class="bi bi-balloon-fill"></span></button></td>';
			...
			{
				$Variable .= '<td align="left"><button class="btn btn-success btn-sm"  value="'.$fila["IAT_CIF_ASOCIADO"].'" onclick="AgregarAcompanante(this.value)"><span class="fa fa-group"></span></button></td>';
				$Variable .= '<td align="left"><button class="btn btn-success btn-sm"  value="'.$fila["IAT_CIF_ASOCIADO"].'" onclick="AgregarAcompananteMenor(this.value)"><span class="bi bi-balloon-fill"></span></button></td>';
