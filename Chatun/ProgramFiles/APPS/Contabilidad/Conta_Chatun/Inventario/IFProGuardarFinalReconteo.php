					...
					<div class="col-lg-12">
						...
						<div class="card">
							...
							<div class="card-body">
								...
								$QueryDetalle = mysqli_query($db, "UPDATE Productos.INVENTARIO_RECONTEO SET
													I_ESTADO = 2
													WHERE IR_CODIGO = '$CodigoConteo'");

								
								

								#recorrido de los datos para almacenar detalles
								$queryInventario = "SELECT * FROM Productos.INVENTARIO_RECONTEO_DETALLE WHERE ID_DIFERENCIA <> 0 AND IR_CODIGO = '$CodigoConteo'";
									$resultInventario = mysqli_query($db, $queryInventario);
									while($rowInventario = mysqli_fetch_array($resultInventario))
									{
									$Cod = $rowInventario["P_CODIGO"];
									$Can = $rowInventario["ID_DIFERENCIA"];
									$Rea = $rowInventario["ID_CANTIDAD_SISTEMA"];
