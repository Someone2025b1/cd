							...
							<?php
                                ...
                                for($i=0; $i<$Contador; $i++)
                                {
									...
									{	
										...
										if($row["EPP_FECHA_KIOSCO"]>$row["EPP_FECHA_AJUSTE_KIOSCO"]){

											$FechaInicio =  $row["EPP_FECHA_KIOSCO"];

										}else{
											$FechaInicio =  $row["EPP_FECHA_AJUSTE_KIOSCO"];
										}
										
										$AnteriorEx=$row["EPP_ANTERIOR_KIOSCO"];
										$ExistenciaActual=$row["EPP_KIOSCO"];
										$UltimoIngreso=$row["EPP_ULTIMO_KIOSCO"];
									...
									AND KARDEX.K_DESCRPCION LIKE '%Venta de Producto Segun Factura de kiosco Azados%'
									...
									FROM Productos.KARDEX
									WHERE KARDEX.P_CODIGO = '".$Prod."'
									AND KARDEX.K_DESCRPCION LIKE '%Devolucion de Producto Segun Factura Anulada de Kiosco Azados%'
									AND KARDEX.K_FECHA BETWEEN '$FechaInicio' AND '$FechaFin'
									AND KARDEX.K_PUNTO_VENTA = 1");
									while($row1 = mysqli_fetch_array($NomTitulo))
											{
									...
									##########FIN##########
											...
											#VALUES('".$CodFin."', CURRENT_DATE(), '".$Prod."', '".$Can."', '".$Con."', '".$Ventas."', '".$Devoluciones."', '".$ExistenciaActual."', '".$Diferencia."', 'Kiosco Azados')");


											$QueryPU = mysqli_query($db, "UPDATE Productos.EX_PUNTOS_PEQUENOS_DET SET
											EPPD_CONTEO = '".$Can."',
											EPPD_VENTA = '".$Ventas."',
											EPPD_DEVOLUCIONES = '".$Devoluciones."',
											EPDD_EX_ANTERIOR = '".$ExistenciaActual."',
											EPDD_DIFERENCIA = '".$Diferencia."',
											EPDD_ESTADO = 0,
											EPPD_CONCILIACION = '".$Con."'
