...
elseif($Bodega==6){
    $NomTitulo = mysqli_query($db, "SELECT * FROM Productos.PRODUCTO WHERE PRODUCTO.P_EXISTENCIA_KIOSCO<>0 AND PRODUCTO.P_TERRAZAS=1 AND PRODUCTO.P_LLEVA_EXISTENCIA=1");
    while($row1 = mysqli_fetch_array($NomTitulo))
            {
                $Cod = $row1["P_CODIGO"];
                $Nombre=$row1["P_NOMBRE"];
                $Ponderado=$row1["P_PRECIO_COMPRA_PONDERADO"];
                $Venta=$row1["P_PRECIO_VENTA"];
                $Souivenirs=$row1["P_EXISTENCIA_KIOSCO"];
    
                $subtot=$row1["P_PRECIO_COMPRA_PONDERADO"]*$row1["P_EXISTENCIA_KIOSCO"];
                $Total+=$subtot;
...
elseif($Bodega==5){
    $NomTitulo = mysqli_query($db, "SELECT * FROM Productos.PRODUCTO WHERE PRODUCTO.P_EXISTENCIA_BODEGA<>0 AND PRODUCTO.P_LLEVA_EXISTENCIA=1");
	while($row1 = mysqli_fetch_array($NomTitulo))
            {
                $Cod = $row1["P_CODIGO"];
                $Nombre=$row1["P_NOMBRE"];
...
if($Bodega==8){
    $NomTitulo = mysqli_query($db, "SELECT * FROM Productos.PRODUCTO WHERE PRODUCTO.P_EXISTENCIA_PIZZERIA<>0 AND PRODUCTO.P_TERRAZAS=1 AND PRODUCTO.P_LLEVA_EXISTENCIA=1");
    while($row1 = mysqli_fetch_array($NomTitulo))
            {
                $Cod = $row1["P_CODIGO"];
                $Nombre=$row1["P_NOMBRE"];
                $Ponderado=$row1["P_PRECIO_COMPRA_PONDERADO"];
                $Venta=$row1["P_PRECIO_VENTA"];
                $Terrazas=$row1["P_EXISTENCIA_PIZZERIA"];
    
                $subtot=$row1["P_PRECIO_COMPRA_PONDERADO"]*$row1["P_EXISTENCIA_PIZZERIA"];
                $Total+=$subtot;
