...
<tr bgcolor="#CCFF99" class="LetraBlanca Tamaño10" id="dateformat2">
  ...
  <?php
	if ($_GET["accion"] == 1) {
		...
		$insertar = "INSERT INTO Estado_Patrimonial.detalle_obligaciones_corto_plazo (colaborador, entidad_financiera, acreedor, garantia, vencimiento, monto_original, saldo_actual, frecuencia, monto_amortizacion) VALUES ('$auxi', '$entidad_financiera', '$acreedor', '$garantia', '$vencimiento', '$monto_original', '$saldo_actual', '$frecuencia', '$monto_amortizacion')";
		mysqli_query($db, $insertar) or die (mysqli_error());
	}
	if ($_GET["accion"] == 4) {
		$id = $_GET["id"];
		$eliminar = "DELETE FROM Estado_Patrimonial.detalle_obligaciones_corto_plazo WHERE id = '$id'";
...
<?php
		...
		$id = $_GET["id"];
		$sql_historial="SELECT * FROM Estado_Patrimonial.detalle_obligaciones_corto_plazo WHERE colaborador = '$auxi' and id = '$id'";
