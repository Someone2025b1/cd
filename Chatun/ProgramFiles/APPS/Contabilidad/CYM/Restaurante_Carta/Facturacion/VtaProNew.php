	...
	<div id="base">
		...
		<?php
			...
			if($TipoPago == 1)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_RS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_FECHA_TRANS, F_HORA, F_USUARIO, F_MONEDA, F_EFECTIVO, F_CAMBIO, F_OBSERVACIONES, RES_NUMERO)
												VALUES ('".$Uid."', '".$SerieAutorizada."', 1, '".$NIT."', ".$TotalFacturaFinal.", CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user.", ".$Moneda.", ".$Efectivo.", ".$Cambio.", '".$Observaciones."', '".$NumeroResolucion."')");
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Restaurante Mirador Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 14, 1, '".$Uid."')") or die(mysqli_error());
				...
				else
				{
					for($i=0; $i<$Contador; $i++)
					{
						...
						$query = mysqli_query($db, "INSERT INTO Bodega.FACTURA_RS_DETALLE (F_CODIGO, FD_CODIGO, FD_CANTIDAD, RS_CODIGO, FD_PRECIO_UNITARIO, FD_SUBTOTAL, FD_DESCUENTO)
											VALUES('".$Uid."', '".$UidD."', ".$Can.", '".$Prod."', ".$Pre.", ".$Sub.", ".$Des.")");
					...
					****************************************************************************************/
					...
					$SqlContable = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_CONCEPTO, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, TRA_SERIE, TRA_FACTURA, TRA_TOTAL, RES_NUMERO, TRA_CORRELATIVO, PC_CODIGO)
											 VALUES('".$Uid."', CURRENT_DATE(), 'Vta. Restaurante Mirador Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 2, 20, '".$SerieAutorizada."', '".$FacturaNumero."', ".$TotalFacturaFinal.", '".$NumeroResolucion."', ".$Correlativo.", ".$Periodo.")") or die(mysqli_error());
					...
					$SqlContableD = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.01.012', ".number_format($TotalFacturaFinal, 2, ".", "").", 0.00)");
					...
					$SqlContableD2 = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '4.01.03.007', 0.00, ".number_format($VentaSinIVA, 3, ".", "").")");
				}
					...
					for($i=1; $i<$Contador; $i++)
				{
					...
					WHERE P_CODIGO = ".$Prod);
						...
						if($rowe["R_CODIGO"]!=NULL){
							...
							while($rower = mysqli_fetch_array($resultr))
							{
								...
								VALUES('".$KCod."', '".$Uid."', '".$ProdR."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Restaurante Mirador Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 1, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
					...
					$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
					VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Restaurante Mirador Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAn."', '".$cantidadgen."', 1, '".$existenciaAn."', '".$nuevaexistencia."')");
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   <?php 
						$Upd_Mesa = mysqli_query($db, "UPDATE Bodega.MESA SET ME_CODIGO = 1, M_CIF_ATIENDE = NULL, M_TIPO_ORDEN = NULL, M_NOMBRE_FACTURA = NULL, M_NIT_FACTURA = NULL, M_DIRECCION_FACTURA = NULL WHERE M_CODIGO = ".$_POST["CodigoMesa"]);

						$Del_DatosOrden = mysqli_query($db, "DELETE FROM Bodega.MESA_ORDEN WHERE M_CODIGO = ".$_POST["CodigoMesa"]);
					   ...
					   $QueryUpdateFactura = mysqli_query($db, "UPDATE Bodega.FACTURA_RS SET F_SERIE = '".$responseCerti->body->serie."', F_CAE = '".$responseCerti->body->numero."', F_DTE = '".$responseCerti->body->uuid."', F_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE F_CODIGO = '".$Uid."'");

					   $QueryUpdateTransaccionBodega = mysqli_query($db, "UPDATE Bodega.TRANSACCION SET TRA_OBSERVACIONES = 'Vta. Restaurante Mirador Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."' WHERE F_CODIGO = '".$Uid."'");

					   $QueryUpdateTransaccionContabilidad = mysqli_query($db, "UPDATE Contabilidad.TRANSACCION SET TRA_CONCEPTO = 'Vta. Restaurante Mirador Según Fact. ".$responseCerti->body->serie.' - '.$responseCerti->body->numero.' - '.$responseCerti->body->uuid."', TRA_FECHA_CERTIFICACION = '".$responseCerti->body->fecha."' WHERE TRA_CODIGO = '".$Uid."'");
					}
					else
					{
						$QueryBitacora = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA 
						SELECT FACTURA_RS.F_CODIGO, '5', FACTURA_RS.F_ORDEN, FACTURA_RS.F_SERIE, FACTURA_RS.F_NUMERO, FACTURA_RS.F_TIPO, FACTURA_RS.CLI_NIT, FACTURA_RS.D_CODIGO, FACTURA_RS.F_CON_DESCUENTO, FACTURA_RS.F_TOTAL, FACTURA_RS.F_EFECTIVO, FACTURA_RS.F_CAMBIO, FACTURA_RS.F_FECHA_TRANS, FACTURA_RS.F_HORA, FACTURA_RS.F_USUARIO, FACTURA_RS.F_MONEDA, FACTURA_RS.F_ESTADO, FACTURA_RS.F_RAZON_ANULACION, FACTURA_RS.F_NOMBRE_CREDITO, FACTURA_RS.F_NO_BOLETA, FACTURA_RS.F_OBSERVACIONES, FACTURA_RS.F_NO_AUTORIZACION, FACTURA_RS.RES_NUMERO, FACTURA_RS.F_CUENTA_BANCARIA, FACTURA_RS.F_REALIZADA, FACTURA_RS.F_CAE, FACTURA_RS.F_DTE, FACTURA_RS.F_FECHA_CERTIFICACION, FACTURA_RS.F_TIPO_TARJETA FROM Bodega.FACTURA_RS 
						WHERE  FACTURA_RS.F_CODIGO = '".$Uid."'")or die(mysqli_error());

							$QueryBitacora1 = mysqli_query($db, "INSERT INTO Bitacoras.FACTURA_DETALLE SELECT F_CODIGO, FD_CODIGO, FD_CORRELATIVO, 5, FD_CANTIDAD, RS_CODIGO, FD_PRECIO_UNITARIO, FD_DESCUENTO, FD_SUBTOTAL FROM Bodega.FACTURA_RS_DETALLE WHERE FACTURA_RS_DETALLE.F_CODIGO = '".$Uid."'")or die(mysqli_error());
							...
							$QueryDosRestaurante = mysqli_query($db, "DELETE 
																...
																WHERE FACTURA_RS_DETALLE.F_CODIGO = '".$Uid."'")or die(mysqli_error());
			...
			elseif($TipoPago == 2)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_RS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_OBSERVACIONES, F_NO_AUTORIZACION, RES_NUMERO)
				...
				$sqlAbono = mysqli_query($db, "INSERT INTO Bodega.TRANSACCION (TRA_CODIGO, TRA_FECHA_TRANS, TRA_OBSERVACIONES, TRA_FECHA_HOY, TRA_HORA, TRA_USUARIO, E_CODIGO, TT_CODIGO, B_CODIGO, F_CODIGO)
											 VALUES('".$UI."', CURRENT_DATE(), 'Vta. Restaurante Mirador Según Fact. ".$SerieAutorizada." ".$FacturaNumero."',  CURRENT_DATE(), CURRENT_TIMESTAMP(), ".$id_user .", 1, 14, 1, '".$Uid."')");
				...
				else
				{
					...
					****************************************************************************************/
					...
					$SqlContableD = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '1.01.03.010', ".$TotalFacturaFinal.", 0.00)");

					$SqlContableD1 = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '2.01.04.001', 0.00, ".$IVADebito.")");

					$SqlContableD2 = mysqli_query($db, "INSERT INTO Contabilidad.TRANSACCION_DETALLE(TRA_CODIGO, TRAD_CODIGO, N_CODIGO, TRAD_CARGO_CONTA, TRAD_ABONO_CONTA) 
												VALUES('".$Uid."', '".$UidD."', '4.01.03.007', 0.00, ".$VentaSinIVA.")");
				}
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			...
			elseif($TipoPago == 3)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_RS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NOMBRE_CREDITO, F_OBSERVACIONES, RES_NUMERO)
...
$TotalFacturaFinal=number_format($TotalFacturaFinal, 2, ".", "");
		...
		while($roweg = mysqli_fetch_array($resultg))
		{
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
					   ...
					   VALUES ('".$CCod."', '".$Uid."', 1, $NIT,  '".$id_user."', '".$TotalFacturaFinal."', 'MIRADOR', 0.00)");
			...
			elseif($TipoPago == 4)
			{
				...
				$QueryEncabezado = mysqli_query($db, "INSERT INTO Bodega.FACTURA_RS (F_CODIGO, F_SERIE, F_TIPO, CLI_NIT, F_TOTAL, F_EFECTIVO, F_CAMBIO, F_FECHA_TRANS, F_HORA, F_USUARIO, F_NO_BOLETA, F_OBSERVACIONES, RES_NUMERO, F_CUENTA_BANCARIA)
