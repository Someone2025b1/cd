...
/*TITULOS*/
    $Titulo = array('col1'=>'Cuenta', 'col2'=>'Nombre', 'col3'=>'Saldo Inicial', 'col4'=>'Cargos', 'col5'=>'Abonos', 'col6'=>'Saldo Final');
/*FIN TITULOS*/
...
FROM Contabilidad.TRANSACCION_DETALLE, Contabilidad.TRANSACCION, Contabilidad.NOMENCLATURA
...
while($FTC = mysqli_fetch_array($ResultTodasCuentas))
{
    ...
    while($row = mysqli_fetch_array($ResultCuentas))
    {
        ...
        $CargosMostrar = number_format($Cargos, 2, ".", "");
        $AbonosMostrar = number_format($Abonos, 2, ".", "");
            ...
            while($row1 = mysqli_fetch_array($ResultCuentasAcumulado))
            {
                ...
                $TotalAcumuladoMostrar = number_format($TotalAcumulado, 2, ".", "");
            }

            $SaldoFinal = $TotalAcumulado + $Total;
            $SaldoFinalMostrar = number_format($SaldoFinal, 2, ".", "");
            ...
            $Data[] = array('col1'=>$Cuenta, 'col2'=>$NombreCuenta, 'col3'=>$TotalAcumuladoMostrar, 'col4'=>$CargosMostrar, 'col5'=>$AbonosMostrar, 'col6'=>$SaldoFinalMostrar);
    }

   
}
 if($FechaFinal >= "2023-01-01"){
        $SumaSaldoInicial+=0.02;
        $SumaSaldoFinal+=0.02;
    }

$Data[] = array('col3'=>number_format($SumaSaldoInicial, 2, ".", ","), 'col4'=>number_format($SumaCargos, 2, ".", ","), 'col5'=>number_format($SumaAbonos, 2, ".", ","), 'col6'=>number_format($SumaSaldoFinal, 2, ".", ","), 'col1'=>'', 'col2'=>'TOTALES');
