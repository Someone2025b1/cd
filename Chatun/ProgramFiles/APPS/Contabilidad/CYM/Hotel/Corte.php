								   ...
								   <div class="row">
										...
										<div class="col-lg-5">
									 		<select class="form-control selectpicker" required data-live-search="true"  id="Recibido" name="Recibido">
									 			...
									 			<?php 
									 			$DataRecibido = mysqli_query($db, "SELECT a.cif, primer_nombre, segundo_nombre, primer_apellido FROM info_colaboradores.Vista_Colaboradores_Nueva a
													INNER JOIN info_colaboradores.datos_laborales b ON a.cif = b.cif
													WHERE b.departamento = 55 ");
									 			while ($RowColaborador = mysqli_fetch_array($DataRecibido))
									 			{
									 			?>
									 			<option value="<?php echo $RowColaborador[cif]?>"><?php echo $RowColaborador['primer_nombre']." ".$RowColaborador['segundo_nombre']." ".$RowColaborador['primer_apellido'] ?></option>
									 			<?php 
									 			}
									 			?>
									 		</select>
