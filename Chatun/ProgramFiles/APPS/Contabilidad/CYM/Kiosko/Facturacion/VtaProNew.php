		...
		<?php
			...
			if($TipoPago == 1)
			{
					...
					for($i=1; $i<$Contador; $i++)
				{
					...
					WHERE P_CODIGO = ".$Prod);
						...
						if($rowe["R_CODIGO"]!=NULL){
							...
							while($rower = mysqli_fetch_array($resultr))
							{
								...
								VALUES('".$KCod."', '".$Uid."', '".$ProdR."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Cafe Los Abuelos Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 4, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
					...
					$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
					VALUES('".$KCod."', '".$Uid."', '".$Prod."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Cafe Los Abuelos Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAn."', '".$cantidadgen."', 4, '".$existenciaAn."', '".$nuevaexistencia."')");

					}
				}

				#PARA SACAR LAS BOLAS DE HELADO
				$QuerySelSabores = "SELECT * FROM Bodega.MESA_BOLAS WHERE M_CODIGO=".$_POST["CodigoMesa"];
				...
				if($RegistrosSelSabores > 0)
				{
					...
					{
						$CodProdSabor = $FilaSelSabores["P_CODIGO"];
						$CanR=3;
					...
					$querykardex = mysqli_query($db, "INSERT INTO Productos.KARDEX (K_CODIGO, TRA_CODIGO, P_CODIGO, K_FECHA, K_HORA, K_DESCRPCION, K_EXISTENCIA_ANTERIOR, K_EXISTENCIA_ACTUAL, K_PUNTO_VENTA, K_EXISTENCIA_ANTERIOR_PUNTO, K_EXISTENCIA_ACTUAL_PUNTO)
					VALUES('".$KCod."', '".$Uid."', '".$CodProdSabor."', CURRENT_DATE(), CURRENT_TIMESTAMP(), 'Venta de Producto Segun Factura de Cafe Los Abuelos Serie Número "."$SerieAutorizada"."  "."$FacturaNumero"."', '".$existenciaGAnR."', '".$cantidadgenR."', 4, '".$existenciaAnR."', '".$nuevaexistenciaR."')");
			...
			$QueryDelSelSabores = mysqli_query($db, "DELETE FROM Bodega.MESA_BOLAS WHERE M_CODIGO=".$_POST["CodigoMesa"]);
######################## FIN KARDEX ####################
			...
			$Generar=$roweg["Generar"];
					...
					for($i=1; $i<=$TotaldeBoletos; $i++)
				{
					$querykardex = mysqli_query($db, "INSERT INTO Bodega.BOLETOS (B_PUNTO, B_FACTURA)
						VALUES('CAFÉ DE LOS ABUELOS', '".$Uid."')");
