...
<script>
		...
		function EnviarForm()
	{
		...
		$(Formulario).attr('action', 'RecibirPedidoPro.php');
		$(Formulario).submit();
			...
			<div class="container">
				<form class="form" method="POST" role="form" id="FormularioEnviar">
					<div class="col-lg-12">
									...
									<div class="col-lg-12">
										<div class="form-group">
											<?php 
                                                ...
                                                $query = "SELECT * FROM Productos.EX_PUNTOS_ENVIOS WHERE EPE_CODIGO = '$CodigoEnv'";
                                                ...
                                                while($row = mysqli_fetch_array($result))
                                                {
                                                    ...
                                                    <div class="col-lg-3">
                                                            <div class="form-group">
                                                            <input class="form-control" type="date" name="Fecha" id="Fecha" value="<?php echo $row["EPE_FECHA"]; ?>"  readonly/>
                                                                <label for="Nombre">Fecha Solicitud</label>
                                                            </div>
                                                    </div>
                                                    <div class="col-lg-5">
                                                            <div class="form-group">
                                                            <input class="form-control" type="text" name="Pedido" id="Pedido" value="<?php echo $row["EPE_CODIGO"]; ?>"  readonly/>
                                                                <label for="Nombre">Pedido</label>
                                                            </div>
                                                    </div>
                                                    <div class="col-lg-4">
                                                            <div class="form-group">
                                                            <textarea class="form-control" name="Descripcion" id="Descripcion"  readonly><?php echo $row["EPE_DESCRIPCION"]; ?></textarea>
                                                ...
                                                <div class="col-lg-6">
                                                            ...
                                                            <input class="form-control" type="text" name="Envia" id="Envia" value="<?php echo $Envia; ?>"  readonly/>
                                                                <label for="Nombre">Quien Envía</label>
                                                            </div>
                                                </div>
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <table class="table" name="tabla" id="tabla">
                                        ...
                                        <tbody>
                                            <?php
                                            $query = "SELECT EX_PUNTOS_ENVIOS_DET.*, EX_PUNTOS_ENVIOS.EPE_CODIGO FROM Productos.EX_PUNTOS_ENVIOS_DET, Productos.EX_PUNTOS_ENVIOS WHERE EX_PUNTOS_ENVIOS_DET.EPE_CODIGO = EX_PUNTOS_ENVIOS.EPE_CODIGO AND EX_PUNTOS_ENVIOS.EPE_ESTADO <> 1 AND EX_PUNTOS_ENVIOS_DET.EPE_CODIGO ='$CodigoEnv'";
                                            $result = mysqli_query($db, $query);
                                            while($row = mysqli_fetch_array($result))
                                            {
                                                $Pro = $row['P_CODIGO'];
                                                    $ListEvento = mysqli_query($db, "SELECT a.P_NOMBRE FROM Productos.PRODUCTO a WHERE a.P_CODIGO =".$Pro);
												while ($RowEvento = mysqli_fetch_array($ListEvento))
												{
													$Producto = $RowEvento["P_NOMBRE"];
												}
                                            ?>
                                            <tr>
                                                <td><h6><input type="text" class="form-control" name="ProductoNombre[]" id="ProductoNombre[]" style="width: 800px" value= <?php echo $Producto; ?> required readonly><?php echo $Producto; ?></h6></td>
                                                <td><h6><input type="number" class="form-control" name="Cantidad[]" id="Cantidad[]" onChange="CalcularTotal()" style="width: 100px" min="0" value="<?php echo number_format($row["EPED_CANTIDAD"], 2, '.', ','); ?>" required readonly></h6></td>
                                                <td class="eliminar">
						...
						<button type="submit" onclick="EnviarForm()" class="btn ink-reaction btn-raised btn-primary" id="btnGuardar">Recibir</button>
							</div>
