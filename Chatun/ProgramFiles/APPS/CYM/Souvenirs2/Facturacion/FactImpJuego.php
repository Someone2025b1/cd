...
$sql = "SELECT * FROM Bodega.FACTURA_SV_2 AS A LEFT JOIN Bodega.RESOLUCION AS B ON A.RES_NUMERO = B.RES_NUMERO WHERE A.F_CODIGO = '".$Factura."'";
...
$sqlD = "SELECT FACTURA_SV_2_DETALLE.* 
 FROM Bodega.FACTURA_SV_2_DETALLE 
 WHERE FACTURA_SV_2_DETALLE.F_CODIGO = '".$Factura."'";
...
while($rowD = mysqli_fetch_array($resultD))
{
    ...
    if ($IdProd == 1) 
    {
      $Name = NombreProducto($rowD["P_CODIGO"]);
    } 
    else if ($IdProd == 2) 
    {
      $Name = NombreCombo($rowD["P_CODIGO"]);
    } 
    else if ($IdProd == 3) 
    {
      $Name = NombreEscala($rowD["P_CODIGO"]);
    } 
    ...
    <td align="left" style="font-size: 30px; width: 230px;">$Name</td>
...
$sqlD = "SELECT FACTURA_SV_2_DETALLE.*, PRODUCTO.P_NOMBRE AS RS_NOMBRE, PRODUCTO.P_ICONO
                FROM Bodega.FACTURA_SV_2_DETALLE, Bodega.PRODUCTO
                WHERE FACTURA_SV_2_DETALLE.P_CODIGO = PRODUCTO.P_CODIGO
                AND FACTURA_SV_2_DETALLE.F_CODIGO = '".$Factura."'";
$resultD = mysqli_query($db, $sqlD);
$Cont = 1;
while($rowD = mysqli_fetch_array($resultD))
{  
...
$Transac  = $rowD["FD_CORRELATIVO"];
if ($rowD["FD_TIPO"]==1) 
{
    for ($i=0; $i < $Cantidad; $i++) { 
    $Transac  = $rowD["FD_CORRELATIVO"]."-".$i; 
    $Icono  = "../../Juegos/Facturacion/".$rowD["P_ICONO"]; 
    ...
    <table border="0">  
        ...
        <tr style="font-size: 34px">
            <td colspan="8" align="center"> $NombreProd </td>
        </tr>   
        <tr style="font-size: 34px">
            <td colspan="8" align="center"><img src="$Icono" width="400" height="400"></td>
        </tr>   
    </table>
    EOD; 
...
elseif ($rowD["FD_TIPO"]==2) 
{
    $QueryCombo = mysqli_query($db, "SELECT B.P_NOMBRE, B.P_ICONO, c.C_Nombre FROM Bodega.COMBO_DETALLE A 
    INNER JOIN Bodega.PRODUCTO B ON A.P_CODIGO = B.P_CODIGO
    INNER JOIN Bodega.COMBO c ON c.C_Id = A.Combo_Id
    WHERE A.Combo_Id = $rowD[P_CODIGO]");
    $i = 0;
    while ($RoC = mysqli_fetch_array($QueryCombo)) 
    { 
    $Transac  = $rowD["FD_CORRELATIVO"]."-".$i; 
    $NombreCombo  = $RoC["C_Nombre"]."-".$RoC["P_NOMBRE"]; 
    $Icono  = "../../Juegos/Facturacion/".$RoC["P_ICONO"];
    $pdf->AddPage();
    $pdf->SetMargins(5,0,10, true);
    $pdf->SetFont('Helvetica', '', 20);  
    $pdf->Cell(0,0, "",0,1,'R'); 
    ...
    <table border="0">  
        ...
        <tr style="font-size: 34px">
            <td colspan="8" align="center"> $NombreCombo </td>
        </tr>  
        <tr style="font-size: 34px">
            <td colspan="8" align="center"><img src="$Icono" width="400" height="400"></td>
        </tr>   
...
elseif ($rowD["FD_TIPO"]==3) 
{
    $QueryCombo = mysqli_fetch_array(mysqli_query($db, "SELECT a.Precio, a.Cantidad, b.P_NOMBRE, a.IdEscala, b.P_ICONO FROM Bodega.ESCALA_PRODUCTO a
        INNER JOIN Bodega.PRODUCTO  b ON a.P_CODIGO = b.P_CODIGO
        WHERE IdEscala = $rowD[P_CODIGO]"));
    $Cantidad = $QueryCombo["Cantidad"]; 
    for ($i=0; $i < $Cantidad; $i++)   
    { 
    $Icono  = "../../Juegos/Facturacion/".$QueryCombo["P_ICONO"];
    $Transac  = $rowD["FD_CORRELATIVO"]."-".$i; 
    $NombreCombo  = $QueryCombo["P_NOMBRE"]; 
    $pdf->AddPage();
    $pdf->SetMargins(5,0,10, true);
    $pdf->SetFont('Helvetica', '', 20);  
    $pdf->Cell(0,0, "",0,1,'R'); 
