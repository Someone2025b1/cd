...
SELECT PRODUCTO.*, '0' as Cantidad, '0' as Tipo FROM Productos.PRODUCTO WHERE ((PRODUCTO.P_NOMBRE LIKE '%".$search."%') OR (PRODUCTO.P_CODIGO_BARRAS = '%".$search."%') OR (PRODUCTO.P_CODIGO = '%".$search."%')) AND PRODUCTO.P_ESTADO = 1 AND PRODUCTO.P_TERRAZAS = 1 AND PRODUCTO.R_CODIGO IS NULL
 LIMIT 100");
      ...
      while ($row_services = mysqli_fetch_array($query_services)) {
          ...
          echo '<h5 class="text-ultra-bold"><a onclick="AgregarProducto(this)"  class="suggest-element text-primary-dark" data="'.$row_services['P_NOMBRE'].'" dataPrecio="'.$row_services['P_PRECIO_VENTA'].'" dataCant="'.$row_services['Cantidad'].'" dataTipo="'.$row_services['P_SERVICIO'].'" dataProducto="'.$row_services['P_CODIGO'].'" dataexistencia="'.$row_services['P_EXISTENCIA_SOUVENIRS'].'" dataInventario="'.$row_services['P_LLEVA_EXISTENCIA'].'">'.$Nombre.' </a></h5>'; 
      }
?>