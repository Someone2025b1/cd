					...
					<div class="card">
						...
						<input type="hidden" name="TipoEvento" value="<?php echo $tipodereevento ?>">
							...
							<table class="table table-hover table-condensed" id="tbl_resultados">
								...
								<?php
									$Contador = 1;
									$Consulta = "SELECT * FROM Productos.NOTAS_CARGO_ABONO AS A WHERE A.NCA_FECHA BETWEEN '$fecha_inicio' AND '$fecha_fin' ORDER BY A.NCA_FECHA ";
									$Resultado = mysqli_query($db, $Consulta);
									while($row = mysqli_fetch_array($Resultado))
									{
										$Codigo = $row["NCA_CODIGO"];
										$EN = $row['PV_CODIGO'];
										...
										if($row['NCA_CARGO']==1){
											...
											$manda=0;
                                                ...
                                                echo '<td style="font-size: 10px">'.$row["NCA_CODIGO"].'</td>';
                                                ...
                                                echo '<td style="font-size: 10px">'.$row["NCA_FECHA"].' a las: '.$row["NCA_HORA"].' horas</td>';
										    	echo '<td><a href="NCAImp.php?Codigo='.$Codigo.'&Tipo='.$manda.'&Punto='.$EN.'&Fecha='.$row["NCA_FECHA"].'"><button type="button" class="btn btn-warning">
														     PDF
														  </button></a>
														</td>';
