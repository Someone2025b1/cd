...
if ($tipodereporte == 1){
    ...
    $QueryOrdenes = mysqli_query($db,"SELECT A.F_CODIGO, A.F_ORDEN, A.F_OBSERVACIONES, A.F_FECHA_TRANS, A.F_HORA, A.F_TOTAL
    ...
    while($FilaOrdenes = mysqli_fetch_array($QueryOrdenes))
    {
            ...
            if (($FilaOrdenes["F_FECHA_TRANS"] >= $fecha_inicio) and ($FilaOrdenes["F_FECHA_TRANS"] <= $fecha_fin)){
                ...
                if($fechatemp==$FechaOrden){
                    ...
                    $TablaProducto .= <<<EOD
                    <tr style="border: 2px;">
                        <td align="left"  style="font-size: 12px; font-weight: normal; width: 35%;"> #  $NoOrden </td>
...
if ($tipodereporte == 2){
    ...
    EOD;
    $QueryOrdenes = mysqli_query($db,"SELECT A.F_CODIGO, A.F_ORDEN, A.F_OBSERVACIONES, A.F_FECHA_TRANS, A.F_HORA
                                    FROM Bodega.FACTURA AS A 
                                    WHERE A.F_ESTADO = 1 
                                    ORDER BY A.F_FECHA_TRANS ASC");
    while($FilaOrdenes = mysqli_fetch_array($QueryOrdenes))
...
$pdf->Cell(0,0, "Restaurante Terrazas",0,1,'C');
