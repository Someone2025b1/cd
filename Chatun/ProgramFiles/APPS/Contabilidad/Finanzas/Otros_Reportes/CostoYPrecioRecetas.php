...
<div class="col-lg-12">
	...
	$Query = mysqli_query($db, "SELECT COUNT(*) AS CONT, sum(P_ULTIMO_COSTO * RD_CANTIDAD) as COSTO, A.*, B.R_NOMBRE, C.P_PRECIO_VENTA
	FROM Productos.RECETA_DETALLE AS A
	JOIN Productos.RECETA AS B ON  B.R_CODIGO = A.R_CODIGO
	JOIN Productos.PRODUCTO AS C ON  C.P_CODIGO = A.P_CODIGO
	GROUP BY A.RD_CODIGO
	ORDER BY CONT DESC");
					...
					<tbody>
						<?php
							...
							while($Fila = mysqli_fetch_array($Query))
							{
								...
								$queryp = "SELECT * FROM Productos.PRODUCTO WHERE PRODUCTO.R_CODIGO ='$Codigo'";
								$result = mysqli_query($db, $queryp);
								while($rowe = mysqli_fetch_array($result))
								{ 
									$PrecioVenta=$rowe["P_PRECIO_VENTA"];

									$queryb = "SELECT * FROM Productos.PRODUCTO WHERE PRODUCTO.P_CODIGO =130";
								$resultb = mysqli_query($db, $queryb);
								while($roweb = mysqli_fetch_array($resultb))
								{
									$precioBola=$roweb["P_ULTIMO_COSTO"];
								}

									$Bolas = $rowe["P_BOLA"]*$precioBola;
